...
import { AuditLine, EmptyBlock, ReadyChip, Section, SettingsPage } from "@/components/app/settings/shared";
      ...
      <Section title="Sending account">
        {state === "not-configured" ? (
          <EmptyBlock
            ...
            action={<Button size="sm" className="h-8 gap-1.5 text-[12px]"><Mail className="size-3.5" /> Connect Gmail</Button>}
          />
        ) : (
              ...
              <div className="flex flex-col items-end gap-2">
                ...
                <div className="flex gap-2">
                  {(state === "reconnect" || state === "error") && (
                    <Button size="sm" className="h-8 gap-1.5 text-[12px]"><RefreshCw className="size-3.5" /> Reconnect</Button>
                  )}
                  <Button size="sm" variant="outline" className="h-8 gap-1.5 text-[12px]" onClick={() => setDisconnect(true)}>
