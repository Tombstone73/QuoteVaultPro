...
export const WORKFLOW_INTENTS = ["Standard production", "Fulfillment only", "Billing only"] as const;
...
export const MATRIX_TIER_BASES = ["None", "Customer Quantity", "Computed Sheet Usage", "Total Sq Ft"] as const;
...
export const RECIPE_BASES = ["Per sq ft", "Per piece", "Per sheet", "Per linear ft", "Per order"] as const;
...
export const ROUTE_TEMPLATE_CATALOG: RouteTemplateRef[] = [
  { name: "Standard Production", description: "Default path for printed goods that need a customer proof before the floor picks them up.", revision: 1, status: "Active", owner: "Routing module", steps: ROUTE_TEMPLATES["Standard Production"]! },
  { name: "Print & Ship", description: "Reorders and approved-art jobs that skip proofing and ship directly.", revision: 3, status: "Active", owner: "Routing module", steps: ROUTE_TEMPLATES["Print & Ship"]! },
  { name: "Design First", description: "Jobs that start at the design desk before proofing and production.", revision: 2, status: "Active", owner: "Routing module", steps: ROUTE_TEMPLATES["Design First"]! },
  { name: "Billing only", description: "Service fees and billing-only products that never touch production.", revision: 1, status: "Active", owner: "Routing module", steps: ROUTE_TEMPLATES["Billing only"]! },
];

export const findRouteTemplate = (name: string) => ROUTE_TEMPLATE_CATALOG.find((t) => t.name === name);
...
export const PRICING_PROFILES = ["Default (Formula)", "Wholesale", "Contract — Municipal", "Rush"] as const;
...
function coroplast(): ProductDraft {
  return {
    ...
    shopName: "Coro",
    name: "Coroplast",
    description: "Coroplast signage — durable corrugated plastic, printed full color, indoor/outdoor.",
    category: "Rigid",
    productType: "Sheet",
    serviceFee: false,
    ...
    pricing: {
      ...
      formula: "sheet_consumption_sqft(w,h,q,sheet_width,sheet_length,usable_drop_min,billable_len)",
      ratePerSqFt: "1.375",
      sheetWidth: "48",
      sheetLength: "96",
      ratePerPiece: "0.00",
    ...
    matrix: {
      ...
      cells: {
        "mt1|4mm|Single-sided": "1.72", "mt1|4mm|Double-sided": "2.38",
        "mt1|10mm|Single-sided": "2.15", "mt1|10mm|Double-sided": "2.94",
        "mt2|4mm|Single-sided": "1.58", "mt2|4mm|Double-sided": "2.19",
        "mt2|10mm|Single-sided": "1.98", "mt2|10mm|Double-sided": "2.71",
        "mt3|4mm|Single-sided": "1.44", "mt3|4mm|Double-sided": "2.02",
        "mt3|10mm|Single-sided": "1.82", "mt3|10mm|Double-sided": "2.49",
        "mt4|4mm|Single-sided": "1.31", "mt4|4mm|Double-sided": "1.85",
        "mt4|10mm|Single-sided": "1.67", "mt4|10mm|Double-sided": "2.28",
      },
    },
    recipe: [
      { id: "rc1", materialId: "m1", basis: "Per sheet", factor: "1", unit: "sheet", conditionOptionId: "o-thick", conditionValue: "4mm", replaces: true, normalize: true },
      { id: "rc2", materialId: "m1", basis: "Per sheet", factor: "1", unit: "sheet", conditionOptionId: "o-thick", conditionValue: "10mm", replaces: true, normalize: true },
      { id: "rc3", materialId: "m4", basis: "Per piece", factor: "4", unit: "grommet", conditionOptionId: "o-grom", conditionValue: "Corners", replaces: false, normalize: false },
    ],
    production: [
      { id: "pu1", name: "Front", station: "Océ Arizona (Flatbed)", note: "Always produced.", conditionOptionId: undefined, conditionValue: undefined },
      { id: "pu2", name: "Back", station: "Océ Arizona (Flatbed)", note: "Second pass on the same sheet.", conditionOptionId: "o-sides", conditionValue: "Double-sided" },
    ],
    routing: { policy: "Route required", template: "Standard Production", steps: ROUTE_TEMPLATES["Standard Production"]! },
    version: {
      draftVersion: "v7 (draft)",
      activeVersion: "v6",
      lastPublished: "Aug 12, 2026",
      changes: [
        { section: "Pricing", label: "Rate — 4mm × Single-sided, 1–4 sheets", from: "$1.50 / sq ft", to: "$1.72 / sq ft" },
        { section: "Pricing", label: "Minimum charge", from: "$4.75", to: "$5.25" },
        { section: "Materials", label: "4mm Coroplast", from: "unchanged", to: "unchanged" },
        { section: "Production", label: "Conditional Back unit", from: "not present", to: "when Print Sides = Double-sided" },
      ],
    },
    ...
    groups: [
      {
        ...
        options: [
          option({
            ...
            required: true,
            choices: [
              choice({ label: "4mm", variantDefining: true, materialOverride: "4mm Coroplast 48x96", tags: "coro, rigid, thickness:4mm", materials: [{ id: uid("cm"), materialId: "m1", basis: "Area (sqft)", perUnit: 1 }] }),
              choice({ label: "10mm", variantDefining: true, materialOverride: "4mm Coroplast 48x96", tags: "coro, rigid, thickness:10mm", impacts: [{ id: uid("i"), kind: "Per sq ft", amount: 0.45 }] }),
            ],
          }),
        ],
      },
        ...
        options: [
          option({
            ...
            choices: [
              ...
              choice({ label: "Double-sided", impacts: [{ id: uid("i"), kind: "Percent of line", amount: 65 }] }),
            ],
          }),
        ],
      },
      {
        ...
        options: [
          option({
            ...
            choices: [
              choice({ label: "None" }),
              choice({ label: "Corners", impacts: [{ id: uid("i"), kind: "Flat", amount: 1.4 }], materials: [{ id: uid("cm"), materialId: "m4", basis: "Per item", perUnit: 4 }] }),
              choice({ label: "Every 2 feet", impacts: [{ id: uid("i"), kind: "Per piece", amount: 0.35 }] }),
              choice({ label: "Custom" }),
            ],
          }),
        ],
      },
      {
        ...
        options: [
          option({
            ...
            choices: [choice({ label: "No" }), choice({ label: "Yes", impacts: [{ id: uid("i"), kind: "Per piece", amount: 2.5 }] })],
          }),
        ],
      },
      {
        ...
        multiSelect: false,
        options: [
          option({ id: "o-gromnote", label: "Custom Grommet Instructions", inputType: "Text", help: "Describe the exact grommet layout." }),
        ],
      },
    ],
    rules: [
...
function banner(): ProductDraft {
  return {
    ...
    recipe: [
      { id: "brc1", materialId: "m2", basis: "Per sq ft", factor: "1", unit: "sq ft", replaces: true, normalize: true, conditionOptionId: undefined, conditionValue: undefined },
      { id: "brc2", materialId: "m4", basis: "Per piece", factor: "6", unit: "grommet", conditionOptionId: "bo-grom", conditionValue: "Every 24in", replaces: false, normalize: false },
    ],
    production: [
      { id: "bpu1", name: "Front", station: "HP Latex 570 (Roll)", note: "Always produced.", conditionOptionId: undefined, conditionValue: undefined },
    ],
    routing: { policy: "Route required", template: "Standard Production", steps: ROUTE_TEMPLATES["Standard Production"]! },
    version: {
      ...
      changes: [
        ...
        { section: "Options", label: "Pole Pockets group", from: "not present", to: "added with location and depth" },
      ],
    },
    flags: { proof: true, productionJob: true, allowZero: false, taxable: true },
    ...
    groups: [
      {
        ...
        options: [
          option({
            ...
            label: "Hem",
            required: true,
            choices: [choice({ label: "All sides" }), choice({ label: "Top & bottom" }), choice({ label: "None" })],
          }),
        ],
      },
      {
        ...
        options: [
          option({
            id: "bo-grom",
            label: "Grommet Placement",
            choices: [choice({ label: "Every 24in" }), choice({ label: "Corners only" }), choice({ label: "None" })],
          }),
        ],
      },
      {
        ...
        options: [
          option({
            ...
            choices: [choice({ label: "No" }), choice({ label: "Yes", impacts: [{ id: uid("i"), kind: "Per piece", amount: 6 }] })],
          }),
          option({
            id: "bo-poleloc",
            label: "Pole Pocket Location",
            choices: [choice({ label: "Top" }), choice({ label: "Bottom" }), choice({ label: "Top & bottom" }), choice({ label: "Left & right" })],
          }),
          option({
            id: "bo-poledepth",
            label: "Pole Pocket Depth",
            choices: [choice({ label: '2"' }), choice({ label: '3"' }), choice({ label: '4"' }), choice({ label: "Custom" })],
          }),
          option({ id: "bo-poledepthcustom", label: "Custom Pole Pocket Depth", inputType: "Text", help: "Finished sleeve depth in inches." }),
        ],
      },
    ],
    rules: [
...
export const findOption = (d: ProductDraft, id: string) => allOptions(d).find((r) => r.option.id === id);
...
export function evaluateRules(d: ProductDraft, sel: Record<string, string>): Record<string, OptionState> {
...
export function validateDraft(d: ProductDraft): Finding[] {
  ...
  if (d.pricing.source === "formula" && !d.pricing.formula.trim()) {
    out.push({ code: "PBV2_E_FORMULA_EMPTY", severity: "error", message: "Custom pricing formula is selected but empty.", section: "pricing" });
  }
  if (!d.matrix.enabled && Number(d.pricing.ratePerSqFt) <= 0 && Number(d.pricing.ratePerPiece) <= 0) {
    out.push({ code: "PBV2_E_NO_RATE", severity: "error", message: "No rate per sq ft or rate per piece is set — pricing resolves to the minimum charge only.", section: "pricing" });
  }
  if (d.matrix.enabled) {
    if (d.matrix.dimensionOptionIds.length === 0) {
      out.push({ code: "PBV2_E_MATRIX_NO_DIM", severity: "error", message: "Matrix pricing is on but no option group is used as a pricing dimension.", section: "pricing" });
    } else {
      const missing = countMissingMatrixCells(d);
      if (missing > 0) {
        out.push({ code: "PBV2_E_MATRIX_GAPS", severity: "error", message: `${missing} matrix cell${missing === 1 ? " is" : "s are"} empty — those combinations cannot be priced.`, section: "pricing" });
      }
    }
    if (d.matrix.tierBasis !== "None" && d.matrix.tiers.length === 0) {
      out.push({ code: "PBV2_W_NO_TIERS", severity: "warning", message: "A tier basis is selected but no tiers are configured.", section: "pricing" });
    }
  } else if (d.pricing.qtyTiers.length === 0 && d.pricing.sizeTiers.length === 0) {
    out.push({ code: "PBV2_I_NO_TIERS", severity: "info", message: "No quantity tiers configured — every quantity prices at list.", section: "pricing" });
  }
  for (const { group, option } of allOptions(d)) {
    if (option.inputType.includes("Choice") && option.choices.length === 0) {
      out.push({ code: "PBV2_W_NO_CHOICES", severity: "warning", message: `“${group.name} → ${option.label}” is a choice option with no choices.`, section: "options" });
    }
  }
  for (const r of d.rules) {
    if (!findOption(d, r.sourceOptionId)) {
      out.push({ code: "PBV2_E_RULE_SOURCE", severity: "error", message: `Rule “${r.label}” points at an option that no longer exists.`, section: "options" });
    }
  }
  if (!d.serviceFee && d.recipe.length === 0) {
    out.push({ code: "PBV2_W_NO_RECIPE", severity: "warning", message: "No recipe lines — this product consumes no inventory.", section: "materials" });
  }
  for (const line of d.recipe) {
    if (line.conditionOptionId && !findOption(d, line.conditionOptionId)) {
      out.push({ code: "PBV2_E_RECIPE_COND", severity: "error", message: "A recipe line is conditioned on an option that no longer exists.", section: "materials" });
    }
  }
  if (d.flags.productionJob && d.production.length === 0) {
    out.push({ code: "PBV2_W_NO_UNITS", severity: "warning", message: "Product creates a production job but defines no production units.", section: "production" });
  }
  if (d.routing.policy === "Unconfigured") {
    out.push({ code: "PBV2_W_ROUTE_UNSET", severity: "warning", message: "Routing policy is unconfigured — orders will need manual routing.", section: "routing" });
  }
  if (d.routing.policy === "Route required" && d.routing.steps.length === 0) {
    out.push({ code: "PBV2_E_ROUTE_EMPTY", severity: "error", message: "A route is required but the selected template has no steps.", section: "routing" });
  }
  if (!d.aiUseDescription && !d.aiDescription.trim()) {
    out.push({ code: "PBV2_I_AI_HINT", severity: "info", message: "No AI parsing description — inbound matching will rely on the product name only.", section: "basics" });
  }
  return out;
}
...
export function countMissingMatrixCells(d: ProductDraft): number {
  ...
  const tiers = d.matrix.tiers.length ? d.matrix.tiers : [{ id: "t0", label: "All", from: "", to: "" }];
  let missing = 0;
  for (const t of tiers) for (const combo of combos) {
...
export interface PreviewResult {
  ...
  rate?: { value: number; unit: MatrixUnit | "per sq ft" | "per piece"; source: string } | undefined;
  base: number;
...
export function computePreview(d: ProductDraft, inputs: PreviewInputs, sel: Record<string, string>): PreviewResult {
  const state = evaluateRules(d, sel);
  const blockers: string[] = [];
  for (const { group, option } of allOptions(d)) {
    const s = state[option.id];
    if (s?.visible && s.required && !sel[option.id]) blockers.push(group.name === option.label ? option.label : `${group.name}: ${option.label}`);
  }
  ...
  if (matrixActive) {
    const basisValue =
      d.matrix.tierBasis === "Computed Sheet Usage" ? sheets
      : d.matrix.tierBasis === "Total Sq Ft" ? sqft
      : qty;
    ...
    if (values.every(Boolean) && tier) {
      ...
      if (cell !== undefined && cell !== "") rate = { value: Number(cell) || 0, unit: d.matrix.unit, source: `Matrix · ${matrixSelection}` };
    }
  ...
  if (!rate) {
    ...
    rate = perSqFt > 0
      ...
      : { value: Number(d.pricing.ratePerPiece) || 0, unit: "per piece", source: "Rate per piece" };
  }

  let base =
    rate.unit === "per sq ft" ? sqft * rate.value
    : rate.unit === "per piece" ? qty * rate.value
    : rate.value;
  ...
  for (const { option } of allOptions(d)) {
    ...
    for (const im of picked.impacts) {
      const amount =
        im.kind === "Flat" ? im.amount
        : im.kind === "Per piece" ? im.amount * qty
        : im.kind === "Per sq ft" ? im.amount * sqft
        : (base * im.amount) / 100;
      adders.push({ label: `${option.label}: ${picked.label} · ${im.kind}`, amount: Number(amount.toFixed(2)) });
    }
  }
  ...
  const weight = [
    ...
    { label: "Computed weight", value: d.material.configuredWeight ? `${(qty * parseFloat(d.material.configuredWeight)).toFixed(2)} lb` : "0.00 lb" },
  ...
  const recipe = d.recipe.map((line) => {
    ...
    const qtyOut =
      line.basis === "Per sheet" ? sheets * (Number(line.factor) || 0)
      : line.basis === "Per piece" ? qty * (Number(line.factor) || 0)
      : line.basis === "Per sq ft" ? sqft * (Number(line.factor) || 0)
  ...
  const production = d.production.map((u) => {
    ...
    return {
      ...
      required,
      station: u.station,
      reason: u.conditionOptionId ? `${cond?.option.label ?? "Option"} = ${u.conditionValue}` : "Always",
    };
  });
...
export function ruleSentence(d: ProductDraft, r: RuleCard): string {
  ...
  const targets = r.targetOptionIds.map((id) => findOption(d, id)?.option.label ?? "(missing)").join(", ");
