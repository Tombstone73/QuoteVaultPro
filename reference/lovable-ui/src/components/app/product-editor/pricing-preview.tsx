...
export function PricingPreview({
  ...
  return (
    <div className="space-y-3">
      <Card
        icon={<Calculator className="size-3.5 text-primary" />}
      ...
      >
          ...
          {visible.map(({ group, option }) => {
            ...
            return (
              <Cell
                key={option.id}
                label={
                  group.name === option.label ? option.label : `${group.name} → ${option.label}`
                }
              >
                {items.length > 0 ? (
                  <Picker
                    value={sel[option.id] ?? s.forcedDefault ?? ""}
                    items={items}
                    onChange={(v) => setSel({ ...sel, [option.id]: v })}
                  />
                ) : (
                  <Input
                    className="h-8 text-[13px]"
                    placeholder="Free text at quote time"
                    value={sel[option.id] ?? ""}
                    onChange={(e) => setSel({ ...sel, [option.id]: e.target.value })}
                  />
                )}
        ...
        <dl className="mt-2.5 space-y-1 border-t border-border pt-2 text-[12px]">
          ...
          {result.matrixSelection && (
            <Row label="Matrix selection" value={result.matrixSelection} />
          )}
          {result.rate && (
            <Row label="Rate" value={`${money(result.rate.value)} ${result.rate.unit}`} />
          )}
          <Row label="Base" value={money(result.base)} />
          {result.adders.map((a, i) => (
            <Row key={i} label={a.label} value={money(a.amount)} muted />
          ))}
          {result.adders.length === 0 && <Row label="Option impacts" value={money(0)} muted />}
          <Row
            label="Minimum charge"
            value={`${money(result.minimum)} — ${result.minimumApplied ? "applied" : "not applied"}`}
            muted
          />
        </dl>
      ...
      <Card title="Material resolution">
        <dl className="space-y-1 text-[12px]">
          ...
          {draft.productType === "Sheet" && (
            <Row
              label="Sheet size"
              value={`${draft.pricing.sheetWidth ?? 48} × ${draft.pricing.sheetLength ?? 96}`}
              muted
            />
          )}
          {result.recipe.map((r, i) => (
            <Row key={i} label={r.label} value={r.detail} muted={!r.active} />
          ))}
        ...
        <dl className="space-y-1 text-[12px]">
          {result.production.map((p) => (
            <div key={p.name} className="flex items-baseline justify-between gap-3">
              <dt className={p.required ? "" : "text-muted-foreground"}>
                {p.name}
                <span className="ml-1.5 text-[11px] text-muted-foreground">{p.reason}</span>
              </dt>
              <dd
                className={`shrink-0 text-[12px] font-medium ${p.required ? "text-ok" : "text-muted-foreground"}`}
              >
      ...
      <Card title="Weight resolution">
        <dl className="space-y-1 text-[12px]">
          {result.weight.map((w) => (
            <Row key={w.label} label={w.label} value={w.value} muted />
          ))}
        </dl>
      </Card>
      ...
      >
        <div className="space-y-2">
          ...
          {findings.map((f) => (
            <FindingRow key={f.code + f.message} f={f} onJump={onJump} />
          ))}
        </div>
      </Card>
    </div>
    ...
    <div className="flex gap-2">
      ...
      <div className="min-w-0">
        ...
        <button
          ...
          className="num text-[10px] uppercase tracking-wide text-muted-foreground hover:text-primary hover:underline"
        >
