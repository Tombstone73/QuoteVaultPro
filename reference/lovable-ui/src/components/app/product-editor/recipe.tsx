    ...
    <div className="space-y-2.5">
      ...
      <div className="space-y-2">
        {draft.recipe.map((line, i) => {
          const set = (fn: (l: RecipeLine) => void) =>
            patch((d) => {
              fn(d.recipe[i]!);
            });
          const cond = line.conditionOptionId
            ? findOption(draft, line.conditionOptionId)
            : undefined;
          return (
            <div key={line.id} className="rounded-md border border-border p-2.5">
              <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  {line.replaces ? (
                    <Chip tone="accent">Replaces primary</Chip>
                  ) : (
                    <Chip>Additional</Chip>
                  )}
                  {cond ? (
                    <span className="truncate text-[11px] text-muted-foreground">
                      When {cond.option.label} = {line.conditionValue}
                    </span>
                  ) : (
              ...
              <div className="mt-2 grid gap-2 @container sm:grid-cols-2 lg:grid-cols-4">
                <Cell label="Material">
                  <Picker
                    value={nameOf(line.materialId)}
                    items={materialNames}
                    onChange={(v) =>
                      set((l) => {
                        l.materialId = materials.find((m) => m.name === v)!.id;
                      })
                    }
                  />
                </Cell>
                ...
                <Cell
                  ...
                  hint="Leave as Always for unconditional consumption."
                >
                  <Picker
                    ...
                    onChange={(v) =>
                      set((l) => {
                        if (v === "Always") {
                          l.conditionOptionId = undefined;
                          l.conditionValue = undefined;
                          return;
                        }
                ...
                <Toggle
                  label="Replaces primary material"
                  checked={line.replaces}
                  onChange={(v) =>
                    set((l) => {
                      l.replaces = v;
                    })
                ...
                />
                <Toggle
                  label="Normalize to inventory unit"
                  hint="Convert computed usage into the material's stocking unit."
                  checked={line.normalize}
                  onChange={(v) =>
                    set((l) => {
                      l.normalize = v;
                    })
