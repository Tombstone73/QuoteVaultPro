    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-[11px] text-muted-foreground">
          {p.mode === "Basic"
            ? "Rates and minimums only — enough for most products."
            : "Formula sources, sheet nesting and tier tables."}
        </span>
        <HelpCircle className="size-3.5 text-muted-foreground" />
      </div>

      <div className="grid gap-3 @container sm:grid-cols-3">
        <Cell label="Rate per sq ft">
          <MoneyInput
            value={p.ratePerSqFt}
            onChange={(v) =>
              setP((c) => {
        ...
        <Cell label="Rate per piece">
          <MoneyInput
            value={p.ratePerPiece}
            onChange={(v) =>
              setP((c) => {
                c.ratePerPiece = v;
              })
        ...
        <Cell label="Tier basis" hint="Which quantity chooses the price break.">
          <Picker
            value={p.tierBasis}
            items={["Computed Sheet Usage", "Customer Quantity", "Total Sq Ft"] as const}
            onChange={(v) =>
              setP((c) => {
                c.tierBasis = v;
              })
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">
                Use lowercase variables such as w, h, q, sqft, total_sqft, base_price. Use ceil(…),
                round(…), max(…) — not Math.ceil(…).
              </p>
              <button
                type="button"
                ...
                <dl className="mt-1.5 grid gap-x-4 gap-y-1 rounded-md border border-border bg-surface-2 p-2.5 text-[11px] sm:grid-cols-2">
                  {PRICING_VARIABLES.map(([name, desc]) => (
                    <div key={name} className="flex gap-2">
                      <dt className="num shrink-0 font-medium">{name}</dt>
          ...
          <div className="rounded-md border border-border p-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              >
                ...
                Add {tierTab === "qty" ? "qty" : "size"} tier
              </Button>
            </div>
            {tiers.length === 0 ? (
              <p className="mt-2.5 text-[12px] italic text-muted-foreground">
                No tiers — every quantity prices at list.
              </p>
            ) : (
                ...
                {tiers.map((t, i) => (
                  <div key={t.id} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2">
                    <Input
                      ...
                      onChange={(e) =>
                        setP((c) => {
                          (tierTab === "qty" ? c.qtyTiers : c.sizeTiers)[i]!.from = e.target.value;
                        })
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setP((c) => {
                          (tierTab === "qty" ? c.qtyTiers : c.sizeTiers)[i]!.to = e.target.value;
                        })
                      }
                    />
                    <Input
                      ...
                      onChange={(e) =>
                        setP((c) => {
                          (tierTab === "qty" ? c.qtyTiers : c.sizeTiers)[i]!.adjust =
                            e.target.value;
                        })
                      }
                    />
                    <Button
                      ...
                      onClick={() =>
                        setP((c) => {
                          (tierTab === "qty" ? c.qtyTiers : c.sizeTiers).splice(i, 1);
                        })
                      }
                    >
      ...
      <p className="text-[11px] text-muted-foreground">
        Pricing changes are saved to the product draft when you click Save Changes.
      </p>
    </div>
  );
}
...
function MoneyInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div className="relative">
      ...
      <Input
        className="num h-8 pl-5 text-[13px]"
        value={value}
