...
import {
  Copy, Image as ImageIcon, RotateCcw, Trash2, Upload, X,
} from "lucide-react";
...
import {
  computeMaterials, computeUsage, formatSize, optionDefsFor, parseSize, productById, requiresDimensions,
...
export function LineEditor({
  docNumber, mode, line, onClose, onCommit, onDelete, onDuplicate,
}: {
  docNumber: string;
  mode: "edit" | "new";
  line: LineItem;
  ...
  useEffect(() => { setDraft(line); setState(mode === "new" ? "dirty" : "clean"); }, [line, mode]);

  const patch = (p: Partial<LineItem>) => { setDraft((d) => ({ ...d, ...p })); setState("dirty"); };
  ...
  const doubleSided = draft.options.some((o) => o.label === "Sides" && o.value === "Double");
  ...
  const changeProduct = (productId: string) => {
    ...
    patch({
      ...
      options: nextDefs.map((d) => {
        ...
        return { label: d.label, value: kept && d.choices.includes(kept.value) ? kept.value : d.default };
      }),
    ...
    <aside className="@container flex h-full min-w-0 flex-col bg-surface">
      <header className="flex items-start justify-between gap-2 border-b border-border px-3 py-2">
        <div className="min-w-0">
          <div className="truncate text-[14px] font-semibold">{mode === "new" ? "New line item" : product?.name}</div>
          <div className="num truncate text-[11px] text-muted-foreground">
            {product?.sku} · {mode === "new" ? "Not yet added" : `Line on #${docNumber}`}
          </div>
        </div>
        <button type="button" onClick={onClose} aria-label="Close line editor" className="rounded p-1 hover:bg-accent">
          <X className="size-4" />
        </button>
      </header>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 content-start gap-3 overflow-y-auto p-3 @[620px]:grid-cols-2 @[980px]:grid-cols-3">
        <Section title="Product" className="@[620px]:col-span-2 @[980px]:col-span-3">
          ...
          <InlineSelect
            ...
            options={products.map((p) => ({ value: p.id, label: p.name, hint: `${p.sku} · ${p.basis}` }))}
          ...
          <div className="mt-2 grid gap-1 @[620px]:mt-0">
            ...
            <Input value={draft.description} className="h-8 text-[13px]" onChange={(e) => patch({ description: e.target.value })} />
          </div>
          </div>
        </Section>
        ...
        <Section title="Configuration" className="@[620px]:col-span-2">
          {defs.length === 0 ? (
            <p className="text-[12px] text-muted-foreground">This product has no configurable options.</p>
          ) : (
        ...
        <Section title={dims ? "Dimensions & quantity" : "Quantity"} className="@[620px]:col-span-1 @[980px]:col-span-2">
          <div className="flex flex-wrap items-end gap-x-3 gap-y-2">
            {dims && (
              <>
                <Field label={`Width (${size.unit})`}>
                  <Input value={size.w} className="num h-8 w-20" onChange={(e) => setDim("w", e.target.value)} />
                </Field>
                <span className="pb-2 text-muted-foreground">×</span>
                <Field label={`Height (${size.unit})`}>
                  <Input value={size.h} className="num h-8 w-20" onChange={(e) => setDim("h", e.target.value)} />
                </Field>
              </>
            )}
            <Field label="Quantity (pcs)">
              <Input
                type="number" min={1} value={draft.qty} className="num h-8 w-24"
                onChange={(e) => patch({ qty: Number(e.target.value) || 0 })}
              />
            </Field>
            {dims && draft.size && <span className="num pb-2 text-[12px] text-muted-foreground">{draft.size}</span>}
          </div>
        ...
        <Section title="Pricing" className="@[980px]:col-span-1">
          <div className="flex items-center justify-between text-[13px]">
            <span className="text-muted-foreground">Calculated unit price <span className="text-[10px] uppercase">(Pricing)</span></span>
            <span className="num">{money(draft.calcUnit)}</span>
          </div>
          <div className="mt-2 grid gap-1">
            <Label className="text-[11px] uppercase text-muted-foreground">Selling unit price</Label>
            <div className="flex flex-wrap items-center gap-1.5">
              <Input
                type="number" step="0.01" value={draft.sellUnit} className="num h-8 w-32"
                onChange={(e) => patch({
                  sellUnit: Number(e.target.value) || 0,
                  overrideReason: "Manual entry", overrideBy: "Dale",
                })}
              />
              {overridden && (
                <Button
                  size="sm" variant="outline" className="h-8 gap-1 text-[11px]"
                  onClick={() => patch({ sellUnit: draft.calcUnit, overrideReason: undefined, overrideBy: undefined })}
                >
          ...
          {overridden && (
            <div className="mt-2 flex flex-wrap items-center justify-between gap-1 text-[11px]">
              <span className="rounded border border-warn/50 bg-warn/15 px-1 font-semibold uppercase text-warn">Manual price</span>
              <span className="text-muted-foreground">{draft.overrideReason} — {draft.overrideBy ?? "Dale"}</span>
            </div>
          )}
          <div className="mt-2 flex justify-between border-t border-border pt-2 text-[13px] font-semibold">
            <span>Line total</span><span className="num">{money(lineTotal(draft))}</span>
          </div>
        </Section>

        {needs.length > 0 && (
          <Section title="Material / inventory need" className="@[620px]:col-span-2 @[980px]:col-span-3">
            <div className="grid gap-1 @[620px]:grid-cols-2 @[980px]:grid-cols-3">
              {needs.map((n) => (
                <div key={n.material} className="flex items-baseline justify-between gap-2 rounded border border-border/70 bg-surface-2/40 px-2 py-1">
                  ...
                  <span className="shrink-0 text-right">
                    ...
                    {n.available && <span className="num block text-[10px] text-muted-foreground">{n.available}</span>}
                  </span>
                </div>
              ))}
            </div>
        ...
        <Section title="Line item notes" className="@[620px]:col-span-2 @[980px]:col-span-1">
          ...
          />
          <p className="mt-1 text-[11px] text-muted-foreground">Applies to this line item, not the whole order.</p>
        </Section>
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (f) toast.success(`${f.name} attached to this line item`);
                  e.target.value = "";
                }}
              />
              <Button size="sm" className="h-7 gap-1 text-[11px]" onClick={() => fileRef.current?.click()}>
                <Upload className="size-3" /> Upload art
              </Button>
              <Button size="sm" variant="outline" className="h-7 text-[11px]" asChild>
                <Link to="/artwork">Open artwork</Link>
              </Button>
            </div>
          </div>
          <div className="mt-2 grid grid-cols-2 gap-2 @[620px]:grid-cols-3 @[980px]:grid-cols-4">
            <ArtTile kind="Line item art" name="customer-art.pdf" side={doubleSided ? "Front" : undefined} />
            <ArtTile kind="Production art" name="print-ready.pdf" side={doubleSided ? "Back" : undefined} ready />
          </div>
        </Section>
      </div>
      ...
      <footer className="flex items-center justify-between gap-2 border-t border-border px-3 py-2">
        <div className="flex items-center gap-1">
          {mode === "edit" && (
            <>
              <Button size="sm" variant="ghost" className="h-8 gap-1 px-2 text-[12px]" onClick={onDuplicate}>
                <Copy className="size-3.5" /> Duplicate
              </Button>
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button size="sm" variant="ghost" className="h-8 gap-1 px-2 text-[12px] text-destructive">
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      {product?.name} · Qty {draft.qty} · {money(lineTotal(draft))} will be removed from #{docNumber}.
                      This is recorded in Order history.
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
...
function ArtTile({
  kind, name, side, ready,
}: { kind: string; name: string; side?: string | undefined; ready?: boolean }) {
  return (
    <div className="rounded border border-border p-1.5">
      <div className="flex h-14 items-center justify-center rounded bg-surface-2 text-muted-foreground">
        <ImageIcon className="size-5" aria-hidden />
      </div>
      <div className="mt-1 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">
        {kind}{side ? ` · ${side}` : ""}
      </div>
      <div className="num truncate text-[11px]">{name}</div>
      {ready && (
        <span className={cn("mt-1 inline-block rounded border border-ok/50 bg-ok/15 px-1 text-[10px] font-semibold text-ok")}>
          Production ready
        </span>
      )}
    </div>
