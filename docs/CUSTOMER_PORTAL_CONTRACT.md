...
- A submission is stored as a customer-visible `customer_upload` attachment with uploader identity, original filename, entity link, optional customer note, and an audit event. It starts at the durable `pending_review` status; staff can explicitly accept or reject it with an optional customer-visible note.
- Acceptance is review only. A separate staff-only, confirmed promotion can promote an accepted quote upload to an approved reference, or an accepted order upload to an approved reference or non-primary artwork reference. Promotion is audited and never makes final art automatically.
- A separate staff-only, confirmed assignment can associate a promoted order artwork reference with a selected line item for staff reference. Assignment does not enter the primary artwork-side workflow and is never a final-art, proof, prepress, production, billing, payment, or EPS action.
- A separate staff-only, confirmed artwork-side intake can make an assigned artwork reference available to existing line-item artwork-side controls. Intake starts with no side and no primary/final-art designation; any later side or final-art action remains separately controlled by existing workflows.
- A separate staff-only, confirmed artwork-side designation can assign Front, Back, or Both only after intake selection. It follows existing side-conflict rules, remains non-primary and not final art, and does not invoke prepress, proof, production, billing, payment, or EPS workflows.
- A separate staff-only, confirmed primary-artwork-candidate selection can identify a side-designated customer reference as a candidate without setting the operational primary-art flag. Candidate conflicts are limited to the line-item side scope, are audited, and do not invoke final-art, proof, prepress, production, billing, payment, or EPS workflows.
- Staff review and promotion do not mark final art, complete prepress, approve a proof, create production work, or alter billing/payment state. Existing safe workflows remain responsible for any later artwork or workflow change.
- Customer submissions begin as reference files only: until separately staff-promoted, assigned, and selected for artwork-side intake, they are not linked to a production line item. They are never made primary artwork or allowed to advance quote, order, proof, fulfillment, invoice, or payment state automatically.
