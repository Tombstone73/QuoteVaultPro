...
| Customers list | Yes: `/customers`; no V2 list route. | Customer detail read exists; no Customer list/create/update HTTP contract. | Lovable list/filter and metrics are mock; V2 has no equivalent list. | MISSING. Customers owns identity/contact; add a narrow authoritative list/read projection, not Finance-owned customer state. | After/alongside customer detail convergence. `customers.index.tsx`; `customerRoutes.ts`. |
| Customer detail / contacts | Yes: `/customers/$id`; V2 `customers` page is reachable only from Finance/Sales links. | `GET /customers/:customerId` exists; no Customer/contact edit, add-contact, notes, task, activity, credit, or pipeline contract. | V2 data is real canonical identity/contact presentation; Lovable detail is mock-rich and mutable. V2 intentionally omits Orders/Invoices history. | PROVISIONAL. Customers owns identity/contact. Replace the temporary detail with approved layout, then add only owner projections/operations that are actually approved. | High priority. `CustomerWorkspace.tsx`; `customerRoutes.ts`; `M3_PAYMENTS_REFUNDS_PROVIDER_FOUNDATION.md`. |
...
| Products catalog/list | Yes: `/products`; V2 has sidebar label but no routed workspace. | Real Product/PBV2 reads exist behind Quote form endpoints (`/quotes/form/products`, configuration, resolve); no Product workspace endpoint. | Real Product data serves Quote editor, but no real product screen; Lovable list is mock. | MISSING. Products/PBV2 owns configuration. Create a read-only routed catalog adapter using a dedicated projection or explicitly adapted existing read; no new Product persistence. | Recommended first M4 implementation slice. `products.tsx`; `quoteRoutes.ts`; `postgresProductsRead.ts`. |
| Product detail / Product Builder | Approved `/products/$id` and `/product-builder`; no V2 route. | Existing read/configuration resolution only; no V2 Product edit/publish contract exposed. | Lovable fields (pricing, route, storefront) are mock/local. | MISSING for read presentation; MOCK-ONLY for mutations. Products/PBV2 owns product configuration; Routing owns route templates; Settings/Portal owns storefront settings. | Follows catalog read slice and requires operation-by-operation contracts. `products.$id.tsx`, `product-builder.tsx`. |
...
* Static: inspected master-plan M4 numbering, M0--M3 architecture documents, reference route inventory, V2 React routes/workspaces, V2 HTTP routers, API client contracts, and Product compatibility reader. Confirmed `1019cd26` is reachable from audited HEAD.
...
* Automated, clone, Playwright/browser, visual, DEV, and MAIN validation: not run for this documentation-only audit. No deployment occurred.
