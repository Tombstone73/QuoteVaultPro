...
| Artwork | source attachments and operator preview/download | canonical Artwork assignment, immutable source/derived lineage, protected generic open/download, and proof/prepress ownership | SUPERSEDES | M7.8G freezes current Production-Art identity for each Run, rejects stale preparation and retains started execution evidence; original-file bundle scope remains an owner decision |
...
| Prepress | work queue and handoff | server-paged search, station/readiness filters, bounded page selection, atomic bulk frozen-destination handoff and per-unit audit | SUPERSEDES | Persistent ordered Runs now supersede the V1 Combined Run membership outcome; return-to-Prepress successor cycles remain P1 |
| Flatbed and Roll | station-specific workboards | common canonical attempts/quantity authority with station presentation, artwork, material, Traveler, good and waste output context | SUPERSEDES | M7.8G adds shared Run execution/history/recovery; machine/printer/calendar controls remain P1 |
| Fulfillment | shipment/pickup records | immutable quantity handoffs, partial/multiple events, recoverable prepared containers, manual shipment metadata and combined compatible shipments | SUPERSEDES | P1: decide whether a combined-container document and post-shipped physical reversal model are required |
...
| Orders | create/edit header, customer/contact, PO/due/terms, fulfillment request | Sales entry and Order application with frozen destination snapshot/instructions | PARITY | M7.8E freezes intent changes after the first handoff |
...
| Orders | per-line notes, duplicate line, persisted line reorder | V1 line dialog / `OrderLineItemsSection.tsx` | staff-only operational line note; duplicate re-resolves current Product pricing; exact-set reorder persists presentation sequence | SUPERSEDES | pricing evidence/line identities are never copied during duplication |
| Orders | ship/pickup/delivery intent, destination and instructions | V1 `OrderFulfillmentPanel.tsx` | Sales-owned structured intent and destination snapshot projected to Fulfillment; post-handoff intent changes fail closed | SUPERSEDES | physical handoffs stay Fulfillment-owned |
...
| Prepress | station/status filters, bounded selection and bulk release | V1 board has unpaged/unbounded filters and client-loop release | V2 has server-paged search, frozen station + readiness filters, 50-item page selection and all-or-nothing canonical handoff | SUPERSEDES | M7.8F: no browser mutation loop or partial result ambiguity |
| Prepress | persistent combined Runs / nesting membership | V1 creates `production_runs`/members with downstream authority | persistent tenant-scoped Run/allocation order and reservation evidence | SUPERSEDES | M7.8G closes the durable membership outcome; no V1 layout geometry was source-proven |
| Prepress | original-files ZIP | narrow V1 per-line stream | scoped private individual Artwork access; no multi-file bundle | OWNER DECISION REQUIRED | V1 existence does not prove a bundle is launch routine; no unsafe ZIP loop was added |
| Prepress | material override / standalone preflight | V1 controls | material snapshots/readiness only; no free-form effective-material mutation | OWNER DECISION REQUIRED | inventory/commercial authority requires a bounded owner-approved design |
...
| Production | partial good quantity/no overproduction/history | V2 attempt/output evidence plus Run allocation/reconciliation ledger | SUPERSEDES | M7.8G terminal disposition prevents release/cancel from receiving completion credit |
| Production | persistent Run execution and operator history | V1 durable runs/members with downstream use | canonical Flatbed/Roll Run domain, attempts/output, ordered events, idempotent commands and shared station detail history | SUPERSEDES | Run state never directly completes Production, Fulfillment, or Orders |
| Production | stale Artwork / preparation refresh | V1 run source does not safely define post-preparation substitution | immutable prepared artwork identity, stale Start conflict and explicit Draft/Ready refresh | SUPERSEDES | started Run retains the artwork used for execution |
| Production | release/cancel recovery and normal successor Run | V1 run allocation lifecycle | non-success attempt disposition, unused-reservation release and normal next-Run reservation | SUPERSEDES | aggregate canonical good output alone satisfies the production obligation |
| Production | Run Sheet document | no source-proven V1 Run Sheet/PDF workflow or required operator outcome | none added | NOT REQUIRED / UNPROVEN | do not create a duplicate document renderer without a bounded launch requirement |
| Production | persistent placement/layout geometry | no source-proven geometry beyond ordered Run membership | no geometry added | SUPERSEDED | durable ordered Run membership closes the proven V1 outcome; an optimizer remains out of scope |
...
| Production | return-to-Prepress / production-art successor | V1 mutable production-return service | immutable linked successor Prepress/Production cycle; original route/work/attempt/output remain historical | SUPERSEDES | M7.8H uses remaining required quantity only and rejects active Run reservations until released |
| Production | partial accepted-good return | V1 return path | predecessor accepted output remains immutable/fulfillable; successor owns remaining quantity | SUPERSEDES | Cases A/B are canonical; successor request itself grants no Fulfillment quantity |
| Production | previously accepted-good rejection/scrap | V1 behavior not safely adopted | append-only, attributed output disposition; usable good is reduced without changing attempt history | SUPERSEDES | bounded by unfulfilled usable supply, blocks prepared shipment reservations, and reopens normal Production reconciliation |
...
| Fulfillment | reopen prepared shipment, correction/void/reversal | V1 shipment detail/recovery | append-only prepared revisions, server-owned reservations, correction/void, and atomic prepared-to-shipped finalization with immutable handoffs/snapshots | SUPERSEDES | shipped quantity reversal remains an explicit owner decision; do not unship immutable history |
