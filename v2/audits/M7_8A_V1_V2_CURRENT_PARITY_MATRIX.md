...
| Artwork | source attachments and operator preview/download | canonical Artwork assignment, immutable source/derived lineage, protected generic open/download, and proof/prepress ownership | SUPERSEDES | M7.8F confirms source/current Production Art context and revision upload are reachable; persistent nesting and original-file bundle scope remain P1 |
...
| Prepress | work queue and handoff | server-paged search, station/readiness filters, bounded page selection, atomic bulk frozen-destination handoff and per-unit audit | SUPERSEDES | Persistent nesting/run and successor-cycle exceptions remain P1 owner decisions |
...
| Prepress | persistent combined runs/nesting and exception-run recovery | V1 creates `production_runs`/members with downstream authority | no V2 run entity; atomic bulk handoff intentionally does not claim nesting | OWNER DECISION REQUIRED | P1: approve a V2 production-run domain only if launch volume requires it |
| Prepress | material override / original-files ZIP / standalone preflight | V1 controls | V2 material snapshots/readiness and private individual Artwork access; no controlled effective-material override or bundle stream | PARTIAL | material override is inventory/commercial authority; ZIP scope and standalone utility require owner decisions |
