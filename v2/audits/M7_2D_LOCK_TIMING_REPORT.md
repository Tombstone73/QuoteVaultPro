...
Status: source-only planning. No timings, relation sizes, lock samples, or clone measurements are claimed here.

The executor serializes reconciliation with a dedicated advisory lock. Stage DDL is expected to run in short transactions; any future batch backfill must expose progress and avoid unbounded transaction growth. No provider or application runtime is required for the measurement.
