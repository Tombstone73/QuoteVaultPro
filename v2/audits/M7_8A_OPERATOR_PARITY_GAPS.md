...
| P1-03 | Artwork/Prepress | **PARTIAL:** persistent batch/nesting, exception-run successor cycles, controlled material override and any approved original-file bundle | M7.8F closes the safe bounded throughput gap with server-paged search, frozen station/readiness filters, 50-item page selection and all-or-nothing canonical bulk handoff; current Production Art/revision and private individual access remain canonical | decide whether the V1 persistent `production_runs` model, effective-material/inventory authority, and a bounded original-file bundle are required at launch; do not substitute UI grouping or a client loop |
