...
const createFixture = async (client: PoolClient): Promise<BrowserFixture> => {
  ...
  try {
    ...
    for (const [org, set] of [[f.organizationA, f.salesSetA], [f.organizationB, salesSetB]] as const)
      for (const capability of ["quote.view", "quote.create", "quote.edit", "quote.send", "quote.convert", "quote.overridePrice", "order.view", "order.edit", "order.overridePrice", "invoice.view", "invoice.issue", "payment.view", "payment.record", "refund.issue", "artwork.view", "artwork.adopt", "artwork.assign", "proof.view", "proof.prepare", "proof.issue", "proof.respond", "prepress.view", "prepress.work", "prepress.complete", "production.view", "production.work", "production.complete", "fulfillment.view", "fulfillment.pickup", "fulfillment.ship"])
