...
async function main(){
  ...
  try {
    ...
    assert.equal([first,competing].filter(result=>result.ok).length,1,"concurrent requests did not produce exactly one successful Draft");assert.equal(await draftCount(pool,concurrent),1,"concurrent requests created ambiguous Draft state");
