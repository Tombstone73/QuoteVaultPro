...
export type ProductDraftFormulaPricing = Readonly<{ productId:string; draftVersionId:string; draftUpdatedAt:string; lifecycle:"draft"; source:"embedded_editable"|"library_product_inputs_editable"|"library_reference_read_only"|"unsupported_legacy"; editable:boolean; expressionEditable:boolean; variablesEditable:boolean; rotationEditable:boolean; inputs:readonly ProductFormulaInput[]; unavailableReason?:string; formulaId?:string; formulaName?:string; expression:string; /** Exact Product-row compatibility expression; never accepted from a client write. */ legacyExpression?:string; canAdoptLegacyFormula?:boolean; variables:Readonly<Record<string,number>>; allowRotation:boolean; rotationControl?:ProductRotationControl; supportedRuntimeVariables:readonly string[]; warnings:readonly string[] }>;
export type UpdateProductDraftFormulaPricingInput = Readonly<{ productId:string; draftVersionId:string; expectedDraftUpdatedAt:string; businessRequestId:string; expression:string; variables:Readonly<Record<string,number>>; allowRotation:boolean; rotationControl?:ProductRotationControl }>;
/** Copies the current Product-row compatibility expression into one existing Draft. */
export type AdoptLegacyProductFormulaInput = Readonly<{ productId:string; draftVersionId:string; expectedDraftUpdatedAt:string; businessRequestId:string }>;
...
export interface ProductVersionTransaction {
  ...
  adoptLegacyProductFormula?(input:Readonly<{organizationId:string;productId:string;draftVersionId:string;expectedDraftUpdatedAt:string;staffActorUserId?:string}>):Promise<ProductDraftFormulaPricing>;
  auditDraftFormulaPricing?(input:Readonly<{organizationId:string;requestId:string;operation:string;resourceId:string;principalKind:"staff"|"delegated_ai"|"portal"|"service";principalSubject:string;staffActorUserId?:string;changedFields:readonly string[]}>):Promise<void>;
...
const adoptLegacyFormulaOperation = "product.draft.pricingFormula.adoptLegacy.v1";
...
const legacyFormulaFingerprint = (input: AdoptLegacyProductFormulaInput) => createHash("sha256").update(JSON.stringify(input)).digest("hex");
...
export class ProductVersionLifecycleApplicationService {
  ...
  async updateDraftFormulaPricing(context:OperationContext,input:UpdateProductDraftFormulaPricingInput):Promise<ApplicationResult<ProductDraftFormulaPricing>>{try{requireOperationPrincipalScope(context);if(!context.businessRequest||context.businessRequest.id!==input.businessRequestId)throw new V2ApplicationError("VALIDATION_ERROR","A matching business request identity is required.");if(!this.authority.decide(context.principal,{capability:"product.edit",resource:{organizationId:context.organizationId}}).allowed)throw new V2ApplicationError("FORBIDDEN","The principal does not have authority to edit this Product Draft.");if(typeof input.allowRotation!=="boolean"||(input.rotationControl!==undefined&&(!input.rotationControl.optionId||!Array.isArray(input.rotationControl.allowWhenChoiceValues)||input.rotationControl.allowWhenChoiceValues.length===0||input.rotationControl.allowWhenChoiceValues.some(value=>typeof value!=="string"||!value))))throw new V2ApplicationError("VALIDATION_ERROR","Rotation settings are invalid.");const value=await this.runner.transaction(async tx=>{if(!tx.updateDraftFormulaPricing||!tx.auditDraftFormulaPricing)throw new V2ApplicationError("CONFLICT","Product Draft Formula editing is unavailable.");const request=await tx.reserve({organizationId:context.organizationId,operation:updateFormulaPricingOperation,businessRequestId:input.businessRequestId,payloadFingerprint:formulaFingerprint(input),...actor(context)});if(request.kind==="replay")return request.request.resultJson as ProductDraftFormulaPricing;const updated=await tx.updateDraftFormulaPricing({organizationId:context.organizationId,productId:input.productId,draftVersionId:input.draftVersionId,expectedDraftUpdatedAt:input.expectedDraftUpdatedAt,formula:input,staffActorUserId:staffActorId(context.principal)});await tx.attribute({organizationId:context.organizationId,requestId:request.request.id,operation:updateFormulaPricingOperation,resourceId:updated.draftVersionId,...actor(context)});await tx.auditDraftFormulaPricing({organizationId:context.organizationId,requestId:request.request.id,operation:updateFormulaPricingOperation,resourceId:updated.draftVersionId,changedFields:updated.expressionEditable?["expression","variables","allowRotation","rotationControl"]:["variables","allowRotation","rotationControl"],...actor(context)});await tx.succeed(context.organizationId,request.request.id,updated.draftVersionId,updated);return updated;});return success(value);}catch(error){return failure(error instanceof V2ApplicationError?error:new V2ApplicationError("CONFLICT",error instanceof Error?error.message:"Product Draft Formula could not be saved."));}}
  async adoptLegacyProductFormula(context:OperationContext,input:AdoptLegacyProductFormulaInput):Promise<ApplicationResult<ProductDraftFormulaPricing>>{try{requireOperationPrincipalScope(context);if(!context.businessRequest||context.businessRequest.id!==input.businessRequestId)throw new V2ApplicationError("VALIDATION_ERROR","A matching business request identity is required.");if(!this.authority.decide(context.principal,{capability:"product.edit",resource:{organizationId:context.organizationId}}).allowed)throw new V2ApplicationError("FORBIDDEN","The principal does not have authority to edit this Product Draft.");const value=await this.runner.transaction(async tx=>{if(!tx.adoptLegacyProductFormula||!tx.auditDraftFormulaPricing)throw new V2ApplicationError("CONFLICT","Legacy Formula adoption is unavailable.");const request=await tx.reserve({organizationId:context.organizationId,operation:adoptLegacyFormulaOperation,businessRequestId:input.businessRequestId,payloadFingerprint:legacyFormulaFingerprint(input),...actor(context)});if(request.kind==="replay")return request.request.resultJson as ProductDraftFormulaPricing;const updated=await tx.adoptLegacyProductFormula({organizationId:context.organizationId,productId:input.productId,draftVersionId:input.draftVersionId,expectedDraftUpdatedAt:input.expectedDraftUpdatedAt,staffActorUserId:staffActorId(context.principal)});await tx.attribute({organizationId:context.organizationId,requestId:request.request.id,operation:adoptLegacyFormulaOperation,resourceId:updated.draftVersionId,...actor(context)});await tx.auditDraftFormulaPricing({organizationId:context.organizationId,requestId:request.request.id,operation:adoptLegacyFormulaOperation,resourceId:updated.draftVersionId,changedFields:["pricingFormula","formulaVariables","legacyProductFormula.adopted"],...actor(context)});await tx.succeed(context.organizationId,request.request.id,updated.draftVersionId,updated);return updated;});return success(value);}catch(error){return failure(error instanceof V2ApplicationError?error:new V2ApplicationError("CONFLICT",error instanceof Error?error.message:"Legacy Product Formula could not be adopted."));}}
