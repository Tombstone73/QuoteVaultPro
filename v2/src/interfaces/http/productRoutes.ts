...
import type { ProductVersionLifecycle } from "../../modules/products/productVersionLifecycle.js";
...
export type ProductHttpDependencies = Readonly<{ workspace: ProductWorkspaceReadPort; lifecycle: ProductVersionLifecycleApplicationService; principals: Readonly<{ principal(request: Request, organizationId: string): Promise<Principal> }> }>;
...
export const createProductRouter = (dependencies: ProductHttpDependencies) => {
  ...
  router.post("/:productId/drafts", async (request, response) => { try { const organizationId = (request.params as Record<string,string>).organizationId; const principal = await dependencies.principals.principal(request, organizationId); if (!new AuthorityPolicy().decide(principal,{capability:"product.edit",resource:{organizationId}}).allowed) return deny(response,403,"FORBIDDEN","Product Draft access is unavailable."); if (!validProductId(request.params.productId)) return deny(response, 404, "NOT_FOUND", "Product is unavailable in this organization."); const body = request.body as Record<string,unknown>; const businessRequestId = typeof body?.businessRequestId === "string" ? body.businessRequestId.trim() : ""; const expectedActiveVersionUpdatedAt = typeof body?.expectedActiveVersionUpdatedAt === "string" ? body.expectedActiveVersionUpdatedAt : ""; if (!businessRequestId || !expectedActiveVersionUpdatedAt) return response.status(400).json({ ok:false,error:{code:"VALIDATION_ERROR",message:"A business request and current Active version state are required."} }); const result = await dependencies.lifecycle.createDraft({ principal, organizationId, operationId: businessRequestId, businessRequest:{id:businessRequestId,payloadFingerprint:businessRequestId} }, { productId: request.params.productId, businessRequestId, expectedActiveVersionUpdatedAt }); return result.ok ? response.status(200).json({ok:true,data:result.value}) : response.status(result.error.code === "FORBIDDEN" ? 403 : result.error.code === "NOT_FOUND" ? 404 : result.error.code === "VALIDATION_ERROR" ? 400 : 409).json({ok:false,error:{code:result.error.code,message:result.error.publicMessage}}); } catch { return deny(response, 403, "FORBIDDEN", "Authenticated access is required."); } });
