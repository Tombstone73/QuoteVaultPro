...
import { pushArtworkLocation, pushContactLocation, pushCustomerLocation, pushFulfillmentLocation, pushInvoiceLocation, pushOrderLocation, pushProductLocation, pushProductionLocation, pushQuoteLocation, pushWorkspaceLocation, readWorkspaceLocation } from "./productRouting";
...
const OrdersPage = ({ organizationId, sessionScope, orderId, setOrderId, bootstrap, openCustomer, openFulfillment, openInvoice, openArtwork, openQuote }: Readonly<{ organizationId: string; setOrganizationId: (value: string) => void; sessionScope: string; orderId: string; setOrderId: (value: string) => void; bootstrap?: import("./api").UiBootstrap; openCustomer: (customerId: string) => void; openFulfillment: (orderId: string) => void; openInvoice: (invoiceId: string) => void; openArtwork: (orderId: string, lineId: string) => void; openQuote: (quoteId: string) => void }>) => {
  const [legacyOrderId, setLegacyOrderId] = useState("");
  if (orderId) return <OrderWorkspace organizationId={organizationId} sessionScope={sessionScope} orderId={orderId} canEdit={bootstrap?.capabilities.orderEdit === true} canOverridePrice={bootstrap?.capabilities.orderOverridePrice === true} canViewInvoice={bootstrap?.capabilities.invoiceView === true} canViewArtwork={bootstrap?.capabilities.artworkView === true} csrfReady={Boolean(bootstrap)} onBack={() => setOrderId("")} openCustomer={openCustomer} openFulfillment={openFulfillment} openInvoice={openInvoice} openArtwork={openArtwork} openQuote={openQuote} />;
