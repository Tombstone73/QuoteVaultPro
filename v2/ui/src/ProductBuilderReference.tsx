...
export const ProductBuilderReference = ({
  ...
  >{{
    ...
    options: <div className="space-y-3"><OptionGroupsSection options={draft.options} disabled={!canEdit || saving} onChange={(options) => patch("options", (value) => ({ ...value, options }))} /><RuleCards conditions={projectCanonicalConditions({ options: draft.options, recipe: draft.recipe, production: draft.general.productionUnitSpecification })} onJumpToOwner={(owner) => sectionJumpRef.current?.(owner)} /></div>,
    ...
    production: <ProductionUnits specification={draft.general.productionUnitSpecification} options={draft.options} disabled={!canEdit || saving} onChange={(productionUnitSpecification) => patch("general", (value) => ({ ...value, general: { ...value.general, productionUnitSpecification } }))} />,
