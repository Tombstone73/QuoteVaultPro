...
const InvoiceDetail = ({ invoice, organizationId, canIssue, csrfReady, busy, onIssue }: Readonly<{ invoice: InvoiceRead; organizationId: string; canIssue: boolean; csrfReady: boolean; busy: boolean; onIssue: () => void }>) => <>
  <header className="v2-invoice-header"><div><span>Billing record · Source Order {invoice.sourceOrderNumber ?? "unavailable"}</span><h1>Invoice</h1><p>{customerName(invoice)}{invoice.customerPresentation?.contactDisplayName ? ` · ${invoice.customerPresentation.contactDisplayName}` : ""}</p></div><div><span className={`v2-invoice-state ${invoice.lifecycle}`}>{invoice.lifecycle}</span><button type="button" onClick={() => window.open(`/v2/organizations/${encodeURIComponent(organizationId)}/invoices/${encodeURIComponent(invoice.invoiceId)}/document.pdf`, "_blank", "noopener,noreferrer")}>Preview PDF</button>{invoice.lifecycle === "draft" && canIssue && <button className="v2-invoice-issue" disabled={!csrfReady || busy} onClick={onIssue}>Issue Invoice</button>}</div></header>
