...
export type ProductDraftFormulaPricing=Readonly<{productId:string;draftVersionId:string;draftUpdatedAt:string;lifecycle:"draft";source:"embedded_editable"|"library_product_inputs_editable"|"library_reference_read_only"|"unsupported_legacy";editable:boolean;expressionEditable:boolean;variablesEditable:boolean;rotationEditable:boolean;inputs:readonly Readonly<{key:string;label:string;unit?:"in"|"sq_ft";minimum?:number;exclusiveMinimum?:boolean}>[];unavailableReason?:string;formulaId?:string;formulaName?:string;expression:string;variables:Record<string,number>;allowRotation:boolean;supportedRuntimeVariables:readonly string[];warnings:readonly string[]}>;
...
export const productApi = {
  ...
  saveDraftFormula:(organizationId:string,productId:string,businessRequestId:string,input:Readonly<{draftVersionId:string;expectedDraftUpdatedAt:string;expression:string;variables:Record<string,number>;allowRotation:boolean}>)=>request<ProductDraftFormulaPricing>(`/v2/organizations/${encodeURIComponent(organizationId)}/products/${encodeURIComponent(productId)}/draft/pricing/formula`,{method:"PATCH",headers:{"x-v2-csrf-token":csrfTokens.get(csrfKey(organizationId))??""},body:JSON.stringify({businessRequestId,...input})}),
