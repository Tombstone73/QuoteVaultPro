...
export type StripeRuntimeReadiness = Readonly<{ mode:"test"|"live"|"unknown";status:"ready"|"not_configured"|"webhook_not_ready"|"action_required";secretKeyConfigured:boolean;publishableKeyMode:"test"|"live"|"unknown"|"missing";publishableKey?:string;webhook:"ready"|"missing"|"invalid";configurationOwner:"tenant_connected_account";actionRequired:string;connection:Readonly<{mode:"test"|"live"|"unknown";status:"not_connected"|"onboarding"|"requirements_due"|"ready"|"reconnect_required"|"disconnected"|"error"|"platform_not_ready";connected:boolean;connectedAccountName:string|null;cardPayments:"active"|"pending"|"inactive"|"unknown";payouts:"active"|"pending"|"inactive"|"unknown";requirementsDue:number;actionRequired:string|null;connectionModel:"accounts_v2_direct"}> }>;
...
export const teamAccessApi = {
  ...
  resendPortalSetup: (organizationId: string, portalAccessId: string, input: Readonly<{ businessRequestId: string; expectedAuthorityRevision: string }>) => request<Readonly<{ portalAccessId: string; status: "pending" }>>(`/v2/organizations/${encodeURIComponent(organizationId)}/settings/team-access/portal-access/${encodeURIComponent(portalAccessId)}/resend-setup`, { method: "POST", headers: { "x-v2-csrf-token": csrfTokens.get(csrfKey(organizationId)) ?? "" }, body: JSON.stringify(input) }),
  setPortalAccessStatus: (organizationId: string, portalAccessId: string, input: Readonly<{ businessRequestId: string; expectedAuthorityRevision: string; status: "suspended" | "disabled" }>) => request<Readonly<{ portalAccessId: string; status: "suspended" | "disabled" }>>(`/v2/organizations/${encodeURIComponent(organizationId)}/settings/team-access/portal-access/${encodeURIComponent(portalAccessId)}/status`, { method: "PATCH", headers: { "x-v2-csrf-token": csrfTokens.get(csrfKey(organizationId)) ?? "" }, body: JSON.stringify(input) }),
};
...
export const stripeSettingsApi = {
  get: (organizationId: string) => request<StripeRuntimeReadiness>(`/v2/organizations/${encodeURIComponent(organizationId)}/settings/payments/stripe`),
  connect: (organizationId:string) => request<Readonly<{onboardingUrl:string}>>(`/v2/organizations/${encodeURIComponent(organizationId)}/settings/payments/stripe/connect`,{method:"POST",headers:{"x-v2-csrf-token":csrfTokens.get(csrfKey(organizationId)) ?? ""},body:"{}"}),
  disconnect: (organizationId:string) => request<unknown>(`/v2/organizations/${encodeURIComponent(organizationId)}/settings/payments/stripe/disconnect`,{method:"POST",headers:{"x-v2-csrf-token":csrfTokens.get(csrfKey(organizationId)) ?? ""},body:"{}"}),
};
...
export type QuickBooksConnectionReadiness = Readonly<{ state: "not_connected" | "connected_sandbox" | "connected_production" | "connected_unknown" | "authorization_required" | "reconnect_required" | "worker_not_ready" | "sync_ready" | "action_required"; environment: "sandbox" | "production" | "unknown"; connected: boolean; companyAssociated: boolean; connectedCompanyName: string | null; actionRequired: string | null; refunds:Readonly<{state:"ready"|"configuration_required";account:QuickBooksRefundDisbursementAccount|null}> }>;
...
export type QuickBooksOperationsRead = Readonly<{eligibleInvoiceCount:number;awaitingApprovalInvoiceCount:number;queueSummary:Readonly<{queued:number;processing:number;succeeded:number;actionRequired:number}>}>;
...
export const quickBooksIntegrationApi = {
  ...
  approveInvoice:(organizationId:string,invoiceId:string)=>request<Readonly<{invoiceId:string;synchronizationVersion:string;alreadyApproved:boolean}>>(`/v2/organizations/${encodeURIComponent(organizationId)}/settings/accounting/invoices/${encodeURIComponent(invoiceId)}/approve`,{method:"POST",headers:{"x-v2-csrf-token":csrfTokens.get(csrfKey(organizationId)) ?? ""},body:"{}"}),
...
export const financeApi = {
  ...
  beginStripePayment: (organizationId: string, invoiceId: string, businessRequestId: string, input: Readonly<{ amountCents:number; currency:string }>) => request<Readonly<{providerOperationId:string;paymentIntentId:string;clientSecret:string;publishableKey:string;stripeAccountId:string;amountCents:number;currency:string}>>(financeEndpoint(organizationId, `/invoices/${encodeURIComponent(invoiceId)}/stripe/payment-intents`), { method:"POST", headers:{"x-v2-csrf-token":csrfTokens.get(csrfKey(organizationId)) ?? ""}, body:JSON.stringify({...input,businessRequestId}) }),
