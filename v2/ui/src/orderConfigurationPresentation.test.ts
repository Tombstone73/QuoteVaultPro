...
const internal =
  /(?:\bopt_|\bchoice_|_import|[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})/iu;
...
assert.equal(
  modern,
  "24 × 18 in · Print sides: Double-sided · Grommets: Every 2 feet",
);
...
assert.match(multiple, /Configuration detail unavailable/);
assert.doesNotMatch(multiple, internal);

assert.equal(
  orderConfigurationPresentation({ selections: {} }),
  "No additional configuration",
);
await import("./orderSavedWorkspacePresentation.test");
await import("./orderProductionPanel.test");
await import("./orderRoutingPresentation.test");
