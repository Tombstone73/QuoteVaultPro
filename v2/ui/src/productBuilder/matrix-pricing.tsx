...
export function MatrixPricing({ matrix, disabled, onChange }: Readonly<{
  ...
  if (!matrix) return <p className="text-[12px] italic text-muted-foreground">Matrix pricing is not configured for this Draft.</p>;
  ...
  return <div className="space-y-3">
    ...
    <div className="grid gap-3 lg:grid-cols-2">
      <div className="rounded-md border border-border p-2.5">
        <div className="flex items-center gap-2"><Grid3x3 className="size-3.5 text-primary" /><span className="text-[12px] font-bold uppercase tracking-wide">Matrix dimensions</span><Chip tone="accent">Option groups</Chip></div>
        ...
        <div className="mt-2 space-y-1.5">
          {matrix.dimensions.map((dimension, index) => <div key={dimension.selectionKey} className="flex items-center gap-2 rounded border border-border bg-surface-2 px-2 py-1.5">
            <span className="num w-14 shrink-0 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{index === 0 ? "Rows" : index === 1 ? "Columns" : `Slice ${index - 1}`}</span>
            <span className="min-w-0 flex-1 truncate text-[13px] font-medium">{dimension.label}</span>
            <span className="num shrink-0 text-[11px] text-muted-foreground">{dimension.values.length} choices</span>
          </div>)}
          {matrix.dimensions.length === 0 && <p className="text-[12px] italic text-muted-foreground">No dimensions yet — add at least one option group.</p>}
        </div>
      </div>

      <div className="rounded-md border border-border p-2.5">
        <div className="flex items-center gap-2"><span className="text-[12px] font-bold uppercase tracking-wide">Tier basis</span><Chip>Not an option group</Chip></div>
        ...
        {matrix.warnings.map((warning) => <p key={warning} className="mt-2 text-[11px] text-warn">{warning}</p>)}
      </div>
    </div>
    ...
    {rowDim && <div className="rounded-md border border-border">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2 border-b border-border px-2.5 py-2 sm:flex sm:flex-wrap sm:justify-between">
        <div className="flex min-w-0 flex-wrap items-center gap-2"><span className="text-[12px] font-bold uppercase tracking-wide">Rates</span><Chip tone="accent">{matrix.pricingUnit === "per_piece" ? "Per piece" : "Per square foot"}</Chip>{missing > 0 && <Chip tone="warn">{missing} empty</Chip>}</div>
        <div className="flex shrink-0 items-center gap-2">
          {extraDims.map((dimension) => <label key={dimension.selectionKey} className="flex items-center gap-1.5"><span className="text-[11px] text-muted-foreground">{dimension.label}</span><select className="h-8 w-[150px] text-[13px]" value={String(sliceValues[dimension.selectionKey])} onChange={(event) => setSlice((current) => ({ ...current, [dimension.selectionKey]: event.target.value }))}>{dimension.values.map((value) => <option key={String(value.value)} value={String(value.value)}>{value.label}</option>)}</select></label>)}
          {matrix.rows.some((row) => row.tiers.length > 1) && <button type="button" className="button secondary h-7 gap-1 text-[12px]" disabled><Copy className="size-3.5" />Copy previous tier</button>}
        </div>
      </div>
      <div className="max-w-full overflow-x-auto"><table className="w-full min-w-[420px] border-collapse text-[13px]"><thead><tr><th className="sticky left-0 z-20 border-b border-r border-border bg-surface-2 px-2.5 py-1.5 text-left text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{rowDim.label}</th>{(colDim ? colDim.values : [{ value: "single", label: "Rate" }]).map((choice) => <th key={String(choice.value)} className="border-b border-border bg-surface-2 px-2.5 py-1.5 text-left text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">{choice.label}</th>)}</tr></thead><tbody>{rowDim.values.map((rowChoice) => <tr key={String(rowChoice.value)}><th className="sticky left-0 z-10 border-b border-r border-border bg-background px-2.5 py-1 text-left text-[12px] font-medium">{rowChoice.label}</th>{(colDim ? colDim.values : [{ value: undefined, label: "" }]).map((columnChoice) => { const row = activeRows(rowChoice.value, columnChoice.value); return <td key={String(columnChoice.value)} className="border-b border-border px-1.5 py-1"><MoneyRate disabled={!editable || !row || row.tierBasis === "computed_sheet_usage"} value={row?.baseRateCents ?? null} onChange={(value) => row && updateRate(row.rowId, value ?? 0)} /></td>; })}</tr>)}</tbody></table></div>
      <p className="border-t border-border px-2.5 py-1.5 text-[11px] text-muted-foreground">Each cell is the rate charged {matrix.pricingUnit === "per_piece" ? "per piece" : "per square foot"} for that combination. N-dimensional rows remain canonical in V2.</p>
    </div>}
    {!matrix.editable && <p className="text-[11px] text-muted-foreground">{matrix.unavailableReason ?? "Matrix pricing is read-only for this Draft."}</p>}
  </div>;
}

function MoneyRate({ value, onChange, disabled }: Readonly<{ value: number | null; onChange: (value: number | null) => void; disabled?: boolean }>) {
  return <div className="relative"><span className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-[12px] text-muted-foreground">$</span><input aria-label="Matrix rate" className="num h-7 w-[92px] pl-5 text-[13px]" placeholder="—" disabled={disabled} inputMode="decimal" value={value == null ? "" : (value / 100).toFixed(2)} onChange={(event) => onChange(event.target.value === "" ? null : Math.round(Number(event.target.value) * 100))} /></div>;
}
