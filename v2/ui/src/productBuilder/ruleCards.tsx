  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="max-w-2xl text-[0.75rem] text-muted-foreground">
      ...
      {conditions.length === 0 && (
        <p className="rounded-md border border-dashed border-border p-6 text-center text-[0.8125rem] text-muted-foreground">
  ...
  return (
    <div className="rounded-md border border-border p-3">
      <div className="flex flex-wrap items-center gap-2">
        <input
          className="h-7 max-w-[280px] border-transparent bg-transparent text-[0.8125rem] font-semibold"
      ...
      <div className="mt-2.5 flex flex-wrap items-end gap-2 rounded-md border border-border bg-surface-2/60 p-2.5">
        <span className="pb-2 text-[0.75rem] font-semibold uppercase tracking-wide text-muted-foreground">When</span>
        <span className="min-w-[200px] flex-[2] rounded border border-border px-2 py-1.5 text-[0.8125rem]">{rule.sourceLabel}</span>
        <span className="w-[92px] rounded border border-border px-2 py-1.5 text-[0.8125rem]">is</span>
        <span className="min-w-[130px] flex-1 rounded border border-border px-2 py-1.5 text-[0.8125rem]">{rule.choiceValue}</span>
        <ArrowRight className="mb-2 size-4 shrink-0 text-muted-foreground" />
        <span className="min-w-[190px] flex-1 rounded border border-border px-2 py-1.5 text-[0.8125rem]">{effect}</span>
      </div>
      <p className="mt-2.5 border-t border-border pt-2 text-[0.75rem] leading-relaxed text-muted-foreground">
      ...
      {onJumpToOwner && <button type="button" className="mt-2 text-[0.75rem] text-primary hover:underline" onClick={() => onJumpToOwner(rule.owner)}>Open owning section</button>}
    </div>
  );
}
