...
export function RecipeEditor({ components, materials, options, disabled, onChange }: Readonly<{
  ...
  return <div className="space-y-2.5">
    <p className="text-[12px] text-muted-foreground">Recipe lines describe consumption, not price. A line with a condition is only consumed when that option choice is selected.</p>
    {components.length === 0 && <p className="rounded-md border border-dashed border-border px-3 py-4 text-center text-[12px] italic text-muted-foreground">No recipe — this Product consumes no inventory.</p>}
      ...
      {components.map((line, index) => {
        ...
        return <div key={line.componentId ?? `${line.materialId}-${index}`} className="rounded-md border border-border p-2.5">
          ...
          <div className="mt-2 grid gap-2 @container sm:grid-cols-2 lg:grid-cols-4">
            ...
            <Cell label="Quantity / factor"><input className="num h-8 text-[13px]" disabled={disabled} inputMode="decimal" value={line.quantity} onChange={(event) => update(index, (current) => ({ ...current, quantity: event.target.value }))} /></Cell>
