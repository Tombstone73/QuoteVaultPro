  ...
  return <div className="space-y-3">
    <Card icon={<Calculator className="size-3.5 text-primary" />} title="Configuration preview" note="Server-calculated">
      {measurementMode === "dimensions_required" ? <div className="grid grid-cols-3 gap-2">
        <Cell label="Width"><input className="num h-8 text-[13px]" inputMode="decimal" value={inputs.width} onChange={(event) => onInputsChange({ ...inputs, width: event.target.value })} /></Cell>
        <Cell label="Height"><input className="num h-8 text-[13px]" inputMode="decimal" value={inputs.height} onChange={(event) => onInputsChange({ ...inputs, height: event.target.value })} /></Cell>
        <Cell label="Qty"><input className="num h-8 text-[13px]" type="number" min="1" value={inputs.quantity} onChange={(event) => onInputsChange({ ...inputs, quantity: event.target.value })} /></Cell>
      </div> : <div className="grid grid-cols-3 gap-2"><Cell label="Qty"><input className="num h-8 text-[13px]" type="number" min="1" value={inputs.quantity} onChange={(event) => onInputsChange({ ...inputs, quantity: event.target.value })} /></Cell></div>}
      ...
      <div className="mt-2.5 space-y-2">
        {visible.map((option) => <Cell key={option.optionId} label={option.label}>
          ...
          </select> : <input className="h-8 text-[13px]" placeholder="Free text at quote time" value={String(inputs.selections[option.optionId] ?? "")} onChange={(event) => onInputsChange({ ...inputs, selections: { ...inputs.selections, [option.optionId]: event.target.value } })} />}
        </Cell>)}
      </div>
...
function PreviewResult({ result }: Readonly<{ result: ProductDraftPricingPreview }>) {
  return <><dl className="mt-2.5 space-y-1 border-t border-border pt-2 text-[12px]">
