  ...
  return <div className="space-y-3">
    <Card icon={<Calculator className="size-3.5 text-primary" />} title="Configuration preview" note="Server-authoritative">
      ...
      <section className="mt-3 border-t border-border pt-2.5" aria-label="Pricing result">
        ...
        {result ? <PreviewResult result={result} options={options} stale={stale} /> : missingConfiguration.length ? <IncompleteConfiguration missing={missingConfiguration} /> : <p className="mt-2 text-[0.75rem] text-muted-foreground">Waiting for the canonical server price…</p>}
      ...
      <section className="mt-3 border-t border-border pt-2.5" aria-label="Configuration validation">
        <p className="mb-1.5 num text-[0.625rem] font-semibold uppercase tracking-wide text-muted-foreground">Configuration validation</p>
        {missingConfiguration.length ? <IncompleteConfiguration missing={missingConfiguration} compact /> : error ? <p className="text-[0.75rem] text-late">The current configuration could not be priced.</p> : <p className="flex items-center gap-1.5 text-[0.75rem] text-ok"><CheckCircle2 className="size-3.5" />Configuration is ready for canonical pricing.</p>}
      </section>
    </Card>
...
function IncompleteConfiguration({ missing, compact }: Readonly<{ missing: readonly string[]; compact?: boolean }>) { return <div className={compact ? "text-[0.75rem] text-warn" : "mt-2 rounded-md border border-warn/40 bg-warn/10 px-2.5 py-2 text-[0.75rem] text-warn"}><p className="font-medium">Select required options before pricing.</p><p className="mt-1 text-[0.6875rem]">{missing.join(" · ")}</p></div>; }
