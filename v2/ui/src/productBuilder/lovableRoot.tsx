...
export function LovableProductBuilderRoot({ title, lifecycle, subtitle, onSave, onPublish, saving, publishing, findings, children, rail, canEdit = false, persisted = false, saveError, publishDisabled = false, publishBlockedReason, sectionJumpRef }: Readonly<{
  ...
  /** The adapter has an unsaved/stale/revision condition. Server publication
   * remains the final readiness authority. */
  publishDisabled?: boolean; publishBlockedReason?: string;
  ...
  return <div className="p-4 pb-24 product-builder-reference-port">
    ...
    <div className="mt-3 grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="min-w-0 space-y-3">{SECTIONS.map(({ id, label }) => <Section key={id} id={id} title={id === "review" ? "Review & publish" : id === "materials" ? "Materials & recipe" : label} hint={hints[id]} open={!collapsed[id]} onToggle={() => setCollapsed((state) => ({ ...state, [id]: !state[id] }))} register={(node) => { refs.current[id] = node; }}>{children[id]}{id === "review" && <div className="mt-3 flex flex-wrap items-center gap-2">{canEdit && persisted && onPublish && <ReferenceButton size="sm" disabled={publishing || publishDisabled || findings.errors > 0} title={publishBlockedReason} aria-describedby={publishBlockedReason ? "product-builder-publish-state" : undefined} onClick={onPublish}>{publishing ? "Publishing…" : "Publish"}</ReferenceButton>}{canEdit && <ReferenceButton variant="outline" size="sm" disabled={saving} onClick={onSave}>{saving ? "Saving…" : "Save Changes"}</ReferenceButton>}{(findings.errors > 0 || publishBlockedReason) && <span className="text-[0.75rem] text-late">{publishBlockedReason ?? `Fix ${findings.errors} blocking issue${findings.errors === 1 ? "" : "s"} first.`}</span>}</div>}</Section>)}</div><div className="min-w-0 xl:sticky xl:top-14 xl:h-[calc(100vh-5rem)] xl:overflow-y-auto xl:pb-6">{rail}</div></div>
  </div>;
}
