...
export function LovableProductBuilderRoot({ title, lifecycle, subtitle, picker, onSave, onPublish, saving, publishing, findings, children, rail, pendingSwitch, canEdit = false, persisted = false, saveError, sectionJumpRef }: Readonly<{
  ...
  return <div className="p-4 pb-24 product-builder-reference-port">
    <PageHeader
      ...
      subtitle={subtitle}
      actions={<><div className="w-[190px] [&>select]:w-full">{picker}</div>{canEdit && <ReferenceButton variant="outline" size="sm" disabled={saving} onClick={onSave}>{saving ? "Saving…" : "Save Changes"}</ReferenceButton>}<ReferenceButton variant="outline" size="sm" onClick={() => jumpTo("review")}>Review</ReferenceButton>{canEdit && persisted && onPublish && <ReferenceButton size="sm" disabled={publishing || findings.errors > 0} onClick={onPublish}>{publishing ? "Publishing…" : "Publish"}</ReferenceButton>}</>}
    />
    ...
    {pendingSwitch && <div className="v2-ref-dialog-backdrop" role="dialog" aria-modal="true" aria-label="Unsaved changes"><div className="v2-ref-dialog"><h2>Discard unsaved changes?</h2><p>{pendingSwitch.productName || "This Product"} has unsaved Draft changes. Switching Products now discards them.</p><div><ReferenceButton variant="outline" size="sm" onClick={pendingSwitch.onKeepEditing}>Keep editing</ReferenceButton><ReferenceButton variant="outline" size="sm" onClick={pendingSwitch.onSaveAndSwitch}>Save and switch</ReferenceButton><ReferenceButton size="sm" onClick={pendingSwitch.onDiscardAndSwitch}>Discard and switch</ReferenceButton></div></div></div>}
  </div>;
}
