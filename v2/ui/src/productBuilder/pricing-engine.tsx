  ...
  return <div className="space-y-3">
    ...
    {advanced && <>
      ...
      <div className="rounded-md border border-border p-3">
        <div className="flex flex-wrap items-center justify-between gap-2"><Segmented value={tierTab} onChange={setTierTab} items={[{ id: "qty", label: `Quantity tiers (${tiers.length})` }, { id: "size", label: "Size tiers" }]} /><button type="button" disabled={!editable} className="button secondary h-7 gap-1 text-[0.75rem]" onClick={addTier}><Plus className="size-3.5" />Add {tierTab === "qty" ? "qty" : "size"} tier</button></div>
