...
export function ProductionUnits({ specification, options, selectionKeys = {}, disabled, onChange }: Readonly<{
  ...
  return <div className="space-y-2.5">
    ...
    <button type="button" className="button secondary h-7 gap-1 text-[0.75rem]" disabled={disabled} onClick={() => onChange({ schemaVersion: 1, rules: [...rules, { key: `unit_${rules.length + 1}` }] })}><Plus className="size-3.5" />Add production unit</button>
  </div>;
}
...
export function RoutingSection({ routing, templates, disabled, onChange, onManageRoutes }: Readonly<{
  ...
  return <div className="space-y-3">
    ...
    {required ? <>
      <div className="rounded-md border border-border p-2.5"><div className="flex flex-wrap items-center gap-2"><span className="text-[0.6875rem] font-semibold uppercase tracking-wide text-muted-foreground">Route preview</span><Chip tone="ok">Read-only</Chip><span className="ml-auto text-[0.6875rem] text-muted-foreground">Revision {routing.sourceTemplateRevision ?? "—"}</span></div><div className="mt-1.5 text-[0.8125rem] font-semibold">{routing.routeTemplateName}</div><ol className="mt-2 flex flex-wrap items-center gap-1.5">{routing.steps.map((step, index) => <li key={step.position} className="flex items-center gap-1.5">{index > 0 && <ArrowRight className="size-3.5 text-muted-foreground" aria-hidden />}{/* Route steps are deliberately preview-only in Product Builder. */}<span className="rounded border border-border bg-surface-2 px-2 py-1 text-[0.75rem] font-medium">{step.kind}</span></li>)}{routing.steps.length === 0 && <li className="text-[0.75rem] italic text-muted-foreground">This template has no steps.</li>}</ol><div className="mt-2.5 flex flex-wrap items-center gap-2"><button type="button" className="button secondary h-7 gap-1.5 text-[0.75rem]" onClick={() => setViewOpen(true)}><Eye className="size-3.5" />View route</button>{onManageRoutes && <button type="button" className="button secondary h-7 gap-1.5 text-[0.75rem]" onClick={onManageRoutes}><ExternalLink className="size-3.5" />Manage routes</button>}<span className="text-[0.6875rem] text-muted-foreground">Editing steps happens in the Routing module.</span></div></div>
