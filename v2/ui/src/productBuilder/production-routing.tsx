...
export function ProductionUnits({ specification, options, selectionKeys = {}, disabled, onChange }: Readonly<{
  ...
  return <div className="space-y-2.5">
      ...
      {rules.map((unit, index) => {
        ...
        return <div key={`${unit.key}-${index}`} className="rounded-md border border-border p-2.5">
          <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-2">
            ...
            <ReferenceButton variant="ghost" size="icon" className="size-7 shrink-0 text-muted-foreground hover:text-late" aria-label="Remove production unit" disabled={disabled} onClick={() => remove(index)}><Trash2 className="size-3.5" /></ReferenceButton>
          </div>
    ...
    <ReferenceButton variant="outline" size="sm" className="h-7 gap-1" disabled={disabled} onClick={() => onChange({ schemaVersion: 1, rules: [...rules, { key: `unit_${rules.length + 1}` }] })}><Plus className="size-3.5" />Add production unit</ReferenceButton>
  </div>;
}
