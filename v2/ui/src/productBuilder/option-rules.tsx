...
export const initialRuleValue = (source: RuleOption, multi = false): unknown => {
  if (source.choices.length) {
    const choice = source.choices[0]?.choiceValue ?? "";
    return multi || source.inputType === "multiselect" ? (choice ? [choice] : []) : choice;
  }
  if (source.inputType === "boolean") return false;
  if (source.inputType === "number") return 0;
  return "";
};
...
export const createProductDraftOptionRule = (
  ...
  return {
    ...
    enabled: true,
    when: { all: first ? [{ optionGroup: first.selectionKey, operator: "equals", value: initialRuleValue(first) }] : [] },
...
function RuleEditor({ rule, index, references, disabled, onChange, onRemove, onMoveUp, onMoveDown }: Readonly<{
  ...
  const addCondition = () => {
    ...
    setConditions([...conditions, { optionGroup: target.selectionKey, operator: "equals", value: initialRuleValue(target) }]);
  };
...
function ConditionRow({ condition, references, disabled, onChange, onRemove }: Readonly<{
  ...
  const updateSource = (optionGroup: string) => {
    ...
    onChange({ optionGroup, operator, ...(needsValue(operator) ? { value: initialRuleValue(next, operator === "in" || operator === "not_in") } : {}) });
  };
  return <div className="grid gap-2 rounded border border-border bg-surface-2/70 p-2 sm:grid-cols-[minmax(0,1fr)_140px_minmax(0,1fr)_auto]">
    ...
    <select aria-label="Rule operator" disabled={disabled} value={operator} onChange={(event) => { const nextOperator = event.target.value as ProductOptionRuleConditionOperator; onChange({ ...condition, operator: nextOperator, ...(needsValue(nextOperator) ? { value: initialRuleValue(source, nextOperator === "in" || nextOperator === "not_in") } : { value: undefined }) }); }}>{operators.map((entry) => <option key={entry} value={entry}>{entry.replace(/_/gu, " ")}</option>)}</select>
    {needsValue(operator) ? <RuleValue value={condition.value} source={source} multi={operator === "in" || operator === "not_in"} disabled={disabled} onChange={(value) => onChange({ ...condition, value })} /> : <span className="self-center text-[0.75rem] text-muted-foreground">No value required</span>}
...
function ActionRow({ action, references, disabled, onChange, onRemove }: Readonly<{
  ...
  return <div className="grid gap-2 rounded border border-border bg-surface-2/70 p-2 sm:grid-cols-[150px_minmax(0,1fr)_minmax(0,1fr)_auto]">
    <select aria-label="Rule action" disabled={disabled} value={action.action} onChange={(event) => { const nextAction = event.target.value as ProductOptionRuleActionType; onChange({ ...action, action: nextAction, ...(nextAction === "set_default" ? { value: initialRuleValue(target) } : { value: undefined }) }); }}>{actions.map((entry) => <option key={entry} value={entry}>{entry.replace(/_/gu, " ")}</option>)}</select>
    <select aria-label="Rule action target" disabled={disabled} value={action.targetOptionGroup} onChange={(event) => { const next = references.find((entry) => entry.selectionKey === event.target.value)!; onChange({ ...action, targetOptionGroup: next.selectionKey, ...(action.action === "set_default" ? { value: initialRuleValue(next) } : {}) }); }}>{references.map((entry) => <option key={entry.selectionKey} value={entry.selectionKey}>{entry.label}</option>)}</select>
    {action.action === "set_default" ? <RuleValue value={action.value} source={target} multi={target.inputType === "multiselect"} disabled={disabled} onChange={(value) => onChange({ ...action, value })} /> : <span className="self-center text-[0.75rem] text-muted-foreground">{action.action === "clear" ? "Clears the current selection" : "Applies when this branch matches"}</span>}
...
function RuleValue({ value, source, multi, disabled, onChange }: Readonly<{ value: unknown; source: RuleOption; multi?: boolean; disabled?: boolean; onChange: (value: unknown) => void }>) {
  const current = multi ? (Array.isArray(value) ? value : value === undefined || value === null || value === "" ? [] : [value]) : value;
  const values = Array.isArray(current) ? current : [];
  if (source.choices.length && multi) {
    const selected = new Set(values.map(String));
    return <fieldset aria-label="Rule values" className="flex flex-wrap gap-x-2 gap-y-1 text-[0.75rem]">{source.choices.map((choice) => <label key={choice.choiceValue} className="inline-flex items-center gap-1"><input type="checkbox" disabled={disabled} checked={selected.has(choice.choiceValue)} onChange={(event) => { const next = event.target.checked ? [...selected, choice.choiceValue] : [...selected].filter((entry) => entry !== choice.choiceValue); onChange(next); }} />{choice.label}</label>)}</fieldset>;
  }
  if (source.choices.length) return <select aria-label="Rule value" disabled={disabled} value={String(current ?? "")} onChange={(event) => onChange(event.target.value)}><option value="">Select choice</option>{source.choices.map((choice) => <option key={choice.choiceValue} value={choice.choiceValue}>{choice.label}</option>)}</select>;
  if (source.inputType === "boolean") return <select aria-label="Rule value" disabled={disabled} value={current === true ? "true" : current === false ? "false" : ""} onChange={(event) => onChange(event.target.value === "true")}><option value="">Select boolean</option><option value="true">True</option><option value="false">False</option></select>;
  if (source.inputType === "number") {
    if (multi) return <input aria-label="Rule values" className="num h-8 text-[0.8125rem]" disabled={disabled} placeholder="e.g. 1, 2" value={values.map(String).join(", ")} onChange={(event) => onChange(event.target.value.split(",").map((item) => Number(item.trim())).filter(Number.isFinite))} />;
    return <input aria-label="Rule value" className="num h-8 text-[0.8125rem]" type="number" disabled={disabled} value={typeof current === "number" ? current : ""} onChange={(event) => { const raw = event.target.value; const parsed = Number(raw); onChange(raw === "" || !Number.isFinite(parsed) ? undefined : parsed); }} />;
  }
  if (multi) return <input aria-label="Rule values" className="h-8 text-[0.8125rem]" disabled={disabled} placeholder="Comma-separated values" value={values.map(String).join(", ")} onChange={(event) => onChange(event.target.value.split(",").map((item) => item.trim()).filter(Boolean))} />;
  const props = { "aria-label": "Rule value", className: "h-8 text-[0.8125rem]", disabled, value: typeof current === "string" ? current : "", onChange: (event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => onChange(event.target.value) };
  return source.inputType === "textarea" ? <textarea {...props} /> : <input {...props} />;
}
