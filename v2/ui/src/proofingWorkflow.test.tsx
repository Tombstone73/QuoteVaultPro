...
const evidencedDraft = { ...draft, recipients:[{contactId:"contact-a",displayName:"QA Customer",email:"qa@example.com"}], versions: [{ version: { ...draft.versions[0]!.version, artwork: [{ position: 0, artworkAssignmentId: "art-a", artworkFileId: "file-a" }] } }] } as unknown as ProofWorkProjection;
assert.match(render(evidencedDraft), /Issue &amp; Notify Customer/);
const issued = { ...evidencedDraft, versions: [{ version: { ...evidencedDraft.versions[0]!.version, issuedAt: "2026-08-20T00:01:00.000Z" } }] };
assert.match(render(issued), /Record Customer Approval/); assert.match(render(issued), /Record Request for Changes/);
...
try { await proofingApi.start("org a", "start-a", "order-a", "line-a"); await proofingApi.createVersion("org a", "work-a", "create-a", ["art-a"]); await proofingApi.issue("org a", "version-a", "issue-a","contact-a"); await proofingApi.respond("org a", "version-a", "response-a", "approved", "QA approval", "customer-a"); } finally { globalThis.fetch = originalFetch; }
...
assert.deepEqual(requests.map((request) => request.body), [{ orderId: "order-a", orderLineId: "line-a", businessRequestId: "start-a" }, { artworkAssignmentIds: ["art-a"], businessRequestId: "create-a" }, { recipientContactId:"contact-a",businessRequestId: "issue-a" }, { outcome: "approved", comment: "QA approval", recordedCustomerId: "customer-a", businessRequestId: "response-a" }]);
