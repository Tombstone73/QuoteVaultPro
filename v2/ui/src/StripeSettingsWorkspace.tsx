...
export const StripeSettingsWorkspace = ({ organizationId, sessionScope, canConfigure }: Readonly<{ organizationId: string; sessionScope: string; canConfigure: boolean }>) => {
  ...
  const value = query.data; const connection=value?.connection; const error = (query.error ?? connect.error ?? disconnect.error) as ApiError | null;
  return <main className="lab v2-sales-tax-settings"><header><small>SETTINGS · BILLING &amp; PAYMENTS</small><h1>Payments</h1><p>Each print shop is its own Stripe merchant of record. PrintersHero never exposes Stripe keys or secrets.</p></header>{query.isLoading ? <p>Loading Stripe readiness…</p> : <section className="v2-sales-notes"><h2>Stripe Connect</h2><p><b>Connection:</b> {status(connection?.status ?? "not_connected")}</p><p><b>Mode:</b> {(connection?.mode ?? value?.mode ?? "unknown").toUpperCase()}</p>{connection?.connectedAccountName && <p><b>Connected shop:</b> {connection.connectedAccountName}</p>}<p><b>Card payments:</b> {status(connection?.cardPayments ?? "unknown")}</p><p><b>Payouts:</b> {status(connection?.payouts ?? "unknown")}</p><p><b>Webhook:</b> {value?.webhook === "ready" ? "Ready" : value?.webhook ?? "Missing"}</p><p><b>Model:</b> Direct charges · Full Stripe Dashboard · Stripe collects processing fees.</p><p>{connection?.actionRequired ?? value?.actionRequired ?? "Stripe readiness is unavailable."}</p>{connection?.status === "ready" ? <button className="v2-quiet-button" disabled={disconnect.isPending} onClick={()=>disconnect.mutate()}>Disconnect Stripe</button> : <button className="v2-invoice-issue" disabled={connect.isPending} onClick={()=>connect.mutate()}>{connect.isPending?"Opening Stripe…":connection?.connected?"Resume Stripe onboarding":"Connect Stripe"}</button>}{error && <p className="notice error">{error.message ?? "Stripe readiness is unavailable."}</p>}</section>}</main>;
};
