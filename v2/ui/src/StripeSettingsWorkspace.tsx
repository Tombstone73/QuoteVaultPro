...
export const StripeSettingsWorkspace = ({ organizationId, sessionScope, canConfigure }: Readonly<{ organizationId: string; sessionScope: string; canConfigure: boolean }>) => {
  ...
  return <main className="lab v2-sales-tax-settings"><header><small>SETTINGS · BILLING &amp; PAYMENTS</small><h1>Payments</h1><p>Stripe credentials and webhook secrets are platform-managed. This page never exposes key or secret material.</p></header>{query.isLoading ? <p>Loading Stripe readiness…</p> : <section className="v2-sales-notes"><h2>Stripe</h2><p><b>Status:</b> {status(value?.status ?? "not_configured")}</p><p><b>Mode:</b> {(value?.mode ?? "unknown").toUpperCase()}</p><p><b>Webhook:</b> {value?.webhook === "ready" ? "Ready" : value?.webhook ?? "Missing"}</p><p>{value?.actionRequired ?? "Stripe readiness is unavailable."}</p><p>Configuration owner: platform-managed infrastructure.</p>{error && <p className="notice error">{error.message ?? "Stripe readiness is unavailable."}</p>}</section>}</main>;
};
