...
const proofStatus = (item: ProofQueueItem) => !item.latest ? "Draft" : item.latest.outcome === "approved" ? "Approved" : item.latest.outcome === "revision_requested" ? "Revision requested" : item.latest.issuedAt ? "Issued" : "Draft";
const versionStatus = (entry: ProofWorkProjection["versions"][number]) => entry.response?.outcome === "approved" ? "Approved" : entry.response?.outcome === "revision_requested" ? "Revision requested" : entry.version.issuedAt ? "Issued" : "Draft";
...
const PreviewUnavailable = () => <section className="v2-proof-preview-unavailable"><h2>Preview unavailable</h2><p>This workspace has no authorized Proof-file preview or download contract. It shows immutable Proofing evidence and canonical Artwork bindings without exposing private storage objects.</p></section>;
