...
const proofStatus = (item: ProofQueueItem) => !item.latest ? "Draft" : item.latest.outcome === "approved" ? "Approved" : item.latest.outcome === "revision_requested" ? "Revision requested" : item.latest.deliveryState === "suppressed" ? "Delivery suppressed in DEV QA" : item.latest.deliveryState === "failed" || item.latest.deliveryState === "ambiguous" ? "Delivery action required" : item.latest.issuedAt ? "Awaiting customer" : "Draft";
const versionStatus = (entry: ProofWorkProjection["versions"][number]) => entry.response?.outcome === "approved" ? "Approved" : entry.response?.outcome === "revision_requested" ? "Revision requested" : entry.delivery?.state === "suppressed" ? "Delivery suppressed in DEV QA" : entry.delivery?.state === "failed" || entry.delivery?.state === "ambiguous" ? "Delivery action required" : entry.version.issuedAt ? "Awaiting customer" : "Draft";
...
type ProofingWorkspaceProps = Readonly<{ organizationId: string; sessionScope: string; canView: boolean; canPrepare?: boolean; canIssue?: boolean; canRespond?: boolean; proofWorkId?: string; orderId?: string; lineId?: string; openOrder?: (orderId: string) => void; openCustomer?: (customerId: string) => void; openArtwork?: (artworkFileId: string) => void; openArtworkContext?: (orderId: string, lineId: string) => void }>;

/** Presents immutable Proofing evidence with a private version-bound viewer; it never owns Artwork, delivery, or recipients. */
export const ProofingWorkspace = ({ organizationId, sessionScope, canView, canPrepare = false, canIssue = false, canRespond = false, proofWorkId, orderId, lineId, openOrder, openCustomer, openArtwork, openArtworkContext }: ProofingWorkspaceProps) => {
  ...
  return <section className="v2-proofing">
    <aside className="v2-proof-queue" aria-label="Proof queue"><header><div><h2>Proof Queue</h2><span>{queue.data?.pagination.totalCount ?? 0} proofs</span></div><input aria-label="Search proof queue" placeholder="Order, customer, item" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} /><small>Server-backed search · paginated work</small></header><OperationalQueuePager page={page} pageSize={pageSize} total={queue.data?.pagination.totalCount ?? 0} totalPages={queue.data?.pagination.totalPages ?? 0} onPage={setPage} onPageSize={(next) => { setPageSize(next); setPage(1); }} /><div className="v2-proof-queue-list">{queue.isLoading && <p>Loading proof queue…</p>}{queue.isError && <p>Proof queue is unavailable.</p>}{queue.isSuccess && !groups.length && <p>No proof work matches this search.</p>}{groups.map(([orderNumber, items]) => <section className="v2-proof-order-group" key={orderNumber}><header><strong>Order #{orderNumber}</strong><span>{items[0]?.customerDisplayName}</span></header>{items.map((item) => <button type="button" key={item.work.proofWorkId} onClick={() => setSelectedId(item.work.proofWorkId)} className={selectedId === item.work.proofWorkId ? "active" : ""}><b>{item.lineDescription}</b><small>{item.latest ? `Current proof v${item.latest.sequence}` : "Needs proof"}</small><em data-status={proofStatus(item)}>{proofStatus(item)}</em></button>)}</section>)}</div><OperationalQueuePager page={page} pageSize={pageSize} total={queue.data?.pagination.totalCount ?? 0} totalPages={queue.data?.pagination.totalPages ?? 0} onPage={setPage} onPageSize={(next) => { setPageSize(next); setPage(1); }} /></aside>
    <main className="v2-proof-main">{loading ? <div className="skeleton" /> : !work.data && !context ? <div className="v2-proof-empty">Open Proofing from an Order line to start canonical Proof Work.</div> : <><ProofHeader context={context} item={selectedQueueItem} projection={work.data} onOpenOrder={openOrder} /><div className="v2-proof-content">{context && <SourceToVersion context={context} projection={work.data} onOpenOrder={openOrder} onOpenCustomer={openCustomer} onOpenArtwork={openArtwork} onOpenArtworkContext={openArtworkContext} />}{work.data && <ExactProofArtifactInspector organizationId={organizationId} projection={work.data} artwork={artwork.data ?? []} selectedVersionId={selectedVersionId} onOpenArtwork={openArtwork} />}{work.data && <ProofVersionHistory projection={work.data} artwork={artwork.data ?? []} selectedVersionId={selectedVersionId} onSelectVersion={setSelectedVersionId} onOpenArtwork={openArtwork} />}</div></>}</main>
    <aside className="v2-proof-rail" aria-label="Proof evidence and actions">{work.data && <ProofEvidenceRail projection={work.data} />}<ProofWorkflowActions organizationId={organizationId} context={context} projection={work.data} canPrepare={canPrepare} canIssue={canIssue} canRespond={canRespond} onOpenArtwork={openArtwork} onOpenArtworkContext={openArtworkContext} onRefresh={refresh} /></aside>
  </section>;
};
...
const SourceToVersion = ({ context, projection, onOpenOrder, onOpenCustomer, onOpenArtwork, onOpenArtworkContext }: Readonly<{ context: ProofingOrderLineContext; projection?: ProofWorkProjection; onOpenOrder?: (orderId: string) => void; onOpenCustomer?: (customerId: string) => void; onOpenArtwork?: (artworkFileId: string) => void; onOpenArtworkContext?: (orderId: string, lineId: string) => void }>) => {
  const latest = projection?.versions[0];
  return <section className="v2-proof-source"><div><small>Source Artwork</small><strong>{context.sourceArtwork.length ? context.sourceArtwork.map((item) => item.file.displayFilename).join(", ") : "No eligible source Artwork"}</strong><span>{context.sourceArtwork.length ? `${context.sourceArtwork.length} canonical assignment${context.sourceArtwork.length === 1 ? "" : "s"} on this Order line` : "Customer-supplied or reference Artwork is required before starting Proof Work."}</span></div><span className="v2-proof-source-arrow" aria-hidden>→</span><div><small>Current Proof Version</small><strong>{latest ? `Proof v${latest.version.sequence}` : "Not created"}</strong><span>{latest ? `${latest.version.artwork.length} exact Artwork assignment${latest.version.artwork.length === 1 ? "" : "s"} bound as immutable evidence` : "Create a version only from canonical Artwork evidence."}</span></div><div className="v2-proof-context-links">{onOpenOrder && <button type="button" onClick={() => onOpenOrder(context.order.order.orderId)}>Open Order</button>}{onOpenCustomer && <button type="button" onClick={() => onOpenCustomer(context.order.order.customerContact.customerId)}>Open Customer</button>}{onOpenArtworkContext && <button type="button" onClick={() => onOpenArtworkContext(context.order.order.orderId, context.line.lineId)}>Open line Artwork</button>}{onOpenArtwork && context.sourceArtwork.map((item) => <button type="button" key={item.file.id} onClick={() => onOpenArtwork(item.file.id)}>Open {item.file.displayFilename}</button>)}</div><p>Proofing owns approval evidence. Prepress and Routing remain separate owner workflows.</p></section>;
};

const bytes = (value: number | undefined) => value === undefined || !Number.isFinite(value) || value < 0 ? "Size unavailable" : value < 1024 ? `${value} B` : value < 1024 * 1024 ? `${(value / 1024).toFixed(1)} KB` : `${(value / (1024 * 1024)).toFixed(1)} MB`;

/**
 * Each rendered URL is scoped to the exact immutable Proof-version binding.
 * It never substitutes current Artwork for an older Proof Version.
 */
export const ExactProofArtifactInspector = ({ organizationId, projection, artwork, selectedVersionId, onOpenArtwork }: Readonly<{ organizationId: string; projection: ProofWorkProjection; artwork: readonly ArtworkOrderProjection[]; selectedVersionId: string; onOpenArtwork?: (artworkFileId: string) => void }>) => {
  const selected = projection.versions.find((entry) => entry.version.proofVersionId === selectedVersionId) ?? projection.versions[0];
  if (!selected) return null;
  const files = new Map(artwork.map((item) => [item.file.id, item]));
  return <section className="v2-proof-artifact-inspector" aria-label={`Exact artifact evidence for Proof version ${selected.version.sequence}`}><header><div><small>{selected.version.proofVersionId === projection.versions[0]?.version.proofVersionId ? "Current Proof" : "Historical Proof"}</small><h2>Exact artifact evidence · Proof v{selected.version.sequence}</h2><p>Only immutable assignments bound to this Proof version are shown. The private viewer is authorized against this exact Proof-version binding.</p></div><span>{selected.version.artwork.length} bound</span></header>{selected.version.artwork.length ? <div className="v2-proof-artifact-list">{selected.version.artwork.map((bound, index) => {
    const item = files.get(bound.artworkFileId);
    const filename = item?.file.displayFilename ?? `Bound artwork ${index + 1}`;
    const artifactUrl = proofingApi.artifactUrl(organizationId, selected.version.proofVersionId, bound.artworkFileId);
    const image = item?.file.contentType.startsWith("image/") === true;
    return <article key={bound.artworkAssignmentId}><div className="v2-proof-artifact-preview">{image ? <img src={artifactUrl} alt={`Exact proof artifact: ${filename}`} /> : <iframe src={artifactUrl} title={`Exact proof artifact: ${filename}`} />}</div><div><strong>{filename}</strong><small>{item ? `${item.file.contentType} · ${bytes(item.file.byteSize)} · ${usage(item.assignment)}` : "The exact immutable assignment is retained, but its file metadata is not present in this workboard response."}</small></div><div className="v2-proof-artifact-actions"><a href={artifactUrl} target="_blank" rel="noreferrer">Open artifact</a>{item && onOpenArtwork ? <button type="button" onClick={() => onOpenArtwork(item.file.id)}>Open canonical Artwork</button> : <span className="v2-proof-artifact-retained">Bound evidence retained</span>}</div></article>;
  })}</div> : <p className="v2-proof-artifact-empty">This Proof version has no bound Artwork evidence and cannot be sent as a valid proof.</p>}</section>;
};
...
export const ProofWorkflowActions = ({ organizationId, context, projection, canPrepare, canIssue, canRespond, onOpenArtwork, onOpenArtworkContext, onRefresh }: Readonly<{ organizationId: string; context?: ProofingOrderLineContext; projection?: ProofWorkProjection; canPrepare: boolean; canIssue: boolean; canRespond: boolean; onOpenArtwork?: (artworkFileId: string) => void; onOpenArtworkContext?: (orderId: string, lineId: string) => void; onRefresh: (proofWorkId?: string) => Promise<void> }>) => {
  ...
  if (latest.response?.outcome === "revision_requested") return <Action title="Revision requested" notice={notice}><p>Version v{latest.version.sequence} remains immutable. Review the customer comment and adopt revised Artwork through the canonical line-artwork workspace before creating the next version.</p>{latest.response.comment && <blockquote className="v2-proof-revision-comment">“{latest.response.comment}”</blockquote>}<div className="v2-proof-revision-actions">{onOpenArtworkContext && <button type="button" className="button secondary" onClick={() => onOpenArtworkContext(context.order.order.orderId, context.line.lineId)}>Open line Artwork</button>}{latest.version.artwork[0] && onOpenArtwork && <button type="button" className="button secondary" onClick={() => onOpenArtwork(latest.version.artwork[0]!.artworkFileId)}>Inspect prior Proof Artifact</button>}{canPrepare && <button type="button" className="button" disabled={pending} onClick={() => createVersion.mutate()}>Create Next Proof Version</button>}</div></Action>;
