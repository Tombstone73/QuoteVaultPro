...
import { formatArtworkInches, inspectArtworkPdf, type ArtworkPdfInspection } from "./artworkPdfDimensions";
  ...
  const select = (files: FileList | null) => {
    for (const file of Array.from(files ?? [])) {
      ...
      void inspectArtworkPdf(file).then((inspection) => {
        ...
        const inspectionError = error instanceof Error ? error.message : "Artwork dimensions could not be read.";
        setArtwork((current) => current.map((entry) => entry.clientArtworkKey === clientArtworkKey ? { ...entry, inspectionError } : entry));
      });
    }
  };
