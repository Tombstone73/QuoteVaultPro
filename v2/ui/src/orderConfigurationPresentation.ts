...
const isInternalIdentifier = (value: string): boolean =>
  uuid.test(value) || internalConfigurationIdentifier.test(value) || importedConfigurationKey.test(value);
...
const frozenPresentation = (value: unknown): readonly FrozenSelection[] | undefined => {
  ...
  const selections = value.selections.flatMap((selection): FrozenSelection[] => {
    if (!isRecord(selection) || typeof selection.label !== "string" || typeof selection.value !== "string") return [];
    if (!selection.label.trim() || isInternalIdentifier(selection.label) || isInternalIdentifier(selection.value)) return [];
 ...
 */
export const orderConfigurationPresentation = (resolved: ResolvedConfiguration): string => {
  ...
  if (frozen?.length) {
    ...
    return [dimensions, ...frozen.map((selection) => `${selection.label}: ${selection.value}`)]
  ...
  if (isRecord(resolved.selections)) {
    for (const [key, rawValue] of Object.entries(resolved.selections)) {
      ...
      if (isInternalIdentifier(key)) {
        parts.push(value ? `Configuration label unavailable: ${value}` : "Configuration label unavailable");
      } else if (value) {
