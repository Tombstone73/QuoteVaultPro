...
export const OrderWorkspace = (props: Readonly<{
  organizationId: string; sessionScope: string; orderId: string; canEdit: boolean; canOverridePrice: boolean; canViewInvoice: boolean; canViewArtwork: boolean; canViewProofing: boolean; csrfReady: boolean;
  onBack: () => void; openCustomer?: (customerId: string) => void; openFulfillment?: (orderId: string) => void; openInvoice?: (invoiceId: string) => void; openArtwork?: (orderId: string, lineId: string) => void; openProofing?: (orderId: string, lineId: string) => void; openQuote?: (quoteId: string) => void;
}>) => {
  ...
  const fulfillmentAvailable = fulfillment.data?.lines.reduce((total, line) => total + line.availableFulfillmentQuantity, 0);
  const headerMetadata = <><div className="v2-sales-compact-meta">
    ...
    <div className="v2-sales-inline-fact"><small>Fulfillment</small><button type="button" className="v2-sales-inline-button" onClick={() => props.openFulfillment?.(current.order.orderId)}>{fulfillment.isSuccess ? `${fulfillmentAvailable ?? 0} available` : "Open Fulfillment"}</button></div>
...
const OrderFulfillment = ({ loading, fulfillment, onOpen }: Readonly<{ loading: boolean; fulfillment?: import("./api").FulfillmentWorkspaceOrder; onOpen: () => void }>) => <section className="v2-order-tab"><header><div><h2>Fulfillment</h2><p>{loading ? "Loading…" : fulfillment ? `${fulfillment.handoffs.length} recorded handoff${fulfillment.handoffs.length === 1 ? "" : "s"}` : "Fulfillment details are unavailable."}</p></div><button className="button secondary" type="button" onClick={onOpen}>Open Fulfillment</button></header>{fulfillment && <dl>{fulfillment.lines.map((line) => <div key={line.orderLineId}><dt>{line.description}</dt><dd>{line.availableFulfillmentQuantity} available · {line.remainingFulfillmentQuantity} not handed off</dd></div>)}</dl>}</section>;
...
const OrderArtworkPanel = ({ lines, artwork, loading, canUpload, onOpen }: Readonly<{ lines: readonly { lineId: string; description: string }[]; artwork: readonly ArtworkOrderProjection[]; loading: boolean; canUpload: boolean; onOpen: (lineId: string) => void }>) => <section className="v2-order-tab"><header><div><h2>Artwork</h2><p>{loading ? "Loading…" : artwork.length ? `${artwork.length} file${artwork.length === 1 ? "" : "s"}` : "No artwork is attached."}</p></div></header>{!loading && <ul className="v2-order-artwork">{lines.map((line) => { const assigned = artwork.filter((entry) => entry.assignment.orderLineId === line.lineId); return <li key={line.lineId}><b>{line.description || "Order line"}</b><span>{assigned.length ? assigned.map((entry) => `${entry.file.displayFilename} · ${artworkRole(entry)} · ${bytes(entry.file.byteSize)}`).join("; ") : "No artwork attached"}</span>{canUpload && <button className="button secondary" type="button" onClick={() => onOpen(line.lineId)}>Upload Artwork</button>}</li>; })}</ul>}</section>;
