...
type Template = RoutingWorkspaceRead["templates"][number];
...
const ProductionDestinationMapping = ({ organizationId, template, canManageTemplates, onSaved }: Readonly<{ organizationId: string; template: Template; canManageTemplates: boolean; onSaved: () => void }>) => {
  const production = template.steps.filter((step) => step.kind === "production");
  const [notice, setNotice] = useState("");
  const save = useMutation({ mutationFn: (input: Readonly<{ stepId: string; station: "flatbed" | "roll" }>) => routingApi.setProductionDestination(organizationId, template.routeTemplateId, input.stepId, newBusinessRequestId(), input.station), onSuccess: async () => { setNotice("Production destination saved."); await onSaved(); }, onError: (error) => setNotice((error as Error).message || "Production destination could not be saved.") });
  if (!production.length) return null;
  return <div className="v2-routing-destinations"><b>Production destination</b><small>Explicit station authority for Direct Production and Prepress handoff. Existing frozen routes keep their own route identity.</small>{production.map((step) => <label key={step.routeTemplateStepId}>Step {step.position + 1}<select disabled={!canManageTemplates || save.isPending} value={step.productionDestination ?? ""} onChange={(event) => { const station = event.currentTarget.value; if (station === "flatbed" || station === "roll") save.mutate({ stepId: step.routeTemplateStepId, station }); }}><option value="">Unconfigured</option><option value="flatbed">Flatbed</option><option value="roll">Roll</option></select>{!step.productionDestination && <small className="v2-routing-destination-warning">Unconfigured: Direct Production and Prepress handoff remain blocked.</small>}</label>)}{!canManageTemplates && <small>You do not have permission to configure Route Template destinations.</small>}{notice && <p className="v2-product-version-message" role="status">{notice}</p>}</div>;
};

const RouteTemplateEditor = ({ organizationId, template, canManageTemplates, onSaved }: Readonly<{ organizationId: string; template: Template; canManageTemplates: boolean; onSaved: () => void }>) => {
  ...
  if (!editing) return canManageTemplates ? <button type="button" onClick={() => { setName(template.name); setActive(template.active); setStepKinds(template.steps.map((step) => step.kind as TemplateStep)); setEditing(true); }}>Edit</button> : null;
...
export const RoutingWorkspace = ({ organizationId, sessionScope, canView, canAdvance, canManageTemplates, openOrder }: Readonly<{ organizationId: string; sessionScope: string; canView: boolean; canAdvance: boolean; canManageTemplates: boolean; openOrder: (id: string) => void }>) => {
  ...
  return <section className="v2-routing"><header className="v2-routing-heading"><div><p>Operations / Routing</p><h1>Routing</h1><span>Canonical templates and frozen Order-line routes.</span></div></header>
    ...
    {canManageTemplates && <form className="v2-product-form" onSubmit={(event) => { event.preventDefault(); create.mutate(); }}><h2>New Route Template</h2><label>Name<input value={name} onChange={(event) => setName(event.target.value)} required maxLength={160} /></label><label>Steps<select multiple value={stepKinds as string[]} onChange={(event) => setStepKinds(Array.from(event.currentTarget.selectedOptions).map((option) => option.value as "proofing"|"prepress"|"production"|"fulfillment"))}><option value="proofing">Proofing</option><option value="prepress">Prepress</option><option value="production">Production</option><option value="fulfillment">Fulfillment</option></select></label><button type="submit" disabled={!name.trim() || !stepKinds.length || create.isPending}>{create.isPending ? "Creating…" : "Create Route Template"}</button>{create.isError && <p className="v2-product-version-message">{(create.error as Error).message}</p>}</form>}
    <section><h2>Route templates</h2><div className="v2-routing-templates">{workspace.data?.templates.map((template) => <article key={template.routeTemplateId}><header><b>{template.name}</b><span>{template.active ? "Active" : "Inactive"}</span></header>{steps(template.steps)}<small>Revision {template.revision} · {template.routeTemplateId}</small><ProductionDestinationMapping organizationId={organizationId} template={template} canManageTemplates={canManageTemplates} onSaved={() => client.invalidateQueries({ queryKey: ["v2", sessionScope, organizationId, "routing", "workspace"] })} /><RouteTemplateEditor organizationId={organizationId} template={template} canManageTemplates={canManageTemplates} onSaved={() => client.invalidateQueries({ queryKey: ["v2", sessionScope, organizationId, "routing", "workspace"] })} /></article>)}{workspace.isSuccess && !workspace.data.templates.length && <p className="v2-proof-empty">No canonical Route Templates are available.</p>}</div></section>
