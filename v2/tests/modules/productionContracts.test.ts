...
const projection:ProductionWorkProjection={work,attempts:[],completedGoodQuantity:0,recordedGoodQuantity:0,rejectedGoodQuantity:0,usableGoodQuantity:0,remainingGoodQuantity:100,unitQuantitySatisfied:false,state:"ready",exceptionEvents:[],outputDispositions:[]};
...
assert.deepEqual(["production.view","production.work","production.complete","production.hold","production.rework","production.note","production.output.reject"].every(x=>capabilityIds.includes(x as typeof capabilityIds[number])),true);
assert.equal(projection.state,"ready","exception state is derived rather than a mutable work field");
assert.deepEqual(projection.exceptionEvents,[],"new work has no invented exception evidence");
console.log("[m7.8c] Production exception contract tests passed (7 assertions).");
