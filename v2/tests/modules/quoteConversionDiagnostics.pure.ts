...
trace.failure("order_insert", { code: "23505" });
...
assert(messages.every((message) => !/(customer|email|token|cookie|authorization|sql)/i.test(message)), "diagnostic messages must not contain forbidden sensitive fields");
