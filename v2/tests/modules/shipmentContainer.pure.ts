...
import { canAttachShipmentHandoff, markShipmentShipped, preparedShipment, voidPreparedShipment } from "../../src/modules/fulfillment/shipmentContainer.js";
...
const voided = voidPreparedShipment(prepared, { voidedAt: "2026-09-07T01:00:00.000Z", principalKind: "staff", principalSubject: "operator", reason: "Address correction" });
assert.equal(voided.status, "voided");
assert.equal(voided.voidReason, "Address correction");
assert.equal(canAttachShipmentHandoff(voided), false);
assert.throws(() => markShipmentShipped(voided, { shippedAt: "later", principalKind: "staff", principalSubject: "operator" }));
