...
class FakeTransaction implements RoutingLifecycleTransaction {
  route = base(); proof = false; prepress = false; requests = new Map<string, unknown>(); advances = 0; audits = 0;
...
assert(!production.ok && production.error.code === "CONFLICT", "Routing refuses to invent Production completion semantics.");
const denied = await service.completeCurrentStep(context("denied", []), { businessRequestId: "denied", routeInstanceId: routeId, expectedRevision: "3" });
