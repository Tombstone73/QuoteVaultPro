...
class MemoryArtworkTransaction implements ArtworkTransaction {
  ...
  async findOrderLineArtwork(_org: never, line: string) { return [...this.assignments.values()].filter((a)=>a.orderLineId===line).map((a)=>({assignment:a,file:this.files.get(a.artworkFileId)!})); }
  async findOrderArtwork(_org: never, orderId: string) { return [...this.assignments.values()].filter((a)=>a.orderId===orderId).map((a)=>({assignment:a,file:this.files.get(a.artworkFileId)!})); }
