...
assert.match(allocator, /v2_allocate_sales_document_number/);
...
assert.match(settings, /FROM quotes/);
assert.match(settings, /FROM orders/);
const migration = readFileSync(resolve("server/db/migrations_v2/0243_v2_quote_order_numbering_writer_convergence.sql"), "utf8");
assert.match(migration, /display_prefix/);
assert.match(migration, /v2_allocate_sales_document_number/);
