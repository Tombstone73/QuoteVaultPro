...
import { PostgresProductVersionLifecycleReader, createInitialProductDraftTree } from "../../infrastructure/products/postgresProductVersionLifecycle";
...
describe("P2 Product version lifecycle read",()=>{
  ...
  test("serializes draft creation on the Product row and never updates the Active pointer",async()=>{const source=await readFile("v2/infrastructure/products/postgresProductVersionLifecycle.ts","utf8");const create=source.slice(source.indexOf("async createDraftFromActive"),source.indexOf("async succeed"));expect(create).toContain("FROM products WHERE organization_id=$1 AND id=$2 FOR UPDATE");expect(create).toContain("status='ACTIVE' FOR UPDATE");expect(create).toContain("status='DRAFT' ORDER BY updated_at DESC,id DESC LIMIT 1 FOR UPDATE");expect(create).toContain("INSERT INTO pbv2_tree_versions");expect(create).not.toMatch(/UPDATE products/);expect(create).not.toMatch(/UPDATE pbv2_tree_versions/);});
