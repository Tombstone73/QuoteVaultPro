...
describe("M4 Customer workspace PostgreSQL projection", () => {
  test("binds active organization scope, searches canonical Customer/Contact facts, and bounds the catalog", async () => {
    ...
    const reader = new PostgresCustomerWorkspaceReader({ query: async <T>(text: string, values?: readonly unknown[]) => { calls.push({ text, values }); return { rows: [{ customer_id: "customer-a", display_name: "Acme", company_name: "Acme Printing", email: "billing@acme.test", phone: "555-0100", contact_id: "contact-a", contact_first_name: "Ada", contact_last_name: "Lovelace", contact_email: "ada@acme.test", contact_phone: "555-0111" }] as T[] }; } } as any);
    await expect(reader.list("org-a", "Ada")).resolves.toMatchObject([{ customerId: "customer-a", primaryContact: { contactId: "contact-a", displayName: "Ada Lovelace" } }]);
  ...
  test("reads Contacts only through the active tenant-scoped Customer relationship", async () => {
    const calls: { text: string; values?: readonly unknown[] }[] = [];
    const responses = [[customer], [customer], [linkedContact]];
