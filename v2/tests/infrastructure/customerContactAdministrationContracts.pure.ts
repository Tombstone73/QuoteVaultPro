...
const routes = source("v2/src/interfaces/http/customerRoutes.ts");
const quote = source("v2/src/modules/sales/quoteApplication.ts");
const order = source("v2/src/modules/sales/orderApplication.ts");
...
assert.match(reader, /openReceivable/);
assert.match(reader, /credit_limit_configured_at/);
assert.match(reader, /internalNotes/);
assert.match(service, /setBillingContact/);
assert.match(service, /is_billing/);
assert.match(service, /addInternalNote/);
assert.match(routes, /billing-contact/);
assert.match(routes, /internal-notes/);
assert.match(routes, /Credit limit must be a non-negative whole-cent amount/);
assert.match(quote, /resolvedTerms/);
assert.match(order, /resolvedTerms/);
