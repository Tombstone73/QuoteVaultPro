...
import { quickBooksPaymentReference } from "../../infrastructure/accounting/quickBooksPaymentReference.js";
...
const approvalMigration = readFileSync(resolve("server/db/migrations_v2/0262_v2_quickbooks_invoice_approval_gate.sql"), "utf8");
const paymentReferenceMigration = readFileSync(resolve("server/db/migrations_v2/0263_v2_quickbooks_payment_reference_sequence.sql"), "utf8");
...
assert.match(approvalMigration, /v2_quickbooks_invoice_approvals/);
assert.match(approvalMigration, /UNIQUE \(organization_id, invoice_id, synchronization_version\)/);
assert.match(approvalMigration, /FOREIGN KEY \(invoice_id, organization_id\)/);
assert.match(queue, /Approve the current V2 Invoice version for accounting before forcing QuickBooks sync/);
assert.match(queue, /v2_quickbooks_invoice_approvals a ON a\.organization_id=i\.organization_id AND a\.invoice_id=i\.id AND a\.synchronization_version=i\.synchronization_version/);
assert.match(queue, /async approveInvoice/);
assert.match(queue, /quickbooks_invoice_approved/);
assert.match(queue, /accountingApproval:row\.approval_id \? "approved" : "required"/);
...
assert.match(provider, /allocations: readonly Readonly<\{ quickBooksInvoiceId: string; amountCents: number \}>\[\]/u);
assert.match(provider, /input\.allocations\.map/u);
assert.match(queue, /v2_billing_payment_allocations a ON a\.organization_id=p\.organization_id AND a\.payment_id=p\.id/u);
assert.match(queue, /allocationTotal !== Number\(row\.payment_cents\)/u);
...
assert.equal(quickBooksPaymentReference("42"), "PMT-42", "a V2 QuickBooks Payment uses a short persisted PMT sequence");
assert.throws(() => quickBooksPaymentReference("0"), /invalid/, "a Payment reference sequence cannot be zero");
...
assert.match(routes, /\/invoices\/:invoiceId\/approve/);
assert.match(routes, /quickBooksSync\.approveInvoice/);
...
assert.match(consoleUi, /Approve for accounting/);
assert.match(consoleUi, /Force Sync selected/);
assert.match(consoleUi, /Approval required/);
assert.match(consoleUi, /awaitingApprovalInvoiceCount/);
