...
describe("Prepress HTTP transport",()=>{
  ...
  test("forwards bounded server-owned station and readiness filters",async()=>{const calls:any[]=[];const server=app(principal("org-a"),calls);await request(server).get("/v2/organizations/org-a/prepress/queue?destination=roll&readiness=ready").expect(200);expect(calls.find(call=>call.kind==="list")).toMatchObject({page:{destination:"roll",readiness:"ready"}});});
  test("accepts one bounded bulk handoff command instead of browser loops",async()=>{const calls:any[]=[];const server=app(principal("org-a"),calls);await request(server).post("/v2/organizations/org-a/prepress/units/send-to-production").send({businessRequestId:"bulk-1",prepressUnitIds:["unit-a","unit-b"]}).expect(200);expect(calls.find(call=>call.kind==="bulk")).toMatchObject({input:{businessRequestId:"bulk-1",prepressUnitIds:["unit-a","unit-b"]}});});
