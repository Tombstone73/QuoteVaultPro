...
assert.match(routes,/router\.get\("\/proofs"/u);
assert.match(routes,/router\.post\("\/proofs\/:proofVersionId\/respond",requireV2CsrfToken/u);
...
assert.match(ui,/Proof v\$\{proof\.revision\}/u);
assert.match(ui,/Request changes/u);
assert.match(ui,/<Notice tone="warning">\{error\}<\/Notice>/u);
