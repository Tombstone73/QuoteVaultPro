...
assert.match(routes,/router\.post\("\/payments\/stripe\/payment-intents",requireV2CsrfToken/u);
assert.match(routes,/beginPaymentAggregate/u);
assert.match(routes,/One card payment cannot span Invoice currencies/u);
assert.match(routes,/Payment allocation exceeds the current Invoice balance/u);
...
assert.match(ui,/\/v2\/portal\/payments\/stripe\/payment-intents/u);
assert.match(ui,/Pay selected invoices/u);
assert.doesNotMatch(ui,/Multi-invoice checkout remains disabled/u);
