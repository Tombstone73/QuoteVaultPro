...
{
  const {value,calls,bootstrapArguments}=app(actor,(organizationId)=>organizationId==="org-a");const response=await request(value).post("/v2/organizations/org-a/settings/team-access/portal-access/m77f-qa-setup").send({businessRequestId:"qa-setup",expectedAuthorityRevision:"7",confirmM77fQaPortalSetup:true,customerId:"customer-a",contactId:"contact-a",permissionSetId:"portal-set"}).expect(201);assert.equal(response.body.data.deliveryState,"suppressed");assert.deepEqual(calls,["portal-bootstrap"]);assert.deepEqual(bootstrapArguments[0][4],{captureM77fQaSetupUrl:true});
}
{
  const {value,calls}=app(actor,()=>false);await request(value).post("/v2/organizations/org-a/settings/team-access/portal-access/m77f-qa-setup").send({businessRequestId:"qa-setup-denied",expectedAuthorityRevision:"7",confirmM77fQaPortalSetup:true,customerId:"customer-a",contactId:"contact-a",permissionSetId:"portal-set"}).expect(403);assert.deepEqual(calls,[]);
}
{
  const {value,calls}=app(actor,()=>true);await request(value).post("/v2/organizations/org-a/settings/team-access/portal-access/m77f-qa-setup").send({businessRequestId:"qa-setup-unconfirmed",expectedAuthorityRevision:"7",customerId:"customer-a",contactId:"contact-a",permissionSetId:"portal-set"}).expect(400);assert.deepEqual(calls,[]);
}
