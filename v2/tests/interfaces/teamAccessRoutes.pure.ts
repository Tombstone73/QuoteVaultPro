...
const snapshot={authorityRevision:"7",staff:[{memberId:"staff-a",status:"active"}],invitations:[],permissionSets:[],portalAccess:[],audit:[],readiness:{status:"ready" as const,reasons:[],activeStaffCount:1,viableAdministratorCount:1,pendingInvitationCount:0},capabilityGroups:[]};
const app=(principal=actor)=>{const calls:string[]=[];const team:any={read:async()=>snapshot,createInvitation:async()=>{calls.push("invite");return{invitationId:"invite-a",status:"pending" as const};},setMembershipActive:async()=>{calls.push("status");},replaceStaffAssignments:async()=>{calls.push("staff-sets");},replacePortalAssignments:async()=>{calls.push("portal-sets");},createCustomSet:async()=>{calls.push("create-set");return{permissionSetId:"set-a"};},updateCustomSet:async()=>{calls.push("update-set");}};const value=express();value.use(express.json());value.use("/v2/organizations/:organizationId/settings/team-access",createTeamAccessRouter({teamAccess:team,principals:{principal:async()=>principal} as any}));return{value,calls};};
