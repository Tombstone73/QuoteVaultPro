...
import type {
  ...
  CustomersReadPort,
} from "../../src/modules/customers/contracts.js";
import {
  brandedId,
  type ContactId,
  type CustomerId,
...
const display = (row: CustomerRow): string =>
  row.display_name?.trim() || row.company_name;
const contactDisplay = (row: ContactRow): string =>
  `${row.first_name} ${row.last_name}`.trim();
const address = (
  row: CustomerRow,
  prefix: "billing" | "shipping",
): CustomerPresentationIdentity["billingAddress"] => {
  const fields = row as unknown as Record<string, string | null>;
  const values = [
    fields[`${prefix}_street1`],
    fields[`${prefix}_street2`],
  ].filter((value): value is string => Boolean(value?.trim()));
  if (
    !values.length &&
    !fields[`${prefix}_city`] &&
    !fields[`${prefix}_state`] &&
    !fields[`${prefix}_postal_code`] &&
    !fields[`${prefix}_country`]
  )
    return undefined;
  return {
    lines: values,
    ...(fields[`${prefix}_city`] ? { city: fields[`${prefix}_city`]! } : {}),
    ...(fields[`${prefix}_state`]
      ? { region: fields[`${prefix}_state`]! }
      : {}),
    ...(fields[`${prefix}_postal_code`]
      ? { postalCode: fields[`${prefix}_postal_code`]! }
      : {}),
    ...(fields[`${prefix}_country`]
      ? { countryCode: fields[`${prefix}_country`]! }
      : {}),
  };
};
...
export class PostgresCustomersCompatibilityReader implements CustomersReadPort {
  ...
  async getCustomer(
  ...
  ): Promise<Readonly<{ id: CustomerId; displayName: string }> | null> {
    ...
    return row
      ? { id: brandedId<"CustomerId">(row.id), displayName: display(row) }
      : null;
  }
  ...
  async getContact(
    ...
    const result = await this.client.query<ContactRow>(
      ...
      FROM customer_contacts WHERE organization_id = $1 AND id = $2 AND status = 'active'`,
      [organizationId, contactId],
    );
    const row = result.rows[0];
    return row
      ? { id: brandedId<"ContactId">(row.id), displayName: contactDisplay(row) }
      : null;
  }

  async validateContactReference(
    reference: CustomerContactReference,
  ): Promise<boolean> {
    if (!reference.customerId)
      return Boolean(
        await this.getContact(reference.organizationId, reference.contactId!),
      );
    if (!reference.contactId)
      return Boolean(
        await this.getCustomer(reference.organizationId, reference.customerId),
      );
    const result = await this.client.query<{ id: string }>(
      ...
      [reference.organizationId, reference.customerId, reference.contactId],
    );
    return result.rows.length === 1;
  }

  async getPresentationIdentity(
    reference: CustomerContactReference,
  ): Promise<CustomerPresentationIdentity> {
    const customer = reference.customerId
      ? await this.customerRow(reference.organizationId, reference.customerId)
      : null;
    const contact = reference.contactId
      ? await this.contactRow(
    ...
    const billingAddress = customer ? address(customer, "billing") : undefined;
    ...
    return {
      ...(customer
        ? {
            customerDisplayName: display(customer),
            companyName: customer.company_name,
          }
        : {}),
      ...(contact ? { contactDisplayName: contactDisplay(contact) } : {}),
      ...(email ? { email } : {}),
      ...(phone ? { phone } : {}),
  ...
  private async contactRow(
    ...
    if (!customerId) {
      const result = await this.client.query<ContactRow>(
        `SELECT id, first_name, last_name, email, phone FROM customer_contacts
    ...
    const result = await this.client.query<ContactRow>(
      ...
      WHERE ct.organization_id = $1 AND ct.id = $2 AND c.id = $3 AND ct.status = 'active' AND c.is_active IS NOT FALSE
        AND COALESCE(c.status, 'active') NOT IN ('archived', 'superseded', 'deleted') AND c.merged_into_customer_id IS NULL`,
      [organizationId, contactId, customerId],
    );
    return result.rows[0] ?? null;
  }
}
