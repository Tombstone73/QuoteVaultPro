...
export class PostgresInvoiceDocumentService {
  ...
  async document(organizationId: OrganizationId, invoiceId: InvoiceId): Promise<OwnerPdfDocument> {
    ...
    const invoiceNumber = invoice.invoiceNumber ?? invoice.sourceOrderNumber ?? "Invoice";
    ...
    return { kind: "invoice", title: `Invoice · ${invoiceNumber}`, number: invoiceNumber, issuedAt: source.date, organization: documentBranding, sections: [
      ...
      ...(issued ? [{ heading: "Issued record", entries: [{ value: "Commercial facts and customer presentation are preserved by Billing's immutable issued checkpoint." }] }] : orderBacked ? [{ heading: "Order-backed status", entries: [{ value: "This payable Invoice follows the current canonical Order commercial projection. Payments and Refunds remain immutable financial facts." }] }] : []),
    ] };
  }
