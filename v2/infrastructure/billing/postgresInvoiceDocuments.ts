...
export class PostgresInvoiceDocumentService {
  ...
  async document(organizationId: OrganizationId, invoiceId: InvoiceId): Promise<OwnerPdfDocument> {
    ...
    return { kind: "invoice", title: `${invoice.lifecycle === "draft" ? "Draft invoice" : "Invoice"} · ${invoice.sourceOrderNumber ?? "Order"}`, number: invoice.sourceOrderNumber ?? "Invoice", issuedAt: source.date, organization: branding, sections: [
