...
export class PostgresInvoiceDocumentService {
  ...
  async document(organizationId: OrganizationId, invoiceId: InvoiceId): Promise<OwnerPdfDocument> {
    ...
    return { kind: "invoice", title: `${invoice.lifecycle === "draft" ? "Draft invoice" : "Invoice"} · ${invoiceNumber}`, number: invoiceNumber, issuedAt: source.date, organization: documentBranding, sections: [
