...
const constraints = [
  ...
  "v2_artwork_assignments_order_line_identity_uidx", "v2_artwork_assignments_file_tenant_fk", "v2_artwork_assignments_order_tenant_fk", "v2_artwork_assignments_order_line_tenant_fk",
] as const;
const indexes = ["v2_artwork_files_org_created_idx", "v2_artwork_assignments_org_order_line_idx", "v2_artwork_assignments_org_file_idx"] as const;
...
export async function checkV2ArtworkPhysicalPostconditions(client: TransactionalClient): Promise<readonly Readonly<{ id: string; passed: boolean; detail: string }>[]> {
  const [foundTables, foundConstraints, foundIndexes, foundTriggers] = await Promise.all([
    ...
    client.query<{ tgname: string }>("SELECT tgname FROM pg_trigger WHERE tgname='v2_artwork_file_lineage_validate_trigger' AND NOT tgisinternal"),
  ]);
