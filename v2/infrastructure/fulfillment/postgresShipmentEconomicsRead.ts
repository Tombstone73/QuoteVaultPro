...
type ShipmentRow = { id:string; estimated_carrier_cost_cents:string|null; customer_shipping_price_cents:string|null; customer_shipping_price_frozen_at:Date|null; shipping_pricing_policy_snapshot:unknown; shipping_responsibility:"titan"|"customer"|"carrier"|"pending"|null; shipping_reason:string|null; shipping_note:string|null; actual_cost:string|null };
...
export class PostgresShipmentEconomicsStaffRead implements ShipmentEconomicsStaffReadPort {
  ...
  async readStaffEconomics(organizationId: OrganizationId, shipmentId: string): Promise<StaffShipmentEconomics | null> {
    const shipment = await this.pool.query<ShipmentRow>(`SELECT s.id,s.estimated_carrier_cost_cents::text,s.customer_shipping_price_cents::text,s.customer_shipping_price_frozen_at,s.shipping_pricing_policy_snapshot,s.shipping_responsibility,s.shipping_reason,s.shipping_note,
