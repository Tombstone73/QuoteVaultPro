...
import { fulfillmentPhysicalIntegrityAnomaly, fulfillmentSupplyQuantity, type FulfillmentAvailability, type FulfillmentHandoff, type FulfillmentHandoffLine, type ReplacementFulfillmentAvailability } from "../../src/modules/fulfillment/contracts.js";
...
type AvailabilityRow={order_document_id:string;order_line_id:string;ordered_quantity:number;pickup_quantity:string;shipment_quantity:string;reserved_prepared_quantity:string;completed_production_quantity:string;workflow_intent:"standard_production"|"fulfillment_only"|"service_fee"|null;requires_production:boolean};
...
const availability=(r:AvailabilityRow):FulfillmentAvailability=>{
 const pickup=Number(r.pickup_quantity),shipment=Number(r.shipment_quantity),reservedPrepared=Math.max(0,Number(r.reserved_prepared_quantity)),completedFulfillment=pickup+shipment;
...
const availabilitySql=`WITH production_output AS (
 ...
 f.pickup_quantity,f.shipment_quantity,pr.reserved_prepared_quantity,p.completed_production_quantity,
...
export class PostgresFulfillmentTransaction implements FulfillmentTransaction {
 ...
 async writeDocumentSnapshot(input:Parameters<FulfillmentTransaction["writeDocumentSnapshot"]>[0]){const result=await this.client.query<{snapshot:unknown}>(`INSERT INTO v2_fulfillment_handoff_document_snapshots(organization_id,handoff_id,snapshot)
   SELECT h.organization_id,h.id,jsonb_build_object('orderNumber',d.display_number,'customer',COALESCE(NULLIF(btrim(c.display_name),''),NULLIF(btrim(c.company_name),''),'Customer unavailable'),'purchaseOrder',d.purchase_order_number,'requestedMethod',o.requested_fulfillment_method,'destination',o.requested_destination,'instructions',o.fulfillment_instructions,'method',h.handoff_method,'completedAt',h.completed_at,'lines',COALESCE(jsonb_agg(jsonb_build_object('description',l.description,'quantity',hl.quantity) ORDER BY l.position), '[]'::jsonb))
   FROM v2_fulfillment_handoffs h JOIN v2_sales_documents d ON d.organization_id=h.organization_id AND d.id=h.order_document_id JOIN v2_sales_order_details o ON o.organization_id=d.organization_id AND o.document_id=d.id LEFT JOIN customers c ON c.organization_id=d.organization_id AND c.id=d.customer_id JOIN v2_fulfillment_handoff_lines hl ON hl.organization_id=h.organization_id AND hl.handoff_id=h.id JOIN v2_sales_document_lines l ON l.organization_id=hl.organization_id AND l.id=hl.order_line_id AND l.document_id=hl.order_document_id
   WHERE h.organization_id=$1 AND h.id=$2 GROUP BY h.organization_id,h.id,d.display_number,d.purchase_order_number,c.display_name,c.company_name,o.requested_fulfillment_method,o.requested_destination,o.fulfillment_instructions,h.handoff_method,h.completed_at
   ON CONFLICT (organization_id,handoff_id) DO NOTHING RETURNING snapshot`,[input.organizationId,input.handoffId]);if(!result.rows[0])throw Error("Fulfillment handoff document snapshot was not available.");}
