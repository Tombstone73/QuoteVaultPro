...
export const composeAuthenticatedFulfillmentRuntime=(input:AuthenticatedFulfillmentRuntimeDependencies):AuthenticatedFulfillmentRuntime=>{const principals=new IssuedV2PrincipalProvider(input.trustedHostIdentity,new PermissionSetPrincipalIssuer(new PostgresPermissionAuthorityReader(input.pool)));return {dependencies:{service:input.service??new FulfillmentApplicationService(new PostgresFulfillmentTransactionRunner(input.pool),undefined,input.orderLifecycle),shipments:new ShipmentContainerApplicationService(new PostgresShipmentContainerRunner(input.pool),undefined,input.orderLifecycle),shipmentEconomics:new PostgresShipmentEconomicsStaffRead(input.pool),replacements:new PostgresReplacementObligationService(input.pool,input.orderLifecycle),principals,workspace:new PostgresFulfillmentWorkspaceReads(input.pool),documents:new PostgresFulfillmentDocumentService(input.pool)},trustedHostMiddleware:input.trustedHostMiddleware};};
