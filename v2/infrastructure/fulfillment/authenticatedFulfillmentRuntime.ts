...
export const composeAuthenticatedFulfillmentRuntime=(input:AuthenticatedFulfillmentRuntimeDependencies):AuthenticatedFulfillmentRuntime=>{const principals=new IssuedV2PrincipalProvider(input.trustedHostIdentity,new PermissionSetPrincipalIssuer(new PostgresPermissionAuthorityReader(input.pool)));return {dependencies:{service:input.service??new FulfillmentApplicationService(new PostgresFulfillmentTransactionRunner(input.pool)),shipments:new ShipmentContainerApplicationService(new PostgresShipmentContainerRunner(input.pool)),principals,workspace:new PostgresFulfillmentWorkspaceReads(input.pool),documents:new PostgresFulfillmentDocumentService(input.pool)},trustedHostMiddleware:input.trustedHostMiddleware};};
