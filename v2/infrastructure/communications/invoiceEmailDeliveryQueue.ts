...
export class PostgresInvoiceEmailDeliveryQueue {
  ...
  async process(job:Job,workerId:string):Promise<InvoiceEmailState>{
    ...
    catch(error){const message=concise(error);const state=providerAttempted?this.providerState(error):this.preProviderState(error);await this.finish(job,workerId,state,message);return state;}
  }
...
export class V2InvoiceEmailDeliveryWorker {
  ...
  async run(input:Readonly<{limit:number;leaseSeconds:number;spacingMs:number}>):Promise<Readonly<Record<InvoiceEmailState|"claimed",number>>>{const result:Record<InvoiceEmailState|"claimed",number>={claimed:0,queued:0,processing:0,retry_wait:0,sent:0,failed:0,ambiguous:0};for(let i=0;i<Math.max(1,input.limit);i+=1){const job=await this.queue.claim(this.workerId,input.leaseSeconds,input.spacingMs);if(!job)break;result.claimed+=1;result[await this.queue.process(job,this.workerId)]+=1;}return result;}
}
