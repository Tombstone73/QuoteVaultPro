...
export class PostgresPrepressTransaction implements PrepressTransaction {
  ...
  async listQueue(org:OrganizationId,limit:number):Promise<readonly PrepressQueueItem[]>{
    ...
    const rows=await this.client.query<{order_id:string;order_number:string;customer_display_name:string;line_id:string;line_description:string;quantity:number;requested_due_date:string|null;step_kind:"proofing"|"prepress"|"production"|"fulfillment"|null;production_requirement_state:"configured"|"unconfigured"}>(`SELECT d.id order_id,d.display_number order_number,COALESCE(c.display_name,c.company_name,'Customer') customer_display_name,l.id line_id,l.description line_description,l.quantity,d.requested_due_date::text,step.step_kind,l.production_requirement_state
