...
type UnitRow = {
  ...
  rework_cycle_id:string|null; rework_reason?:string|null; predecessor_production_work_id?:string|null; rework_remaining_required_quantity?:number|null;
...
const unit = (r:UnitRow):PrepressUnit => ({
  ...
  ...(r.rework_cycle_id?{reworkCycleId:brandedId<"ProductionReworkCycleId">(r.rework_cycle_id)}:{}),...(r.rework_reason?{reworkReason:r.rework_reason}:{}),...(r.predecessor_production_work_id?{predecessorProductionWorkId:brandedId<"ProductionWorkId">(r.predecessor_production_work_id)}:{}),...(r.rework_remaining_required_quantity===null||r.rework_remaining_required_quantity===undefined?{}:{reworkRemainingRequiredQuantity:r.rework_remaining_required_quantity}),
...
export class PostgresPrepressTransaction implements PrepressTransaction {
  ...
  async listQueue(org:OrganizationId,request:PrepressQueuePageRequest):Promise<OperationalQueuePage<PrepressQueueItem>>{
    ...
    const ready=`l.production_requirement_state='configured'
        ...
        AND NOT EXISTS(SELECT 1 FROM v2_sales_line_production_requirements requirement WHERE requirement.organization_id=l.organization_id AND requirement.order_line_id=l.id AND NOT EXISTS(
          SELECT 1 FROM v2_current_artwork_assignments assignment JOIN v2_prepress_units completed ON completed.organization_id=assignment.organization_id AND completed.artwork_assignment_id=assignment.id AND completed.completed_at IS NOT NULL
    ...
    const [requirements,evidence,artwork,proofs,materials]=await Promise.all([
      ...
      this.client.query<CoverageEvidenceRow>(`SELECT r.order_line_id coverage_order_line_id,r.requirement_key,pu.*,cycle.reason rework_reason,cycle.predecessor_production_work_id,cycle.remaining_required_quantity rework_remaining_required_quantity,a.id artwork_assignment_id FROM v2_sales_line_production_requirements r
        LEFT JOIN v2_current_artwork_assignments a ON a.organization_id=r.organization_id AND a.order_line_id=r.order_line_id AND a.purpose='production' AND a.side IS NOT DISTINCT FROM r.side AND a.source_page_index IS NOT DISTINCT FROM r.source_page_index AND a.layer_key IS NOT DISTINCT FROM r.layer_key AND a.layer_order IS NOT DISTINCT FROM r.layer_order
  ...
  async coverage(org:OrganizationId,line:OrderLineId):Promise<OrderLinePrepressCoverage>{
    ...
    const evidence=await this.client.query<CoverageEvidenceRow>(`SELECT r.order_line_id coverage_order_line_id,r.requirement_key,pu.*,cycle.reason rework_reason,cycle.predecessor_production_work_id,cycle.remaining_required_quantity rework_remaining_required_quantity,a.id artwork_assignment_id FROM v2_sales_line_production_requirements r
      LEFT JOIN v2_current_artwork_assignments a ON a.organization_id=r.organization_id AND a.order_line_id=r.order_line_id AND a.purpose='production' AND a.side IS NOT DISTINCT FROM r.side AND a.source_page_index IS NOT DISTINCT FROM r.source_page_index AND a.layer_key IS NOT DISTINCT FROM r.layer_key AND a.layer_order IS NOT DISTINCT FROM r.layer_order
      LEFT JOIN v2_prepress_units pu ON pu.organization_id=a.organization_id AND pu.artwork_assignment_id=a.id
      LEFT JOIN v2_production_rework_cycles cycle ON cycle.organization_id=pu.organization_id AND cycle.id=pu.rework_cycle_id
  ...
  async eligibleProductionAssignment(org:OrganizationId,assignment:ArtworkAssignmentId){const r=await this.client.query<{valid:boolean}>(`SELECT EXISTS(
      SELECT 1 FROM v2_current_artwork_assignments a JOIN v2_route_instances ri ON ri.organization_id=a.organization_id AND ri.order_document_id=a.order_document_id AND ri.order_line_id=a.order_line_id
      ...
      SELECT 1 FROM v2_current_artwork_assignments a JOIN v2_production_rework_cycles cycle ON cycle.organization_id=a.organization_id AND cycle.order_line_id=a.order_line_id AND cycle.state='prepress_pending'
      JOIN v2_production_works predecessor ON predecessor.organization_id=cycle.organization_id AND predecessor.id=cycle.predecessor_production_work_id
      WHERE a.organization_id=$1 AND a.id=$2 AND a.purpose='production' AND a.side IS NOT DISTINCT FROM predecessor.side AND a.source_page_index IS NOT DISTINCT FROM predecessor.source_page_index AND a.layer_key IS NOT DISTINCT FROM predecessor.layer_key AND a.layer_order IS NOT DISTINCT FROM predecessor.layer_order
  ...
  async createOrGetUnit(input:Parameters<PrepressTransaction["createOrGetUnit"]>[0]){
    const rework=await this.client.query<{unit_id:string}>(`SELECT unit.id unit_id FROM v2_production_rework_cycles cycle JOIN v2_prepress_units unit ON unit.organization_id=cycle.organization_id AND unit.id=cycle.successor_prepress_unit_id JOIN v2_production_works predecessor ON predecessor.organization_id=cycle.organization_id AND predecessor.id=cycle.predecessor_production_work_id JOIN v2_artwork_assignments art ON art.organization_id=predecessor.organization_id AND art.id=$2 WHERE cycle.organization_id=$1 AND cycle.state='prepress_pending' AND unit.started_at IS NULL AND unit.completed_at IS NULL AND art.order_line_id=predecessor.order_line_id AND art.purpose='production' AND art.side IS NOT DISTINCT FROM predecessor.side AND art.source_page_index IS NOT DISTINCT FROM predecessor.source_page_index AND art.layer_key IS NOT DISTINCT FROM predecessor.layer_key AND art.layer_order IS NOT DISTINCT FROM predecessor.layer_order FOR UPDATE`,[input.organizationId,input.artworkAssignmentId]);
    if(rework.rows[0]){const updated=await this.client.query<UnitRow>(`UPDATE v2_prepress_units unit SET artwork_assignment_id=art.id,artwork_file_id=art.artwork_file_id FROM v2_artwork_assignments art WHERE unit.organization_id=$1 AND unit.id=$2 AND art.organization_id=$1 AND art.id=$3 RETURNING unit.*`,[input.organizationId,rework.rows[0].unit_id,input.artworkAssignmentId]);if(!updated.rows[0])throw Error("Rework Artwork preparation could not be updated.");return unit(updated.rows[0]);}
    const r=await this.client.query<UnitRow>(`INSERT INTO v2_prepress_units(id,organization_id,order_document_id,order_line_id,artwork_assignment_id,artwork_file_id,side,source_page_index,layer_key,layer_order,created_principal_kind,created_principal_subject,created_staff_actor_user_id)
      SELECT $1,a.organization_id,a.order_document_id,a.order_line_id,a.id,a.artwork_file_id,a.side,a.source_page_index,a.layer_key,a.layer_order,$3,$4,$5 FROM v2_current_artwork_assignments a WHERE a.organization_id=$2 AND a.id=$6 AND a.purpose='production'
      ON CONFLICT DO NOTHING RETURNING *`,[input.id,input.organizationId,input.principalKind,input.principalSubject,input.staffActorUserId??null,input.artworkAssignmentId]);
    if(r.rows[0]){await this.hooks?.afterUnit?.();return unit(r.rows[0]);}
    const prior=await this.client.query<UnitRow>("SELECT * FROM v2_prepress_units WHERE organization_id=$1 AND artwork_assignment_id=$2 AND rework_cycle_id IS NULL FOR UPDATE",[input.organizationId,input.artworkAssignmentId]);
