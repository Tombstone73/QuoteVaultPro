...
type PortalBootstrapResult = Readonly<{ portalAccessId: string; status: "pending" | "active"; deliveryState: "not_sent" | "pending" | "succeeded" | "uncertain" | "suppressed"; portalSetupUrl?: string }>;
...
export class PostgresTeamAccess {
  ...
  async bootstrapPortalAccess(actor: StaffPrincipal, organizationId: string, input: { customerId: string; contactId: string; permissionSetId: string }, context: Context, options: Readonly<{ captureM77fQaSetupUrl?: boolean }> = {}): Promise<PortalBootstrapResult> {
    ...
    if(captureM77fQaSetupUrl){
      ...
      try{await done.query("BEGIN");await done.query("UPDATE v2_portal_invitation_delivery_attempts SET delivery_state='succeeded',completed_at=now(),updated_at=now() WHERE organization_id=$1 AND portal_access_id=$2 AND operation_request_id=$3 AND delivery_state='pending'",[organizationId,portalAccessId,requestId]);await done.query("INSERT INTO v2_permission_audit_events(organization_id,event_type,actor_principal_kind,actor_principal_subject,staff_actor_user_id,portal_access_id,detail) VALUES($1,'portal_invitation_delivery_suppressed','staff',$2,$2,$3,$4::jsonb)",[organizationId,actor.userId,portalAccessId,JSON.stringify({businessRequestId:context.businessRequestId,deliveryMode:"m77f_qa_suppressed",providerCall:"not_attempted"})]);await this.requests.succeed(done,organizationId,requestId,{resourceType:"portal_access",resourceId:portalAccessId,resultJson:{portalAccessId,status:"pending",deliveryState:"suppressed"}});await done.query("COMMIT");}catch(error){await done.query("ROLLBACK");throw error;}finally{done.release();}
      return{portalAccessId,status:"pending",deliveryState:"suppressed",portalSetupUrl};
    }
