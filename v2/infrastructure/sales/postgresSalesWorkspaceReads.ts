...
const readOperational = async (client: PoolClient, organizationId: OrganizationId, orderIds: readonly string[]): Promise<ReadonlyMap<string, NonNullable<OrderListItem["operational"]>>> => {
  ...
  const result = await client.query<OperationalRow>(`
    SELECT d.id,d.contact_id,trim(concat_ws(' ',ct.first_name,ct.last_name)) contact_display_name,d.sales_representative_id,
      ...
      COALESCE((SELECT count(*) FROM v2_sales_line_production_requirements r LEFT JOIN v2_sales_line_workflow_exceptions e ON e.organization_id=r.organization_id AND e.order_line_id=r.order_line_id WHERE r.organization_id=d.organization_id AND r.document_id=d.id AND e.production_requirement IS DISTINCT FROM 'not_required' AND NOT EXISTS (SELECT 1 FROM v2_current_artwork_assignments a WHERE a.organization_id=r.organization_id AND a.order_line_id=r.order_line_id AND a.purpose='production' AND NOT EXISTS (SELECT 1 FROM v2_artwork_assignments successor WHERE successor.organization_id=a.organization_id AND successor.supersedes_artwork_assignment_id=a.id))),0)::text missing_artwork_requirement_count,
      ...
      COALESCE((SELECT count(*) FROM v2_production_works w WHERE w.organization_id=d.organization_id AND w.order_document_id=d.id AND v2_usable_production_good_quantity(w.organization_id,w.id)>=w.ordered_quantity),0)::text production_satisfied_count,
      ...
      COALESCE((SELECT count(*) FROM v2_sales_document_lines l LEFT JOIN pbv2_tree_versions v ON v.organization_id=l.organization_id AND v.product_id=l.product_id AND v.id=l.resolved_configuration->>'pricingConfigurationId' WHERE l.organization_id=d.organization_id AND l.document_id=d.id AND COALESCE(l.resolved_configuration#>>'{productFacts,workflowIntent}',v.tree_json#>>'{meta,general,workflowIntent}') IS DISTINCT FROM 'service_fee'),0)::text fulfillment_line_count,
      COALESCE((SELECT count(*) FROM v2_sales_document_lines l LEFT JOIN pbv2_tree_versions v ON v.organization_id=l.organization_id AND v.product_id=l.product_id AND v.id=l.resolved_configuration->>'pricingConfigurationId' WHERE l.organization_id=d.organization_id AND l.document_id=d.id AND COALESCE(l.resolved_configuration#>>'{productFacts,workflowIntent}',v.tree_json#>>'{meta,general,workflowIntent}') IS DISTINCT FROM 'service_fee' AND COALESCE((SELECT sum(fhl.quantity) FROM v2_fulfillment_handoff_lines fhl WHERE fhl.organization_id=l.organization_id AND fhl.order_document_id=l.document_id AND fhl.order_line_id=l.id),0)>=l.quantity),0)::text fulfillment_satisfied_count,
      ...
      COALESCE((SELECT sum(i.total_cents - COALESCE((SELECT sum(a.amount_cents) FROM v2_billing_payment_allocations a WHERE a.organization_id=i.organization_id AND a.invoice_id=i.id),0) + COALESCE((SELECT sum(e.amount_cents) FROM v2_billing_refund_allocation_evidence e WHERE e.organization_id=i.organization_id AND e.invoice_id=i.id),0)) FROM v2_billing_invoices i WHERE i.organization_id=d.organization_id AND i.sales_order_document_id=d.id AND i.invoice_state='issued'),0)::text billing_open_balance_cents,
  ...
  async listOrdersForWorkspace(organizationId: OrganizationId, request: SalesWorkspacePageRequest): Promise<SalesWorkspacePage<OrderListItem>> {
    return this.read(async (client) => {
      ...
      const nativeOperationalFilter = `AND ($11::text IS NULL OR $11::text='all'
        ...
        OR ($11::text='production' AND EXISTS (SELECT 1 FROM v2_production_works w WHERE w.organization_id=d.organization_id AND w.order_document_id=d.id AND v2_usable_production_good_quantity(w.organization_id,w.id)<w.ordered_quantity))
      ...
      // Summary queries do not use the keyset cursor or list limit. PostgreSQL
      // requires every positional parameter up to the highest reference to have
      // a resolved type, so they use a compact contiguous parameter set.
      const summaryParameters = [organizationId, search, lifecycle, dueFrom, dueTo, archive, operationalFilter];
      const nativeSummaryOperationalFilter = nativeOperationalFilter.replaceAll("$11", "$7");
      const legacySummaryOperationalFilter = legacyOperationalFilter.replaceAll("$11", "$7");
