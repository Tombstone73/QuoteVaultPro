...
const readOperational = async (client: PoolClient, organizationId: OrganizationId, orderIds: readonly string[]): Promise<ReadonlyMap<string, NonNullable<OrderListItem["operational"]>>> => {
  ...
  const result = await client.query<OperationalRow>(`
    SELECT d.id,d.contact_id,trim(concat_ws(' ',ct.first_name,ct.last_name)) contact_display_name,d.sales_representative_id,
      ...
      COALESCE((SELECT sum(i.total_cents - COALESCE((SELECT sum(a.amount_cents) FROM v2_billing_payment_allocations a WHERE a.organization_id=i.organization_id AND a.invoice_id=i.id),0) + COALESCE((SELECT sum(a.amount_cents) FROM v2_billing_refund_allocations a WHERE a.organization_id=i.organization_id AND a.invoice_id=i.id),0)) FROM v2_billing_invoices i WHERE i.organization_id=d.organization_id AND i.sales_order_document_id=d.id AND i.invoice_state='issued'),0)::text billing_open_balance_cents,
