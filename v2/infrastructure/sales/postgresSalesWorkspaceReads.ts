...
export class PostgresSalesWorkspaceReads implements SalesWorkspaceReadPort {
  ...
  async listOrdersForWorkspace(organizationId: OrganizationId, request: SalesWorkspacePageRequest): Promise<SalesWorkspacePage<OrderListItem>> {
    return this.read(async (client) => {
      const sort = sortFor(request.sort), direction = directionFor(sort), operator = operatorFor(sort), cursor = cursorFor(request);
      const search = request.search?.trim() || null, lifecycle = request.lifecycle ?? null, dueFrom = request.dueFrom || null, dueTo = request.dueTo || null, archive = request.archive ?? "active";
      const parameters = [organizationId, search, lifecycle, dueFrom, dueTo, archive, cursor?.updatedAt ?? null, cursor?.source ?? null, cursor?.id ?? null, limitFor(request.limit) + 1];
      type NativeRow = Common & { archived_at: Date | null; invoice_id: string | null; invoice_total_cents: string | null; route_count: string; purchase_order_number: string | null; line_count: string };
      ...
      const page = this.page(candidates, request), summary = summaryFrom(nativeSummary.rows[0], legacySummary.rows[0]);
      return { totalMatching: summary.itemCount, summary, ...(page.nextCursor ? { nextCursor: page.nextCursor } : {}), items: page.items.map(({ id: _id, customer_display_name: _customer, selling_total_cents: _total, requested_due_date: _due, updated_at: _updated, cursor_updated_at: _cursor, archived_at: _archivedAt, ...item }) => item) };
    });
  }
