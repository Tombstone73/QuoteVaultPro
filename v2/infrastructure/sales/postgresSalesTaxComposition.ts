...
export const composePostgresSalesTax = async (input: Readonly<{
  ...
  const jurisdictions = await input.client.query<{
    id: string; name: string; country_code: string; region_code: string; postal_code: string | null; rate_basis_points: number; active: boolean; home_business: boolean;
  }>("SELECT id,name,country_code,region_code,postal_code,rate_basis_points,active,home_business FROM v2_sales_tax_jurisdictions WHERE organization_id=$1", [input.organizationId]);
  const configured: TenantTaxJurisdiction[] = jurisdictions.rows.map((row) => ({
    jurisdictionId: row.id, name: row.name, receiptLocation: { country: row.country_code, region: row.region_code, ...(row.postal_code ? { postalCode: row.postal_code } : {}) }, rateBasisPoints: row.rate_basis_points, active: row.active, homeBusiness: row.home_business,
  }));
