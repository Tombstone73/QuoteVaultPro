...
export class PostgresFormulaDomainReads {
  ...
  async list(organizationId:string, input:Readonly<{ includeInactive?:boolean; query?:string; productId?:string; includeProductScoped?:boolean }>={}):Promise<readonly FormulaIdentity[]> {
    ...
    const result=await this.pool.query<CurrentFormulaRow>(`${selectCurrent} JOIN formula_revisions r ON r.id=f.current_revision_id AND r.organization_id=f.organization_id WHERE f.organization_id=$1 ${input.includeInactive?"":"AND f.status='active'"} ${productEligibility} ${queryEligibility} ORDER BY f.name,f.id`,values);
