...
class Transaction implements FormulaDomainTransaction {
  ...
  private async read(organizationId:string,formulaId:string):Promise<FormulaIdentity>{const found=await this.client.query<IdentityRow & RevisionRow>(`${select} JOIN formula_revisions r ON r.id=f.current_revision_id AND r.organization_id=f.organization_id WHERE f.organization_id=$1 AND f.id=$2`,[organizationId,formulaId]);if(!found.rows[0])throw new V2ApplicationError("NOT_FOUND","The tenant-scoped Formula was not found.");return identity(found.rows[0]!,found.rows[0]!);}
