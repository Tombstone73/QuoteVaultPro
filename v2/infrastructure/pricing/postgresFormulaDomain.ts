...
type IdentityRow = { id:string; organization_id:string; name:string; description:string|null; visibility:FormulaVisibility; scope_product_id:string|null; status:FormulaStatus; current_revision_id:string; created_at:Date; updated_at:Date; usage_count:string };
...
const identity = (header:IdentityRow, current:RevisionRow): FormulaIdentity => ({ formulaId:header.id,organizationId:header.organization_id,name:header.name,...(header.description?{description:header.description}:{}),visibility:header.visibility,...(header.scope_product_id?{scopeProductId:header.scope_product_id}:{}),status:header.status,currentRevisionId:header.current_revision_id,revision:revision(current),usageCount:Number(header.usage_count),createdAt:header.created_at.toISOString(),updatedAt:header.updated_at.toISOString() });
const select = `SELECT f.id,f.organization_id,f.name,f.description,f.visibility,f.scope_product_id,f.status,f.current_revision_id,f.created_at,f.updated_at,
...
const selectCurrent = `SELECT f.id,f.organization_id,f.name,f.description,f.visibility,f.scope_product_id,f.status,f.current_revision_id,f.created_at,f.updated_at,
...
export class PostgresFormulaDomainReads {
  ...
  async list(organizationId:string, input:Readonly<{ includeInactive?:boolean; query?:string; productId?:string }>={}):Promise<readonly FormulaIdentity[]> {
    ...
    const productEligibility = productId ? "AND (f.visibility='library' OR (f.visibility='product_scoped' AND f.scope_product_id=$2))" : "AND f.visibility='library'";
