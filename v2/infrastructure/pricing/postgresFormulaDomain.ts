...
type IdentityRow = { id:string; organization_id:string; name:string; description:string|null; visibility:FormulaVisibility; scope_product_id:string|null; scope_product_name:string|null; status:FormulaStatus; current_revision_id:string; created_at:Date; updated_at:Date; created_by_user_id:string|null; created_by_display_name:string|null; updated_by_user_id:string|null; updated_by_display_name:string|null; usage_count:string };
type RevisionRow = { id:string; organization_id:string; formula_id:string; revision_number:number; expression:string; declared_inputs:unknown; validation_evidence:unknown; created_at:Date; created_by_user_id:string|null; created_by_display_name:string|null };
type CurrentFormulaRow = IdentityRow & { formula_revision_id:string; formula_revision_organization_id:string; formula_revision_formula_id:string; formula_revision_number:number; formula_revision_expression:string; formula_revision_declared_inputs:unknown; formula_revision_validation_evidence:unknown; formula_revision_created_at:Date; formula_revision_created_by_user_id:string|null; formula_revision_created_by_display_name:string|null };
...
const identity = (header:IdentityRow, current:RevisionRow): FormulaIdentity => ({ formulaId:header.id,organizationId:header.organization_id,name:header.name,...(header.description?{description:header.description}:{}),visibility:header.visibility,...(header.scope_product_id?{scopeProductId:header.scope_product_id}:{}),...(header.scope_product_name?{scopeProductName:header.scope_product_name}:{}),status:header.status,currentRevisionId:header.current_revision_id,revision:revision(current),usageCount:Number(header.usage_count),createdAt:header.created_at.toISOString(),updatedAt:header.updated_at.toISOString(),...(header.created_by_user_id?{createdByUserId:header.created_by_user_id}:{}),...(header.created_by_display_name?{createdByDisplayName:header.created_by_display_name}:{}),...(header.updated_by_user_id?{updatedByUserId:header.updated_by_user_id}:{}),...(header.updated_by_display_name?{updatedByDisplayName:header.updated_by_display_name}:{}) });
const actorName = (alias:string) => `COALESCE(NULLIF(btrim(concat_ws(' ', ${alias}.first_name, ${alias}.last_name)), ''), ${alias}.email)`;
const select = `SELECT f.id,f.organization_id,f.name,f.description,f.visibility,f.scope_product_id,scoped_product.name scope_product_name,f.status,f.current_revision_id,f.created_at,f.updated_at,f.created_by_user_id,${actorName("created_by")} created_by_display_name,f.updated_by_user_id,${actorName("updated_by")} updated_by_display_name,
  ...
  FROM v2_formula_identities f LEFT JOIN products scoped_product ON scoped_product.id=f.scope_product_id AND scoped_product.organization_id=f.organization_id LEFT JOIN users created_by ON created_by.id=f.created_by_user_id LEFT JOIN users updated_by ON updated_by.id=f.updated_by_user_id`;
const selectCurrent = `SELECT f.id,f.organization_id,f.name,f.description,f.visibility,f.scope_product_id,scoped_product.name scope_product_name,f.status,f.current_revision_id,f.created_at,f.updated_at,f.created_by_user_id,${actorName("created_by")} created_by_display_name,f.updated_by_user_id,${actorName("updated_by")} updated_by_display_name,
  r.id formula_revision_id,r.organization_id formula_revision_organization_id,r.formula_id formula_revision_formula_id,r.revision_number formula_revision_number,r.expression formula_revision_expression,r.declared_inputs formula_revision_declared_inputs,r.validation_evidence formula_revision_validation_evidence,r.created_at formula_revision_created_at,r.created_by_user_id formula_revision_created_by_user_id,${actorName("revision_created_by")} formula_revision_created_by_display_name,
  (SELECT count(*)::text FROM v2_product_version_formula_revision_bindings b WHERE b.organization_id=f.organization_id AND b.formula_id=f.id) usage_count
  FROM v2_formula_identities f LEFT JOIN products scoped_product ON scoped_product.id=f.scope_product_id AND scoped_product.organization_id=f.organization_id LEFT JOIN users created_by ON created_by.id=f.created_by_user_id LEFT JOIN users updated_by ON updated_by.id=f.updated_by_user_id JOIN formula_revisions r ON r.id=f.current_revision_id AND r.organization_id=f.organization_id LEFT JOIN users revision_created_by ON revision_created_by.id=r.created_by_user_id`;
const current = `SELECT r.id,r.organization_id,r.formula_id,r.revision_number,r.expression,r.declared_inputs,r.validation_evidence,r.created_at,r.created_by_user_id,${actorName("revision_created_by")} created_by_display_name FROM formula_revisions r LEFT JOIN users revision_created_by ON revision_created_by.id=r.created_by_user_id`;

export class PostgresFormulaDomainReads {
  constructor(private readonly pool:Pool) {}
  async list(organizationId:string, input:Readonly<{ includeInactive?:boolean; query?:string; productId?:string; includeProductScoped?:boolean }>={}):Promise<readonly FormulaIdentity[]> {
    const query=(input.query??"").trim();
    const productId=typeof input.productId === "string" && input.productId.trim() ? input.productId.trim() : undefined;
    const values = query && productId ? [organizationId, productId, `%${query}%`] : productId ? [organizationId, productId] : query ? [organizationId, `%${query}%`] : [organizationId];
    // The default is the reusable Formula Library. Product Builder supplies a
    // Product id and gets only its eligible identities. Formula administration
    // is an explicitly authorized third read mode, never an implicit picker.
    const productEligibility = productId ? "AND (f.visibility='library' OR (f.visibility='product_scoped' AND f.scope_product_id=$2))" : input.includeProductScoped ? "" : "AND f.visibility='library'";
    const queryEligibility = query ? `AND f.name ILIKE $${productId ? 3 : 2}` : "";
    const result=await this.pool.query<CurrentFormulaRow>(`${selectCurrent} WHERE f.organization_id=$1 ${input.includeInactive?"":"AND f.status='active'"} ${productEligibility} ${queryEligibility} ORDER BY f.name,f.id`,values);
    return result.rows.map(row=>identity(row,currentRevision(row)));
  }
  async get(organizationId:string,formulaId:string):Promise<FormulaIdentity|null>{const result=await this.pool.query<CurrentFormulaRow>(`${selectCurrent} WHERE f.organization_id=$1 AND f.id=$2`,[organizationId,formulaId]);return result.rows[0]?identity(result.rows[0]!,currentRevision(result.rows[0]!)):null;}
...
class Transaction implements FormulaDomainTransaction {
  ...
  private async read(organizationId:string,formulaId:string):Promise<FormulaIdentity>{const found=await this.client.query<CurrentFormulaRow>(`${selectCurrent} WHERE f.organization_id=$1 AND f.id=$2`,[organizationId,formulaId]);if(!found.rows[0])throw new V2ApplicationError("NOT_FOUND","The tenant-scoped Formula was not found.");return identity(found.rows[0]!,currentRevision(found.rows[0]!));}
  async create(input:Parameters<FormulaDomainTransaction["create"]>[0]):Promise<FormulaIdentity>{
    ...
    if (input.visibility === "product_scoped") {
      const product = await this.client.query<{id:string}>("SELECT id FROM products WHERE organization_id=$1 AND id=$2 FOR KEY SHARE", [input.organizationId, input.scopeProductId]);
      if (!product.rows[0]) throw new V2ApplicationError("NOT_FOUND", "The tenant-scoped Product was not found.");
    }
    try { await this.client.query("INSERT INTO v2_formula_identities(id,organization_id,name,normalized_name,description,visibility,scope_product_id,status,created_by_user_id,updated_by_user_id) VALUES($1,$2,$3,$4,$5,$6,$7,'active',$8,$8)",[formulaId,input.organizationId,input.name,normalize(input.name),input.description??null,input.visibility,input.visibility === "product_scoped" ? input.scopeProductId ?? null : null,input.staffActorUserId??null]); } catch(error:any) { if(error?.code==="23505") throw new V2ApplicationError("CONFLICT","A Formula with that name already exists."); throw error; }
  ...
  private async updateVisibilityOrStatus(organizationId:string,formulaId:string,expected:string,column:"visibility"|"status",value:string,userId?:string):Promise<FormulaIdentity>{
    const assignment = column === "visibility" ? "visibility=$1,scope_product_id=NULL" : "status=$1";
    const updated=await this.client.query("UPDATE v2_formula_identities SET "+assignment+",updated_at=now(),updated_by_user_id=$2 WHERE organization_id=$3 AND id=$4 AND current_revision_id=$5 RETURNING id",[value,userId??null,organizationId,formulaId,expected]);if(!updated.rows[0]){const exists=await this.client.query("SELECT 1 FROM v2_formula_identities WHERE organization_id=$1 AND id=$2",[organizationId,formulaId]);throw new V2ApplicationError(exists.rows[0]?"STALE_STATE":"NOT_FOUND",exists.rows[0]?"The Formula changed elsewhere. Refresh and try again.":"The tenant-scoped Formula was not found.");}return this.read(organizationId,formulaId);}
