...
const constraints = [
  ...
  "v2_route_instance_steps_position_chk", "v2_route_instance_steps_kind_chk", "v2_route_instance_steps_instance_tenant_fk", "v2_route_instance_steps_production_destination_station_chk", "v2_route_instance_steps_production_destination_kind_chk",
] as const;
const indexes = [
  ...
  "v2_route_templates_org_active_idx", "v2_route_instances_org_state_idx", "v2_route_instance_steps_production_destination_idx",
] as const;
...
export async function checkV2RoutingPhysicalPostconditions(client: TransactionalClient): Promise<RoutingPhysicalPostcondition[]> {
  const [foundTables, foundConstraints, foundIndexes, columns] = await Promise.all([
    ...
    client.query<{ table_name: string; column_name: string; data_type: string; is_nullable: string }>("SELECT table_name,column_name,data_type,is_nullable FROM information_schema.columns WHERE table_schema='public' AND (table_name,column_name) IN (('v2_route_instances','current_step_id'),('v2_route_instances','source_template_revision'),('v2_route_instance_steps','step_kind'),('v2_route_instance_steps','source_template_step_id'),('v2_route_instance_steps','production_destination_station_key'))"),
  ]);
  ...
  return [
    ...
    { id: "column:v2_route_instance_steps.production_destination_station_key", passed: columnMap.get("v2_route_instance_steps.production_destination_station_key")?.data_type === "character varying" && columnMap.get("v2_route_instance_steps.production_destination_station_key")?.is_nullable === "YES", detail: "Production destination is an optional immutable Route Instance snapshot" },
  ];
}
