...
export class PostgresRoutingWorkspaceReads implements RoutingWorkspaceReadPort {
  ...
  async workspace(organizationId: string): Promise<RoutingWorkspaceRead> {
    ...
    try {
      ...
      const templateSteps = await client.query<{ id: string; route_template_id: string; position: number; step_kind: RouteStepKind; station_key: "flatbed" | "roll" | null }>(
        `SELECT s.id,s.route_template_id,s.position,s.step_kind,d.station_key
         FROM v2_route_template_steps s
         LEFT JOIN v2_route_template_production_destinations d ON d.organization_id=s.organization_id AND d.route_template_step_id=s.id
         WHERE s.organization_id=$1 ORDER BY s.route_template_id,s.position`, [organizationId]);
      ...
      return {
        templates: templates.rows.map((row) => ({ routeTemplateId: row.id, name: row.name, active: row.active, revision: row.revision, definitionFingerprint: row.definition_fingerprint, steps: templateSteps.rows.filter((step) => step.route_template_id === row.id).map(({ id, position, step_kind, station_key }) => ({ routeTemplateStepId: id, position, kind: step_kind, ...(station_key ? { productionDestination: station_key } : {}) })) })),
