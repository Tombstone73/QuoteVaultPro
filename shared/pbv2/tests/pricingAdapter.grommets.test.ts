...
import { pbv2ToMaterialEffects, pbv2ToPricingAddons } from "../pricingAdapter";
...
describe("pbv2/pricingAdapter (banner grommets proof)", () => {
  test("spacing=24 => total=6 extra=0 addOn=0", () => {
    ...
    const mats = pbv2ToMaterialEffects(
      tree,
      { explicitSelections: { grommetsEnabled: true, grommetSpacingIn: 24 } },
      { widthIn: 24, heightIn: 48 }
    );
    expect(mats.materials).toEqual([{ skuRef: "GROMMET_STD", uom: "ea", qty: 6, sourceNodeId: "price_grommets_overage" }]);
  });
  ...
  test("spacing=12 => total=10 extra=4 addOn=100", () => {
    ...
    expect(mats.materials).toEqual([{ skuRef: "GROMMET_STD", uom: "ea", qty: 10, sourceNodeId: "price_grommets_overage" }]);
  });
