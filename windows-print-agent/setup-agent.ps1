...
try {
  $heartbeat = Invoke-AgentApi $ApiBaseUrl $AgentToken '/api/local-bridge/heartbeat' @{ name = $env:COMPUTERNAME; agentVersion = 'installer-1.0.15' }
  if (-not (Test-SuccessStatus $heartbeat)) { throw 'PrintersHero returned an unsuccessful heartbeat status.' }
} catch {
