-- P7C repair: legacy Materials may lack an operational consumption unit, but
-- an explicit unit must match immutable Production consumption exactly.
...
BEGIN
  ...
  IF material_row.consumption_unit IS NOT NULL AND material_row.consumption_unit<>NEW.quantity_unit THEN RAISE EXCEPTION 'Production material consumption unit must match its configured Material unit' USING ERRCODE='23514'; END IF;
