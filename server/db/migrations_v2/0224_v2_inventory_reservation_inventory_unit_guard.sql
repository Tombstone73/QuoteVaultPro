-- P7D repair: a reservation is expressed in the frozen requirement's
-- inventory basis.  Consumption may use a distinct Material consumption unit.
-- Keep the legacy consumption-unit fallback only for Materials that have not
-- yet declared an inventory basis.
...
BEGIN
  ...
  IF material_row.inventory_unit IS NOT NULL AND material_row.inventory_unit<>NEW.quantity_unit THEN
    RAISE EXCEPTION 'Inventory reservation unit must match the frozen Material requirement inventory basis' USING ERRCODE='23514';
  END IF;
  IF material_row.inventory_unit IS NULL AND material_row.consumption_unit IS NOT NULL AND material_row.consumption_unit<>NEW.quantity_unit THEN
    RAISE EXCEPTION 'Inventory reservation unit must match the configured Material unit' USING ERRCODE='23514';
  END IF;
  RETURN NEW;
