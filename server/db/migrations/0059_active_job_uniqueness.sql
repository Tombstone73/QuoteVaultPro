-- Migration 0059: Active job uniqueness constraint
...
DECLARE
  ...
  _closed INT := 0;
BEGIN
  -- For each (organization_id, line_item_id) that has more than 1 active job,
  -- close all but the most recently updated one.
  FOR _dup IN
    SELECT id
    FROM (
      SELECT id,
  ...
  END LOOP;
  IF _closed > 0 THEN
    RAISE NOTICE '[0059] Closed % duplicate active production jobs', _closed;
  END IF;
END $$;
