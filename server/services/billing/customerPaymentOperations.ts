...
export async function recordCustomerPayment(input: Input) {
  ...
  const outcome = await db.transaction(async (tx) => {
    ...
    if (existing) return { batch: existing, payments: await tx.select().from(payments).where(eq(payments.customerPaymentBatchId, existing.id)), newlyPaid: [], reused: true };
    ...
    const [batch] = await tx.insert(customerPaymentBatches).values({ organizationId: input.organizationId, customerId, amountCents: input.amountCents, method: input.method, allocationMode: input.allocationMode, idempotencyKey: input.idempotencyKey, reference: input.reference?.trim() || null, notes: input.notes?.trim() || null, appliedAt: input.appliedAt, createdByUserId: input.actorUserId } as any).returning();
    const byId = new Map(rows.map((x) => [x.invoice.id, x])); const created: any[] = []; const newlyPaid: any[] = [];
    for (const allocation of result) { const row = byId.get(allocation.invoiceId)!; const [payment] = await tx.insert(payments).values({ organizationId: input.organizationId, invoiceId: row.invoice.id, customerPaymentBatchId: batch.id, provider: "manual", providerIdempotencyKey: `customer-batch:${batch.id}:invoice:${row.invoice.id}`, status: "succeeded", currency: row.invoice.currency || "USD", amount: cents(allocation.amountCents), amountCents: allocation.amountCents, method: input.method, notes: input.notes?.trim() || null, note: input.notes?.trim() || null, paidAt: input.appliedAt, createdByUserId: input.actorUserId, syncStatus: "skipped", syncError: heldQuickBooksReason, metadata: { source: "customer_payment_batch", customerPaymentBatchId: batch.id, allocationMode: input.allocationMode, ...(input.reference ? { reference: input.reference } : {}) } } as any).returning();
