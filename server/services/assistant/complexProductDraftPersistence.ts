...
export async function createComplexProductDraft(input: { organizationId: string; proposalId: string; fingerprint: string; actorUserId: string | null; idempotencyKey: string }) {
  return db.transaction(async (tx) => {
    ...
    const now = new Date(); const treeJson = buildCanonicalComplexProductTree(specification); const perPiece = pricingUnitForComplexProductMatrix(specification.pricing) === "per_piece"; const [product] = await tx.insert(products).values({ organizationId: input.organizationId, name: specification.name, description: specification.description, category: specification.category, pricingMode: perPiece ? "quantity" : "area", pricingEngine: "pricingProfile", pricingProfileKey: perPiece ? "qty_only" : "default", requiresProductionJob: true, requiresProofApproval: false, isTaxable: specification.taxable, isService: false, isActive: false, optionTreeJson: null, pbv2ActiveTreeVersionId: null }).returning();
