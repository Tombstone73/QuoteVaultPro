  ...
  private async createAiFirstTurn(input: {
    ...
    const plannerResult = await this.intentPlanner.plan({
      ...
      user: JSON.stringify({
        ...
        contract: "Return only semantic planner fields: operation, capabilityId when a registered capability is selected, confidence when useful, target.kind, requiresClarification, and clarificationQuestion. The server derives version, domain, mode, reasonCode, target entity ID, trusted context, authorization, and execution metadata. For general help use general_conversation with target kind none. For a capability question select assistant_capabilities with operation explain and target kind none. For a detailed new product select canonical_product_intent_compiler with operation create and target kind new_entity.",
      }),
    });
    if (!plannerResult.ok) {
      return this.persistAiFirstResponse(input, {
