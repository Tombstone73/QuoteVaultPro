...
export function createConfigurableProductDraftCommandDefinition(): AssistantCommandDefinition<ConfigurableProductDraftInput, unknown, z.infer<typeof resultSchema>> {
  ...
  return { name: configurableProductDraftCommandName, version: "v1", domain: "products", mode: "write", description: "Create one persisted configurable inactive PBV2 DRAFT product.", risk: "high", requiredCapability: "assistant.products.create_inactive_draft", allowedRoles: ["owner", "admin"], inputSchema: configurableProductDraftInputSchema, previewSchema: z.unknown(), resultSchema, maxAffectedRecords: 1, bulkAllowed: false, confirmationRequired: true, reauthenticationRequired: true, confirmationExpiresInMs: 10 * 60_000, idempotencyPolicy: "server_generated_with_request_hash", recordFingerprintStrategy: "stable_field_hash", transactionPolicy: "all_or_nothing", partialFailurePolicy: "fail_fast", auditCategory: "assistant_configurable_product_draft", undoSupport: "none", abandonmentPolicy: "none", testOnly: false, devEnabled: true, mainEnabled: true, adapter };
}
