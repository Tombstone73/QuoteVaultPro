  ...
  return {
    ...
    async buildPreview({ scope, arguments: raw }) {
      ...
      try {
        ...
        if (!validation.valid) throw new ExecutionPlanError("PLAN_STALE", validation.summary);
        return { arguments: input, preview: preview(validation.proposal) };
      } catch (error) {
        if (error instanceof ExecutionPlanError) throw error;
        const known = error && typeof error === "object" && "code" in error && typeof (error as { code?: unknown }).code === "string" && error instanceof Error;
        if (known) {
          const code = (error as Error & { code: string }).code;
          throw new ExecutionPlanError(code === "NO_PRODUCT_LIFECYCLE_CHANGES" ? "PLAN_NO_CHANGES" : "PLAN_PREPARATION_FAILED", error.message);
        }
        throw error;
      }
    },
    ...
    async execute({ plan, scope }): Promise<ExecutionCommandResult> {
      ...
      const operationSummary = result.canonicalOperationReference === "products.update_option_configuration.v1"
