...
import {
  ...
  assistantOrderSearchInputSchema,
  assistantOrderSearchResultSchema,
...
const toolMetadata = {
  ...
  "orders.search": {
    description: "Search, filter, and sort the current tenant's Orders using the same canonical query as the Orders UI. Use for recent Orders and general Order lists; use orders.get_summary for one exact Order.",
    requiredPermission: "internal_staff", requiredContext: ["trusted_actor"] as const,
    inputSchema: assistantOrderSearchInputSchema,
    providerInputSchema: { type: "object", additionalProperties: false, properties: { query: { type: "string" }, status: { type: "string" }, priority: { type: "string" }, customerId: { type: "string" }, sort: { enum: ["newest", "order_number_asc", "order_number_desc", "customer_asc", "customer_desc", "total_asc", "total_desc", "due_date_asc", "due_date_desc", "status_asc", "status_desc", "priority_asc", "priority_desc"] }, limit: { type: "integer", minimum: 1, maximum: 20 } } },
    resultSchema: assistantOrderSearchResultSchema, maxResults: 20, timeoutMs: 5_000,
    dataClassification: "internal", sourceLinkBehavior: "optional", auditCategory: "assistant_order_search", modelSummarizationAllowed: true,
  },
  "products.get_summary": {
    description: "Return a reduced catalog, pricing-engine sheet-layout configuration, and production-routing summary for one product. This is not a calculated customer price. Product-level Allow Rotation / Mixed Sheet Layout is distinct from customer-selectable PBV2 options.",
  ...
  "products.get_pricing": {
    description: "For a standalone named-product request, return authoritative Product pricing-engine configuration and semantic PBV2 pricing for one tenant-scoped product. Use search.global only for genuinely unresolved broad discovery. Product-level Allow Rotation / Mixed Sheet Layout is reported separately from customer-selectable options. This read never changes a product, quote, or order.",
