...
function compare(left: unknown, comparison: z.infer<typeof operationSchema> & { op: "filter" }): boolean {
  ...
  if (typeof left !== "number" || typeof comparison.value !== "number") return false;
  if (comparison.comparison === "greater_than") return left > comparison.value;
  if (comparison.comparison === "greater_than_or_equal") return left >= comparison.value;
  if (comparison.comparison === "less_than") return left < comparison.value;
