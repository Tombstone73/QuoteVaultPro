...
type DirectIntake = { kind: "direct"; customerId: string; customerName: string; contactId: string | null; dueDate: string | null; lines: DirectLine[] };
...
export class OrderIntakeService {
  async respond(input: { organizationId: string; userId: string; conversationId: string; message: string }): Promise<{ handled: boolean; response: string; cards: AssistantOrderCard[] }> {
    ...
    const session = await this.createSession(input, { kind: "direct", customerId: customer!.id, customerName: customer!.companyName ?? "Customer", contactId: contact?.id ?? null, dueDate: parseDueDate(input.message), lines: [{ productId: product!.id, productName: product!.name ?? "Product", quantity: quantity!, width: dimensions!.width, height: dimensions!.height }] });
  ...
  async revalidateCreateProposal(input: { organizationId: string; orderIntakeSessionId: string; expectedProposalFingerprint: string }): Promise<{ valid: true; proposal: any } | { valid: false; code: string; summary: string }> {
    ...
    const [org, customer] = await Promise.all([
      ...
      db.select().from(customers).where(and(eq(customers.id, intake.customerId), eq(customers.organizationId, input.organizationId))).limit(1).then((rows) => rows[0]),
    ]);
    if (!org || !customer) return { valid: false, code: "ORDER_CUSTOMER_NOT_FOUND", summary: "The customer is no longer available." };
  ...
  private async createDirect(organizationId: string, actorUserId: string, intake: DirectIntake, proposal: any) {
    ...
    const [org, customer] = await Promise.all([db.select().from(organizations).where(eq(organizations.id, organizationId)).limit(1).then((rows) => rows[0]), db.select().from(customers).where(and(eq(customers.id, intake.customerId), eq(customers.organizationId, organizationId))).limit(1).then((rows) => rows[0])]);
    if (!org || !customer) throw new AssistantOrderIntakeError("ORDER_CUSTOMER_NOT_FOUND", "The customer is unavailable.");
