...
export const capabilityInventoryParityValues = [
  ...
  "ai_integration_pending",
  "deliberately_ai_ineligible",
  "partial_or_indirect",
  "unknown",
] as const;
...
const sharedCanonicalOperationalCommands = new Set([
...
function canonicalOperationSource(command: (typeof assistantProductionCommandAllowlist)[number]): CapabilityInventorySource | null {
  if (command.startsWith("production.")) return source("server/services/canonicalProductionOperations.ts", "CanonicalProductionOperations");
  if (command.startsWith("fulfillment.")) return source("server/services/fulfillment/canonicalFulfillmentOperations.ts", "CanonicalFulfillmentOperations");
  if (command.startsWith("customers.") || command.startsWith("contacts.")) return source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations");
  if (command.startsWith("billing.")) return source("server/services/billing/canonicalInvoiceOperations.ts", "CanonicalInvoiceOperations");
  if (command.startsWith("payments.")) return source("server/services/billing/canonicalPaymentOperations.ts", "CanonicalPaymentOperations");
  return null;
}
...
const commandInventory: CapabilityInventoryItem[] = assistantProductionCommandAllowlist.map((command) => {
  ...
  return {
    ...
    canonicalCandidate: sharedCanonicalPricingCommands.has(command)
      ...
      : canonicalOperationSource(command) ?? definition,
    ...
    parity: sharedCanonicalPricingCommands.has(command) || sharedCanonicalOperationalCommands.has(command) ? "shared_canonical_today" : "partial_or_indirect",
    migrationNotes: sharedCanonicalPricingCommands.has(command)
      ...
      : sharedCanonicalOperationalCommands.has(command)
        ? "Confirmed Assistant adapter delegates to the same tenant-scoped canonical operational boundary used by the UI. Existing authority, GO, stale revalidation, audit, and downstream state services remain authoritative."
...
const uiOnlyInventory: CapabilityInventoryItem[] = [
  ...
  { id: "ui.production.manage_jobs", domain: "production", operation: "transition line-item workflow, update jobs, and add job notes", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/jobs.routes.ts", undefined, "POST /api/line-items/:lineItemId/workflow-transition"), resultContractSource: source("server/services/canonicalProductionOperations.ts", "CanonicalProductionOperations"), permissionSource: source("server/routes/jobs.routes.ts", "isAuthenticated/tenantContext"), permissionRequirement: "authenticated_tenant_user (UI); reviewed GO command subset (AI)", tenantScopingSource: source("server/services/canonicalProductionOperations.ts", "CanonicalProductionOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/canonicalProductionOperations.ts", "CanonicalProductionOperations"), canonicalCandidate: source("server/services/canonicalProductionOperations.ts", "CanonicalProductionOperations"), routes: [source("server/routes/productionJobs.routes.ts", undefined, "POST /api/production/jobs/:jobId/start")], audit: source("server/routes/productionJobs.routes.ts", "appendEvent/audit"), parity: "partial_or_indirect", migrationNotes: "Production start and the reviewed intake, return-to-Prepress, and note commands share canonical boundaries. Completion, reopening, assignment, and combined-run management remain UI-only." },
  { id: "ui.fulfillment.manage_orders_shipments_pickups", domain: "fulfillment", operation: "ready/unready fulfillment, shipments, pickup tickets, and shipping state", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/fulfillment.routes.ts", undefined, "fulfillment routes"), resultContractSource: source("server/services/fulfillment/canonicalFulfillmentOperations.ts", "CanonicalFulfillmentOperations"), permissionSource: source("server/routes/fulfillment.routes.ts", "isAuthenticated/tenantContext"), permissionRequirement: "authenticated_tenant_user (UI); reviewed GO command subset (AI)", tenantScopingSource: source("server/services/fulfillment/canonicalFulfillmentOperations.ts", "CanonicalFulfillmentOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/fulfillment/service.ts", "FulfillmentService"), canonicalCandidate: source("server/services/fulfillment/canonicalFulfillmentOperations.ts", "CanonicalFulfillmentOperations"), routes: [source("server/routes/fulfillment.routes.ts", undefined, "POST/PATCH /api/fulfillment")], audit: source("server/services/fulfillment/service.ts", "FulfillmentService"), parity: "partial_or_indirect", migrationNotes: "Routes and reviewed Assistant shipment/pickup/note commands share FulfillmentService through the canonical facade. Ready/unready, checklist, void, pickup-ready, and pickup-complete remain UI-only." },
  { id: "ui.invoicing.manage_invoice", domain: "invoicing", operation: "create, edit, bill, send, and remind invoices", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/mvpInvoicing.routes.ts", undefined, "invoice routes"), resultContractSource: source("server/services/billing/canonicalInvoiceOperations.ts", "CanonicalInvoiceOperations"), permissionSource: source("server/routes/mvpInvoicing.routes.ts", "isAuthenticated/tenantContext"), permissionRequirement: "route_specific_or_unknown", tenantScopingSource: source("server/services/billing/canonicalInvoiceOperations.ts", "CanonicalInvoiceOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/billing/canonicalInvoiceOperations.ts", "CanonicalInvoiceOperations"), canonicalCandidate: source("server/services/billing/canonicalInvoiceOperations.ts", "CanonicalInvoiceOperations"), routes: [source("server/routes/mvpInvoicing.routes.ts", undefined, "POST /api/orders/:orderId/invoices; POST /api/invoices/:id/bill; POST /api/invoices/:id/mark-sent")], audit: source("server/services/billing/canonicalInvoiceOperations.ts", "CanonicalInvoiceOperations"), parity: "partial_or_indirect", migrationNotes: "Draft creation, safe draft updates, manual sent marker, internal notes, and finalize share canonical operations. Actual email delivery, broad editor fields, reminders, and QuickBooks actions remain UI-only." },
  { id: "ui.payments.record_and_manage", domain: "payments", operation: "record, void, synchronize, refund, capture, and configure payments", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/mvpInvoicing.routes.ts", undefined, "POST /api/invoices/:id/payments/manual"), resultContractSource: source("server/services/billing/canonicalPaymentOperations.ts", "CanonicalPaymentOperations"), permissionSource: source("server/routes/mvpInvoicing.routes.ts", "isAuthenticated/tenantContext"), permissionRequirement: "authenticated_tenant_user (UI); reviewed GO command subset (AI)", tenantScopingSource: source("server/services/billing/canonicalPaymentOperations.ts", "CanonicalPaymentOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/invoicesService.ts", "recordManualPaymentCanonical"), canonicalCandidate: source("server/services/billing/canonicalPaymentOperations.ts", "CanonicalPaymentOperations"), routes: [source("server/routes/mvpInvoicing.routes.ts", undefined, "POST /api/invoices/:id/payments/manual")], audit: source("server/invoicesService.ts", "recordManualPaymentCanonical"), parity: "partial_or_indirect", migrationNotes: "Primary UI and AI manual payment recording share the locked exact-cent idempotent operation. Soft void, processor flows, sync, and provider administration remain UI-only; refunds remain disabled." },
  { id: "ui.customers.manage_customer", domain: "customers", operation: "create, edit, merge, import, and manage customer production references", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/customers.routes.ts", undefined, "POST/PATCH /api/customers"), resultContractSource: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), permissionSource: source("server/routes/customers.routes.ts", "isAuthenticated/tenantContext/isAdmin"), permissionRequirement: "route_specific_or_unknown", tenantScopingSource: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), canonicalCandidate: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), routes: [source("server/routes/customers.routes.ts", undefined, "POST/PATCH /api/customers")], audit: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), parity: "partial_or_indirect", migrationNotes: "Core create/profile/commercial updates share canonical Customer operations. Merge, import, archive, notes, credit transactions, and production-folder references remain UI-only." },
  { id: "ui.contacts.manage_contact_relationships", domain: "contacts", operation: "create, edit, link, set primary, and manage contact status", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/customerRelations.routes.ts", undefined, "customer contact routes"), resultContractSource: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), permissionSource: source("server/routes/customerRelations.routes.ts", "isAuthenticated/tenantContext"), permissionRequirement: "authenticated_tenant_user", tenantScopingSource: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), canonicalCandidate: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), routes: [source("server/routes/customerRelations.routes.ts", undefined, "POST /api/contacts; POST /api/customers/:customerId/contacts; PATCH /api/customer-contacts/:id")], audit: source("server/services/customers/canonicalCustomerContactOperations.ts", "CanonicalCustomerContactOperations"), parity: "partial_or_indirect", migrationNotes: "Core linked contact create/update share canonical Contact operations. Link/unlink, relationship status/primary changes, archive, and notes remain UI-only." },
  ...
  { id: "ui.products.activate_published_configuration", domain: "products", operation: "publish a reviewed PBV2 DRAFT and activate/deactivate one Product", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: "products.update_existing_product", inputSchemaSource: source("server/services/assistant/existingProductEditContract.ts", "existingProductEditOperationListSchema"), resultContractSource: source("server/services/products/canonicalProductPublishOperations.ts", "CanonicalProductPublishOperations"), permissionSource: source(productRoute, "isAdmin"), permissionRequirement: "admin", tenantScopingSource: source("server/services/products/canonicalProductPublishOperations.ts", "CanonicalProductPublishOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/products/canonicalProductPublishOperations.ts", "CanonicalProductPublishOperations"), canonicalCandidate: source("server/services/products/canonicalProductLifecycleOperations.ts", "CanonicalProductLifecycleOperations"), routes: [source(productRoute, undefined, "PATCH /api/products/:id"), source(productRoute, undefined, "POST /api/pbv2/tree-versions/:id/publish")], audit: source("server/services/products/canonicalProductPublishOperations.ts", "product_configuration_published"), parity: "shared_canonical_today", migrationNotes: "Product Editor and the reviewed existing-Product GO command share products.publish_configuration.v1 and products.update_lifecycle.v1. Publish warnings require explicit confirmation; advanced override administration remains AI-ineligible." },
  ...
  { id: "ui.prepress.manage_sessions_and_files", domain: "prepress", operation: "manage prepress sessions, files, routing, and material overrides", mode: "mutation", uiExposure: "page_and_route", aiExposure: "command_plan_only", readToolName: null, commandName: null, inputSchemaSource: source("server/routes/prepress.routes.ts", "registerPrepressQueueRoutes"), resultContractSource: source("server/services/canonicalPrepressOperations.ts", "CanonicalPrepressOperations"), permissionSource: source("server/routes/prepress.routes.ts", "tenantContext"), permissionRequirement: "route_specific_or_unknown", tenantScopingSource: source("server/services/canonicalPrepressOperations.ts", "CanonicalPrepressOperations"), confirmation: "go_required", idempotency: "server_generated_with_request_hash", lifecycleValidationSource: source("server/services/productionReturnToPrepressService.ts", "returnProductionJobsToPrepressInTransaction"), canonicalCandidate: source("server/services/canonicalPrepressOperations.ts", "CanonicalPrepressOperations"), routes: [source("server/routes/prepress.routes.ts", undefined, "POST /api/prepress/session/start")], audit: source("server/services/productionReturnToPrepressService.ts", "returnProductionJobsToPrepressInTransaction"), parity: "partial_or_indirect", migrationNotes: "The reviewed Assistant return-to-Prepress command shares the active-run-safe return operation. Session/file/artwork, destination, and material override editing remain UI-only." },
...
export const productParityInventory = Object.freeze([
  ...
  { id: "products.materials", productArea: "Primary material", uiSource: source(productRoute, undefined, "PATCH /api/products/:id"), normalOperatorSource: source("server/services/products/canonicalProductMaterialOperations.ts", "CanonicalProductMaterialOperations"), classification: "shared_canonical_today", notes: "Product Editor, confirmed existing-Product edits, and canonical new-draft proposals share products.update_material_configuration.v1 tenant/active validation. Material links and PBV2 consumption/overrides remain separate." },
  ...
  { id: "products.prepress_routing", productArea: "Advanced Prepress and production routing", uiSource: source(productRoute, undefined, "PATCH /api/products/:id"), normalOperatorSource: "none", classification: "ai_integration_pending", notes: "Reviewed Product routing fields remain UI-only pending typed AI integration." },
  { id: "products.lifecycle", productArea: "Active/inactive and PBV2 publish", uiSource: source(productRoute, undefined, "PATCH /api/products/:id; POST /api/pbv2/tree-versions/:id/publish"), normalOperatorSource: source("server/services/products/canonicalProductPublishOperations.ts", "CanonicalProductPublishOperations"), classification: "shared_canonical_today", notes: "UI and AI share validated publication and Product lifecycle operations; DRAFT-only activation transparently proposes publish then activation behind GO." },
  { id: "products.pricing_engine_rotation", productArea: "Pricing Engine rotation / mixed sheet layout", uiSource: source(productRoute, undefined, "PATCH /api/products/:id"), normalOperatorSource: source("server/services/products/canonicalProductPricingEngineConfigurationOperations.ts", "CanonicalProductPricingEngineConfigurationOperations"), classification: "shared_canonical_today", notes: "UI and AI share the typed allowRotation operation without exposing raw pricingProfileConfig JSON." },
  { id: "products.pricing_engine_advanced", productArea: "Pricing formula/profile/library and coupled sheet metadata", uiSource: source(productRoute, undefined, "PATCH /api/products/:id"), normalOperatorSource: "none", classification: "ai_integration_pending", notes: "These established UI controls have coupled Product/PBV2 semantics and remain intentionally unwired pending a typed server-owned operation." },
  { id: "products.delete_advanced_override", productArea: "Delete and advanced PBV2 override administration", uiSource: source(productRoute, undefined, "Product admin routes"), normalOperatorSource: "none", classification: "deliberately_ai_ineligible", notes: "Destructive Product deletion and temporary advanced override/bypass controls remain intentionally outside AI eligibility." },
