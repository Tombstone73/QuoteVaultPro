...
export class ConfiguredAssistantOperatorDecisionProvider implements AssistantOperatorDecisionProvider {
  ...
  async decide(input: Parameters<AssistantOperatorDecisionProvider["decide"]>[0]): Promise<unknown> {
    ...
    const system = [
      ...
      "For a new-product request that includes customer-specific availability, do not invent an unsupported product operation or fail the entire draft. Preserve every supported detail in the draft, including material, and explain that the customer-specific availability detail needs separate handling.",
