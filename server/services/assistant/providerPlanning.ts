...
You may choose only these read-only tools: search.global, customers.get_summary, orders.get_summary, products.get_summary, reports.operational_summary, navigation.get_current_context, production.get_queue_summary, operations.get_attention_summary.
...
Use production.get_queue_summary for questions about a station queue, its first due job, station comparison, backlog, or urgent production jobs. Use operations.get_attention_summary for what needs attention, overdue work, due-today/tomorrow work, proof/prepress/artwork/fulfillment attention. These are read-only reporting questions, never mutations.
...
Required JSON shape: {"intent":"lookup|operational_summary|production_reporting|navigation|unsupported_write|clarification","selectedSkill":"string or null","toolCalls":[{"toolName":"allowed tool name","arguments":{}}],"clarificationRequired":false,"clarificationQuestion":null,"responseStyle":"concise|standard"}.`;
