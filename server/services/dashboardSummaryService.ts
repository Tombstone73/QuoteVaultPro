...
export async function getDashboardSummary(organizationId: string): Promise<DashboardSummary> {
  ...
  try {
    ...
    // Ambiguous without a canonical schema mapping in current stack.
    summary.ordersPipeline.scheduled = null;
    summary.ordersPipeline.readyForPickup = null;
    summary.ordersPipeline.slaRisk = null;
    devWarn("ordersPipeline.scheduled is null (no canonical scheduled status)");
    devWarn("ordersPipeline.readyForPickup is null (no canonical ready_for_pickup status)");
  ...
  try {
    ...
    // Step definitions are not canonicalized for these KPIs in schema.
    summary.productionJobs.printing = null;
    summary.productionJobs.finishing = null;
    summary.productionJobs.qaInspection = null;
    devWarn("productionJobs.printing/finishing/qaInspection are null (no canonical step mapping)");
  } catch (error) {
