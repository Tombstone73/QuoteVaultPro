...
function promptForCompilation(input: ProductIntentCompilerInput): { system: string; user: string } {
  ...
  const user = continuation
    ? {
      ...
      semanticOperationContract: { operations: "Use add_option_group before its add_option_value operations, then set_option_default with optionGroup and value display labels. Use set_pricing_basis with per_piece or per_square_foot; set_matrix_rate with option group, value, and integer cents; set_category with the user's category phrase; remove_option_value or remove_option_group using a business option-group label; set_product_name with the explicitly requested name; and set_proof_requirement when the user explicitly changes proof approval. If counted grommets cannot be represented by the available option model, use record_unsupported_detail with grommet_quantity while retaining every valid group, value, and default. Do not use canonical keys, paths, patch operations, revisions, IDs, audit data, lifecycle data, or PBV2 structures." },
