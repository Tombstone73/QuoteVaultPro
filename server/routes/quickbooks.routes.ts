...
export function registerQuickBooksRoutes(
   ...
   *
   * Query params:
   *   scope=open_ar|historical|all_unsynced; page=1..n; pageSize=25|50|100|200
  ...
  app.get('/api/integrations/quickbooks/import-preview/invoices', isAuthenticated, tenantContext, isAdminOrOwner, async (req: any, res) => {
    try {
      ...
      if (!['open_ar', 'historical', 'all_unsynced'].includes(scope)) return res.status(400).json({ success: false, error: 'scope must be one of: open_ar, historical, all_unsynced' });
      if (!Number.isInteger(page) || page < 1) return res.status(400).json({ success: false, error: 'page must be a positive integer' });
      if (![25, 50, 100, 200].includes(pageSize)) return res.status(400).json({ success: false, error: 'pageSize must be one of: 25, 50, 100, 200' });
      const preview = await quickbooksService.fetchQBInvoicePreviewPage({ organizationId, scope: scope as quickbooksService.QBInvoicePreviewScope, page, pageSize, includeReferenceDebug });
  ...
  app.post('/api/integrations/quickbooks/import/invoices', isAuthenticated, tenantContext, isAdminOrOwner, async (req: any, res) => {
    try {
      ...
      if (Array.isArray(invoicesArray)) {
        ...
        if (invoicesArray.length > 100) return res.status(400).json({ success: false, error: 'A QuickBooks invoice import batch may contain at most 100 entries.' });
        ...
        if (rawIds.length > 100) return res.status(400).json({ success: false, error: 'A QuickBooks invoice import batch may contain at most 100 entries.' });
        ids = rawIds;
      ...
      if (ids.length === 0) {
        ...
        return res.json({ success: true, data: { created: 0, updated: 0, skipped: invoicesArray?.length ?? 0, excluded: 0, failed: 0, importedOpenAr: 0, importedHistorical: 0, numberingConflicts: 0, errors: [] } });
      }
      const uniqueIds = Array.from(new Set(ids.map((id) => String(id).trim()).filter(Boolean)));
      if (uniqueIds.length > 100) return res.status(400).json({ success: false, error: 'A QuickBooks invoice import batch may contain at most 100 unique invoice IDs.' });
