...
export function registerAccountsReceivableReportRoutes(app: Express, middleware: { isAuthenticated: any; tenantContext: any }): void {
  app.get('/api/reports/accounts-receivable', middleware.isAuthenticated, middleware.tenantContext, async (req: any, res) => {
    try {
      ...
      const report = await getAccountsReceivableReport({ organizationId, page: Number(req.query.page) || 1, pageSize: Number(req.query.pageSize) || 50, sortBy: normalizeAccountsReceivableSort(queryString(req.query.sortBy)), sortDir: queryString(req.query.sortDir) === 'desc' ? 'desc' : 'asc', filters: { customerId: queryString(req.query.customerId), agingBucket: queryString(req.query.agingBucket) as any, invoiceStatus: queryString(req.query.invoiceStatus), sendStatus: queryString(req.query.sendStatus) as any, jobStatus: queryString(req.query.jobStatus) as any, overdueOnly: String(req.query.overdue || '').toLowerCase() === 'true' } });
