...
describe("assistant deferred-production order commands", () => {
  ...
  it("builds a confirmation preview that explicitly excludes production and financial side effects", async () => {
    ...
    expect(prepared.preview.orderCreate).toMatchObject({ customer: { id: "customer_1", name: "Acme" }, orderStatus: "new", productionDeferred: true, totalCents: 12_500, lines: [{ productName: "Banner", quantity: 2, dimensions: { widthIn: 24, heightIn: 36, unit: "in" }, selections: [{ groupLabel: "Sides", valueLabel: "Single Sided" }], unitPriceCents: 6_250 }] });
