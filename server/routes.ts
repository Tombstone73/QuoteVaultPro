...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // =============================
  // Enterprise Import Jobs (Validate â†’ Apply)
  // =============================
              ...
              switch (row.actionType) {
                ...
                case 'status_pill_changed':
                  if (row.fromStatus && row.toStatus) {
                    message = `Status changed: ${row.fromStatus} â†’ ${row.toStatus}`;
                ...
                case 'priority_change':
                  if (meta.oldValue && meta.newValue) {
                    message = `Priority changed: ${meta.oldValue} â†’ ${meta.newValue}`;
                ...
                case 'due_date_change':
                  if (meta.oldValue && meta.newValue) {
                    message = `Due date changed: ${meta.oldValue} â†’ ${meta.newValue}`;
                  } else if (meta.newValue) {
                ...
                case 'promised_date_change':
                  if (meta.oldValue && meta.newValue) {
                    message = `Promised date changed: ${meta.oldValue} â†’ ${meta.newValue}`;
                  } else if (meta.newValue) {
                ...
                case 'job_label_change':
                  if (meta.oldValue && meta.newValue) {
                    message = `Job label changed: "${meta.oldValue}" â†’ "${meta.newValue}"`;
                  } else if (meta.newValue) {
                ...
                case 'po_number_change':
                  if (meta.oldValue && meta.newValue) {
                    message = `PO # changed: ${meta.oldValue} â†’ ${meta.newValue}`;
                  } else if (meta.newValue) {
                ...
                case 'line_item_status_change':
                  if (meta.lineItemId && meta.oldStatus && meta.newStatus) {
                    message = `Line item status changed: ${meta.oldStatus} â†’ ${meta.newStatus}`;
                  } else {
                ...
                default:
                  ...
                  if (row.fromStatus || row.toStatus) {
                    message = `${row.fromStatus || ''} â†’ ${row.toStatus || ''}`.trim();
                  } else {
      ...
      try {
        ...
        if (oIds.length) {
          ...
          for (const row of js) {
            ...
            const msg = oldLabel ? `Job status: ${oldLabel} â†’ ${label}` : `Job status: ${label}`;
            events.push({
