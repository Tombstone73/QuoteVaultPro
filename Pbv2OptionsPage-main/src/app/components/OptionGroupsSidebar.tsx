...
import type { OptionGroup } from '@/app/types';
      ...
      <ScrollArea className="flex-1">
        <div className="p-2">
          {optionGroups.map((group, index) => {
            const hasPricing = group.options.some(
              opt => opt.pricingBehavior.type !== 'none'
            );
            const hasProductionFlags = group.options.some(
              opt => opt.productionFlags.length > 0
            );
            const hasConditionals = group.options.some(
              opt => opt.conditionalLogic !== null
            );
