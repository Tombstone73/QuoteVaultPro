...
>(({ className = "", sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={`z-50 min-w-[8rem] overflow-hidden rounded-md border border-[#334155] bg-[#1e293b] p-1 text-slate-200 shadow-md ${className}`}
      {...props}
    />
  </DropdownMenuPrimitive.Portal>
))
...
>(({ className = "", ...props }, ref) => (
  <DropdownMenuPrimitive.Separator
    ref={ref}
    className={`-mx-1 my-1 h-px bg-[#334155] ${className}`}
    {...props}
  />
))
