...
export default function LocalBridgeSettings() {
  ...
  const create = useMutation({ mutationFn: async () => (await apiRequest("POST", "/api/local-bridge/admin/agents", { name })).json() as Promise<any>, onSuccess: (r: any) => { setToken(r.data.token); queryClient.invalidateQueries({ queryKey: ["/api/local-bridge/admin/agents"] }); } });
  const revoke = useMutation({ mutationFn: (id: string) => apiRequest("POST", `/api/local-bridge/admin/agents/${id}/revoke`), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["/api/local-bridge/admin/agents"] }) });
