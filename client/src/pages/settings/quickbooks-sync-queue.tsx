...
function EligibilityDisplay({ item }: { item: QueueItem }) {
  // Synced is the canonical accounting state. It must take precedence over a
  // generic blocked reason, which is only useful for records that still need
  // operator action.
  if (item.queueState === 'synced') return <span>Synced</span>;
  if (item.eligibility === 'syncable') return <Badge className="bg-green-600">Auto syncable</Badge>;
  if (item.eligibility === 'queueable') return <Badge variant="secondary">Queueable</Badge>;

  const status = item.status.trim().toLowerCase();
  const conciseReason = status === 'void' ? 'Void' : ['canceled', 'cancelled'].includes(status) ? 'Canceled' : item.ineligibleReason || 'Blocked';
  return item.canManualForce
    ? <span className="text-xs text-muted-foreground">Waiting for automatic sync window</span>
    : <span className="text-xs text-muted-foreground" title={item.ineligibleReason || undefined}>{conciseReason}</span>;
}
...
export default function QuickBooksSyncQueuePage() {
  ...
  const selectedForceable = selectedItems.filter((item) => item.canManualForce);
  ...
  const syncSelected = useMutation({
    mutationFn: async (explicit?: Array<{ id: string; resourceType: 'invoice' | 'payment' }>) => {
      const res = await fetch('/api/integrations/quickbooks/queue/sync-selected', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ items: explicit ?? selectedForceable }) });
      const data = await res.json();
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Local accounting sync work</CardTitle><CardDescription>Queueing is local and remains available while QuickBooks is disconnected. Force Sync immediately sends selected queued records; the automatic worker still waits for its stability window.</CardDescription></CardHeader><CardContent className="space-y-4">
