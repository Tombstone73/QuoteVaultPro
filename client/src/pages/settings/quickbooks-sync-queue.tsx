...
export default function QuickBooksSyncQueuePage() {
  ...
  return <div className="space-y-6">
    ...
    <Card><CardHeader><CardTitle>Local accounting sync work</CardTitle><CardDescription>Queueing is local and remains available while QuickBooks is disconnected. Sync now only transmits records that are already queued.</CardDescription></CardHeader><CardContent className="space-y-4">
      ...
      <div className="flex items-center justify-between text-sm"><span>{query.data?.data.totalCount ?? 0} matching records · {selectedCount} selected</span><div className="flex gap-2"><Button variant="outline" size="sm" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Previous</Button><Button variant="outline" size="sm" disabled={!query.data || page >= query.data.data.totalPages} onClick={() => setPage((value) => value + 1)}>Next</Button></div></div>
    </CardContent></Card>
  </div>;
}
