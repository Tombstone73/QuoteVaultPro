...
export default function ReportsPage() {
  ...
  return <main className="mx-auto max-w-6xl p-4 sm:p-6"><header className="flex flex-wrap items-end justify-between gap-3 border-b pb-5"><div><p className="text-sm font-medium text-primary">Reporting</p><h1 className="mt-1 text-2xl font-semibold tracking-tight">Reports</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Operational reports and validated analytical snapshots for your shop.</p></div></header>
    <section className="mt-6 rounded-xl border bg-card p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex gap-3"><ClipboardList className="mt-0.5 h-5 w-5 text-primary" /><div><h2 className="font-semibold">Daily Production List</h2><p className="mt-1 max-w-2xl text-sm text-muted-foreground">A print-ready list of open New and In Production orders, grouped by due-date urgency with Roll and Flatbed production breakdowns.</p></div></div><Button asChild><Link to={ROUTES.dailyProductionReport}>Open Daily Production List</Link></Button></div></section>
    <h2 className="mt-8 text-lg font-semibold">Saved reports</h2>
