                            ...
                            {filteredProducts.map((product) => (
                              <CommandItem
                                ...
                                onSelect={async () => {
                                  ...
                                  // Auto-create a draft/temporary line item when a product is selected.
                                  // - If quoteId exists: create a draft line item within the quote
                                  // - If quoteId is null (/quotes/new): create a temporary line item for artwork uploads
                                  if (!draftLineItemId) {
                                    try {
                                      setIsCreatingDraft(true);
                                      const response = quoteId
                                        ? await apiRequest("POST", `/api/quotes/${quoteId}/line-items`, {
                                        ...
                                        : await apiRequest("POST", "/api/line-items/temp", {
                                            productId: product.id,
                                            productName: product.name,
                                            productType: "wide_roll",
                                            width: requiresDimensions ? parseFloat(width || "0") : 1,
                                      ...
                                      console.error("Failed to create draft/temporary line item", err);
                                    } finally {
                                      setIsCreatingDraft(false);
                                    }
                                  } else if (quoteId) {
                                    // Update existing draft with new product (only supported when draft is tied to a quote)
                                    patchDraftLineItem({
                                      productId: product.id,
                                      productName: product.name,
                                      selectedOptions: [],
                                    ...
                                    // New quote + existing temporary draft: create a new temporary line item for the new product
                                    setDraftLineItemId(null);
                                  }
                                }}
                              >
