...
function ReportTable({ rows, title }: { rows: DailyProductionReportRow[]; title?: string }) {
  return <section className={title ? "daily-production-section mt-6" : "daily-production-section"}>
    ...
    <div className="overflow-x-auto rounded-lg border">
      <table className="w-full min-w-[760px] text-left text-sm">
        ...
        <tbody>{rows.length ? rows.map((row) => <tr key={`${title ?? "overview"}-${row.orderId}`} data-due-state={row.dueState} className="border-t align-top">
          ...
          <td className="px-3 py-3">{destinationLabel[row.destination]}</td><td className="px-3 py-3">{row.fulfillment}</td>
        </tr>) : <tr><td colSpan={7} className="px-3 py-8 text-center text-muted-foreground">No qualifying production items.</td></tr>}</tbody>
      </table>
    </div>
  </section>;
}
...
export default function DailyProductionListPage() {
  ...
  React.useEffect(() => { void (async () => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not load the Daily Production List"); }
  })(); }, []);
  ...
  const printStyles = <style>{`@media print {
    ...
    .daily-production-report [data-due-state="today"] { background: #ffedd5 !important; }
    .daily-production-report [data-due-state="tomorrow"] { background: #fef9c3 !important; }
  ...
  return <main className="daily-production-report mx-auto max-w-6xl p-4 sm:p-6">{printStyles}
    ...
    <section className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[
      ["Open Orders", report.summary.open], ["Due Today", report.summary.dueToday], ["Due Tomorrow", report.summary.dueTomorrow], ["Overdue", report.summary.overdue], ["No Due Date", report.summary.noDueDate],
    ].map(([label, value]) => <div key={String(label)} className="rounded-lg border bg-card p-4"><div className="text-xs font-medium uppercase tracking-wide text-muted-foreground">{label}</div><div className="mt-1 text-2xl font-bold tabular-nums">{value}</div></div>)}</section>
    ...
    {view === "overview" ? <ReportTable rows={report.overview} /> : <><ReportTable title="Roll" rows={report.roll} /><ReportTable title="Flatbed" rows={report.flatbed} /></>}
  </main>;
}
