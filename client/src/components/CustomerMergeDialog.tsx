...
export function CustomerMergeDialog({ customerIds, open, onOpenChange }: { customerIds: string[]; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const merge = useMutation({
    mutationFn: async () => {
      const response = await fetch("/api/customers/merge", {
        ...
        body: JSON.stringify({ survivorCustomerId, sourceCustomerIds: customerIds.filter((id) => id !== survivorCustomerId), fieldChoices, reviewed: true }),
      });
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
      ...
      <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button disabled={!preview || !survivorCustomerId || !confirmed || merge.isPending} onClick={() => merge.mutate()}>Merge into {survivorName}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
