...
export default function CurrentCustomerStatement({ customerId }: { customerId: string }) {
  ...
  return <section className="space-y-5 p-4 print:p-0" data-testid="customer-current-statement">
    <header className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="text-xl font-semibold">Customer Statement</h2><p className="text-sm text-muted-foreground">{data.customer.companyName} · Statement date {data.statementDate}</p>{data.customer.billingAddress ? <p className="text-sm text-muted-foreground">{data.customer.billingAddress}</p> : null}</div><div className="flex w-full flex-wrap justify-start gap-2 print:hidden sm:w-auto sm:justify-end" data-testid="customer-statement-actions"><Button variant="outline" onClick={print} aria-label="Print customer statement"><Printer className="mr-2 h-4 w-4" />Print</Button><Button variant="outline" onClick={download} aria-label="Download customer statement PDF"><Download className="mr-2 h-4 w-4" />Download PDF</Button><Button variant="outline" onClick={() => { setEmailResult(null); setComposeError(null); setEmailOpen(true); }} aria-label="Email customer statement"><Mail className="mr-2 h-4 w-4" />Email Statement</Button></div></header>
