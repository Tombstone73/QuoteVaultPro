...
export default function CurrentCustomerStatement({ customerId }: { customerId: string }) {
  ...
  const emailStatement = useMutation({ mutationFn: async (recipientEmails: string[]) => {
    const response = await apiFetch(`${statementPath}/email`, { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ recipientEmails, idempotencyKey: crypto.randomUUID() }) });
  ...
  useEffect(() => {
    if (emailOpen && recipients.data) setSelectedRecipients(recipients.data.map((recipient) => recipient.email));
  }, [emailOpen, recipients.data]);
