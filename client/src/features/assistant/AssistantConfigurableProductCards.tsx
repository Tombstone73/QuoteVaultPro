...
const matrixCellKey = (row: string, column: string) => `${encodeURIComponent(row.trim().replace(/\s+/g, " "))}:${encodeURIComponent(column.trim().replace(/\s+/g, " "))}`;

export type ConfigurableProductConfirmationCard = {
  proposalId: string; fingerprint: string; name: string; category: string; measurementMode: "dimensions_required" | "fixed_size" | "quantity_only" | "unresolved"; sheetWidthIn: number | null; sheetHeightIn: number | null;
  allowRotation: boolean | null; route: string | null; minimumChargeCents: number | null; pricingComponent: "per_piece" | "per_square_foot" | "unresolved"; optionGroups: Array<{ key: string; name: string; values: Array<{ value: string; label: string }> }>;
  rows: string[]; columns: string[]; cells: Record<string, number>; warnings: string[]; blockers: string[]; ready: boolean;
};
export type ConfigurableProductResultCard = { proposalId: string; productId: string; treeId: string; editorLink: string | null; optionGroupCount: number; optionValueCount: number; matrixRowCount: number; matrixColumnCount: number; matrixCellCount: number; warnings: string[]; blockers: string[]; reused: boolean; };
...
export function toConfigurableProductConfirmation(value: unknown): ConfigurableProductConfirmationCard | null {
  ...
  // v1 cards without these generalized fields are historical per-square-foot
  // envelopes; retain their prior semantics while new cards are explicit.
  const pricingComponent = matrix.pricingComponent === "per_piece" || matrix.pricingComponent === "per_square_foot" ? matrix.pricingComponent : matrix.pricingComponent === "unresolved" ? "unresolved" : "per_square_foot";
  const measurementMode = product.measurementMode === "dimensions_required" || product.measurementMode === "fixed_size" || product.measurementMode === "quantity_only" || product.measurementMode === "unresolved" ? product.measurementMode : product.requiresDimensions === false ? "quantity_only" : "dimensions_required";
  if (!proposalId || !/^[a-f0-9-]{16,}$/i.test(proposalId) || !fingerprint || !/^[a-f0-9]{64}$/i.test(fingerprint) || !name || !category || (product.minimumChargeCents !== undefined && minimumChargeCents === null) || optionGroups.length !== 2 || rows.length === 0 || columns.length === 0 || !rawCells) return null;
  const cells: Record<string, number> = {};
  for (const row of rows) for (const column of columns) { const key = matrixCellKey(row, column); const value = cents(rawCells[key]); if (value === null) return null; cells[key] = value; }
  const blockers = list(dto.blockers); const ready = readiness.ready === true && blockers.length === 0 && dto.goEligible === true;
  return { proposalId, fingerprint, name, category, measurementMode, sheetWidthIn, sheetHeightIn, allowRotation: typeof product.allowRotation === "boolean" ? product.allowRotation : null, route, minimumChargeCents, pricingComponent, optionGroups, rows, columns, cells, warnings: list(dto.warnings), blockers, ready };
}
...
export function toConfigurableProductResult(value: unknown): ConfigurableProductResultCard | null {
  ...
  const proposalId = text(dto.proposalId); const productId = text(dto.productId); const treeId = text(dto.pbv2TreeVersionId); const editorLink = text(dto.editorLink);
  ...
  return { proposalId, productId, treeId, editorLink: editorLink?.startsWith("/") ? editorLink : null, optionGroupCount: dto.optionGroupCount as number, optionValueCount: dto.optionValueCount as number, matrixRowCount: dto.matrixRowCount as number, matrixColumnCount: dto.matrixColumnCount as number, matrixCellCount: dto.matrixCellCount as number, warnings: list(dto.warnings), blockers: list(dto.blockers), reused: dto.reused };
}
...
const pricingLabel = (unit: ConfigurableProductConfirmationCard["pricingComponent"]) => unit === "per_piece" ? "Per piece" : unit === "per_square_foot" ? "Per square foot" : "Not set";
const measurementLabel = (mode: ConfigurableProductConfirmationCard["measurementMode"]) => mode === "dimensions_required" ? "Dimensions required" : mode === "fixed_size" ? "Fixed size" : mode === "quantity_only" ? "Quantity only" : "Not set";
...
export function ConfigurableProductConfirmationCardView({ confirmation, onCreatePlan, creating }: { confirmation: ConfigurableProductConfirmationCard; onCreatePlan?: () => void; creating?: boolean }) {
  return <section className="mt-2 rounded-md border border-primary/25 bg-background/80 p-3 text-xs" aria-label={`Configurable product confirmation: ${confirmation.name}`}>
    ...
    <dl className="mt-2 grid gap-1 sm:grid-cols-2"><div><dt className="inline font-medium">Lifecycle: </dt><dd className="inline">Inactive · PBV2 DRAFT · Unpublished</dd></div><div><dt className="inline font-medium">Measurement: </dt><dd className="inline">{measurementLabel(confirmation.measurementMode)}</dd></div><div><dt className="inline font-medium">Pricing: </dt><dd className="inline">{pricingLabel(confirmation.pricingComponent)}</dd></div><div><dt className="inline font-medium">Sheet size: </dt><dd className="inline">{confirmation.sheetWidthIn !== null && confirmation.sheetHeightIn !== null ? `${confirmation.sheetWidthIn} × ${confirmation.sheetHeightIn} in` : "Not set"}</dd></div><div><dt className="inline font-medium">Rotation: </dt><dd className="inline">{confirmation.allowRotation === null ? "Not set" : confirmation.allowRotation ? "Allowed" : "Not allowed"}</dd></div><div><dt className="inline font-medium">Production route: </dt><dd className="inline">{confirmation.route ?? "Not set"}</dd></div><div><dt className="inline font-medium">Minimum charge: </dt><dd className="inline">{confirmation.minimumChargeCents === null ? "Not set" : money(confirmation.minimumChargeCents)}</dd></div></dl>
    ...
    <div className="mt-3 overflow-x-auto"><p className="font-medium">{pricingLabel(confirmation.pricingComponent)} pricing matrix</p><table className="mt-1 w-full min-w-[28rem] border-collapse text-left"><thead><tr><th className="border-b p-1">{confirmation.optionGroups[0].name}</th>{confirmation.columns.map((column) => <th key={column} className="border-b p-1">{confirmation.optionGroups[1].values.find((value) => value.value === column)?.label ?? column}</th>)}</tr></thead><tbody>{confirmation.rows.map((row) => <tr key={row}><th className="border-b p-1">{row}</th>{confirmation.columns.map((column) => <td key={column} className="border-b p-1">{money(confirmation.cells[matrixCellKey(row, column)])}</td>)}</tr>)}</tbody></table></div>
...
export function ConfigurableProductResultCardView({ result }: { result: ConfigurableProductResultCard }) { return <section className="mt-3 rounded border border-primary/25 bg-primary/5 p-3 text-xs" aria-label="Configurable product execution result"><p className="font-semibold">Configurable product draft created</p><p className="mt-1">Product ID: <span className="font-mono">{result.productId}</span></p><p>PBV2 tree/version ID: <span className="font-mono">{result.treeId}</span></p><p className="mt-1">Inactive · PBV2 DRAFT · Unpublished · Not live-quotable</p><p className="mt-1">{result.optionGroupCount} option groups, {result.optionValueCount} option values; {result.matrixRowCount} × {result.matrixColumnCount} pricing matrix ({result.matrixCellCount} cells).</p><p className="mt-1">{result.reused ? "Replay returned the original product and tree IDs; no duplicate product was created." : "Created once from the confirmed persisted proposal."}</p>{result.editorLink ? <p className="mt-2"><a className="text-primary underline-offset-2 hover:underline" href={result.editorLink}>Open this exact PBV2 DRAFT in Product Builder</a></p> : null}<Messages title="Warnings" values={result.warnings} tone="warning" /><Messages title="Blockers" values={result.blockers} tone="blocker" /></section>; }
