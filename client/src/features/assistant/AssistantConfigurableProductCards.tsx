...
export type ConfigurableProductConfirmationCard = {
  proposalId: string; fingerprint: string; name: string; category: string; requiresDimensions: boolean; sheetWidthIn: number | null; sheetHeightIn: number | null;
  allowRotation: boolean | null; route: string | null; minimumChargeCents: number; pricingComponent: "per_piece" | "per_square_foot" | "unresolved"; optionGroups: Array<{ key: string; name: string; values: Array<{ value: string; label: string }> }>;
...
export function toConfigurableProductConfirmation(value: unknown): ConfigurableProductConfirmationCard | null {
  ...
  const requiresDimensions = product.requiresDimensions === false ? false : true;
  if (!proposalId || !/^[a-f0-9-]{16,}$/i.test(proposalId) || !fingerprint || !/^[a-f0-9]{64}$/i.test(fingerprint) || !name || !category || minimumChargeCents === null || optionGroups.length !== 2 || rows.length === 0 || columns.length === 0 || !rawCells) return null;
  ...
  return { proposalId, fingerprint, name, category, requiresDimensions, sheetWidthIn, sheetHeightIn, allowRotation: typeof product.allowRotation === "boolean" ? product.allowRotation : null, route, minimumChargeCents, pricingComponent, optionGroups, rows, columns, cells, warnings: list(dto.warnings), blockers, ready };
}
...
export function ConfigurableProductConfirmationCardView({ confirmation, onCreatePlan, creating }: { confirmation: ConfigurableProductConfirmationCard; onCreatePlan?: () => void; creating?: boolean }) {
  return <section className="mt-2 rounded-md border border-primary/25 bg-background/80 p-3 text-xs" aria-label={`Configurable product confirmation: ${confirmation.name}`}>
    ...
    <dl className="mt-2 grid gap-1 sm:grid-cols-2"><div><dt className="inline font-medium">Lifecycle: </dt><dd className="inline">Inactive · PBV2 DRAFT · Unpublished</dd></div><div><dt className="inline font-medium">Measurement: </dt><dd className="inline">{confirmation.requiresDimensions ? "Dimensions required" : "Quantity only"}</dd></div><div><dt className="inline font-medium">Pricing: </dt><dd className="inline">{pricingLabel(confirmation.pricingComponent)}</dd></div><div><dt className="inline font-medium">Sheet size: </dt><dd className="inline">{confirmation.sheetWidthIn !== null && confirmation.sheetHeightIn !== null ? `${confirmation.sheetWidthIn} × ${confirmation.sheetHeightIn} in` : "Not set"}</dd></div><div><dt className="inline font-medium">Rotation: </dt><dd className="inline">{confirmation.allowRotation === null ? "Not set" : confirmation.allowRotation ? "Allowed" : "Not allowed"}</dd></div><div><dt className="inline font-medium">Production route: </dt><dd className="inline">{confirmation.route ?? "Not set"}</dd></div><div><dt className="inline font-medium">Minimum charge: </dt><dd className="inline">{money(confirmation.minimumChargeCents)}</dd></div></dl>
