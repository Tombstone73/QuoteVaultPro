...
export function ResultCards({
  ...
  const publicDiagnosticReference = cards.flatMap((card: any) => [card.summary, card.body, ...(Array.isArray(card.details?.errors) ? card.details.errors : [])]).find((value) => typeof value === "string" && /\b(?:aip|pic|pso)-[0-9a-f-]{36}\b/i.test(value))?.match(/\b(?:aip|pic|pso)-[0-9a-f-]{36}\b/i)?.[0] ?? null;
