...
export function AssistantPlanCard({
  ...
  const actionLabel = plan.quoteInternalNote ? "Add internal quote note" : isPbv2PricingEdit ? (plan.pbv2PricingEdit?.kind === "matrix" ? "Replace inactive pricing matrix" : "Replace inactive quantity tiers") : isCloneInactiveDraft ? "Clone to inactive product draft" : isConfigurableProduct ? "Create configurable inactive product draft" : isProductPricingRollback ? "Roll back product pricing" : isProductPricingChangeSet ? "Adjust product pricing" : isQuoteDraftUpdate ? "Update draft quote" : isQuoteDraft ? "Create draft quote" : isProductDraftUpdate ? "Update inactive product draft" : isProductDraft ? "Create inactive product draft" : genericActionLabel ?? "Proposed action";
