...
export type AssistantPlanCardModel = {
  ...
  cloneInactiveDraft: { sourceName: string | null; productName: string | null; beforePricing: UnknownRecord | null; afterPricing: UnknownRecord | null; category: string | null; measurementMode: string | null; productionRoute: string | null; sheetSettings: string | null; rotation: boolean | null; options: string | null; resultPath: string | null } | null;
...
function toCloneInactiveDraft(action: string | null, preview: UnknownRecord | null, details: UnknownRecord | null): AssistantPlanCardModel["cloneInactiveDraft"] {
  ...
  const sourceTree = asRecord(asRecord(clone?.source)?.pbv2Tree);
  const resultProduct = asRecord(asRecord(clone?.result)?.product);
  const pricing = asRecord(clone?.basePricing);
  const configuration = asRecord(sourceProduct?.configuration);
  const treeMeta = asRecord(asRecord(sourceTree?.treeJson)?.meta);
  const optionNames = Array.isArray(treeMeta?.optionGroups) ? treeMeta!.optionGroups.flatMap((item) => asText(asRecord(item)?.name)) : [];
  const resultPath = asText(asRecord(details?.cloneInactiveDraft)?.editorLink);
  return {
    sourceName: asText(sourceProduct?.name), productName: asText(resultProduct?.name), beforePricing: asRecord(pricing?.before), afterPricing: asRecord(pricing?.after),
    category: asText(sourceProduct?.category), measurementMode: asText(sourceProduct?.measurementMode), productionRoute: asText(configuration?.productionRoute),
    sheetSettings: asText(configuration?.sheetOrRollConstraints), rotation: typeof configuration?.allowRotation === "boolean" ? configuration.allowRotation : null,
    options: optionNames.length ? optionNames.join(", ") : "None", resultPath: resultPath?.startsWith("/") ? resultPath : null,
  };
}

function clonePricingRows(before: UnknownRecord | null, after: UnknownRecord | null): Array<{ label: string; before: string; after: string }> {
  return [["perSqftCents", "Price per square foot"], ["perPieceCents", "Price per piece"], ["minimumChargeCents", "Minimum charge"]].flatMap(([key, label]) => {
    const prior = toCents(before?.[key]); const next = toCents(after?.[key]);
    return prior !== null && next !== null && prior !== next ? [{ label, before: moneyFromCents(prior), after: moneyFromCents(next) }] : [];
  });
}
...
export function AssistantPlanCard({
  ...
  const isCanonicalProductDraft = plan.action === "products.create_from_canonical_intent";
  const isProductDraft = isCanonicalProductDraft || plan.action === "products.create_inactive_draft" || plan.action === "products.update_inactive_draft";
  ...
  const hasConfirmableDraft = Boolean(plan.quoteInternalNote?.noteText || (isQuoteDraft && plan.preview && plan.missingInformation.length === 0 && !plan.partialFailureSummary) || (isProductDraft && plan.preview && plan.missingInformation.length === 0 && !plan.partialFailureSummary && (!isProductDraftUpdate || Boolean(plan.productDraftUpdate && plan.productDraftUpdate.changes.length > 0 && plan.productDraftUpdate.validationErrors.length === 0))) || (isConfigurableProduct && plan.configurableProduct?.ready && plan.missingInformation.length === 0 && !plan.partialFailureSummary) || (isCloneInactiveDraft && plan.cloneInactiveDraft?.productName && plan.missingInformation.length === 0 && !plan.partialFailureSummary) || (isPbv2PricingEdit && plan.pbv2PricingEdit?.productId && plan.missingInformation.length === 0 && !plan.partialFailureSummary) || (isProductPricingChangeSet && plan.preview && plan.missingInformation.length === 0 && !plan.partialFailureSummary && Boolean(plan.productPricingChangeSet?.rows.length)) || (isProductPricingRollback && plan.preview && plan.missingInformation.length === 0 && !plan.partialFailureSummary && Boolean(plan.productPricingRollback?.rows.length)) || (genericPlan && plan.preview && plan.missingInformation.length === 0 && !plan.partialFailureSummary));
  ...
  const actionLabel = plan.quoteInternalNote ? "Add internal quote note" : isPbv2PricingEdit ? (plan.pbv2PricingEdit?.kind === "matrix" ? "Replace inactive pricing matrix" : "Replace inactive quantity tiers") : isCloneInactiveDraft ? "Clone to inactive product draft" : isConfigurableProduct ? "Create configurable inactive product draft" : isProductPricingRollback ? "Roll back product pricing" : isProductPricingChangeSet ? "Adjust product pricing" : isQuoteDraftUpdate ? "Update draft quote" : isQuoteDraft ? "Create draft quote" : isProductDraftUpdate ? "Update inactive product draft" : isCanonicalProductDraft ? "Create canonical inactive product draft" : isProductDraft ? "Create inactive product draft" : genericActionLabel ?? "Proposed action";
  const goLabel = isPbv2PricingEdit ? (plan.pbv2PricingEdit?.kind === "matrix" ? "GO replace inactive matrix" : "GO replace inactive quantity tiers") : isCloneInactiveDraft ? "GO clone inactive draft" : isConfigurableProduct ? "GO create configurable inactive draft" : isProductPricingRollback ? "GO roll back product pricing" : isProductPricingChangeSet ? "GO adjust product pricing" : isQuoteDraftUpdate ? "GO update inactive draft" : isQuoteDraft ? "GO create draft quote" : isProductDraftUpdate ? "GO update inactive draft" : isCanonicalProductDraft ? "GO create canonical inactive draft" : isProductDraft ? "GO create inactive draft" : "GO add internal note";
  ...
  return <section className="mt-2 rounded-md border border-primary/25 bg-background/80 p-3 text-xs" aria-label={`Execution plan: ${plan.title}`}>
    ...
    {plan.cloneInactiveDraft ? <div className="mt-3 rounded border border-primary/20 bg-primary/5 p-3"><p className="font-semibold">Clone configuration</p><p className="mt-1"><span className="font-medium">Source product:</span> {plan.cloneInactiveDraft.sourceName || "Source product"}</p><p><span className="font-medium">New product:</span> {plan.cloneInactiveDraft.productName || "Inactive clone"}</p><p className="mt-1 text-muted-foreground">PBV2 configuration is copied as an exact DRAFT snapshot. Source, activation, and publication are excluded.</p>{(() => { const rows = clonePricingRows(plan.cloneInactiveDraft.beforePricing, plan.cloneInactiveDraft.afterPricing); return rows.length ? <div className="mt-2 overflow-x-auto"><p className="font-medium">Changes</p><table className="mt-1 w-full text-left"><thead className="text-muted-foreground"><tr><th className="border-b p-1 font-medium">Field</th><th className="border-b p-1 font-medium">Before</th><th className="border-b p-1 font-medium">After</th></tr></thead><tbody>{rows.map((row) => <tr key={row.label}><th className="border-b p-1 font-medium">{row.label}</th><td className="border-b p-1">{row.before}</td><td className="border-b p-1">{row.after}</td></tr>)}</tbody></table></div> : null; })()}<div className="mt-2"><p className="font-medium">Unchanged</p><ul className="mt-1 list-disc space-y-0.5 pl-4"><li>Measurement: {plan.cloneInactiveDraft.measurementMode === "dimensions_required" ? "Width and height required" : plan.cloneInactiveDraft.measurementMode === "quantity_only" ? "No dimensions required" : "Not set"}</li><li>Category: {plan.cloneInactiveDraft.category ?? "Not set"}</li><li>Production route: {plan.cloneInactiveDraft.productionRoute ?? "Not set"}</li><li>Sheet settings: {plan.cloneInactiveDraft.sheetSettings ?? "Not set"}</li><li>Rotation: {plan.cloneInactiveDraft.rotation === null ? "Not set" : plan.cloneInactiveDraft.rotation ? "Allowed" : "Not allowed"}</li><li>Options: {plan.cloneInactiveDraft.options ?? "Not set"}</li><li>Lifecycle: Inactive PBV2 DRAFT</li></ul></div>{plan.status === "succeeded" && plan.cloneInactiveDraft.resultPath ? <p className="mt-2"><a className="text-primary underline-offset-2 hover:underline" href={plan.cloneInactiveDraft.resultPath}>Open the exact cloned draft in Product Builder</a></p> : null}</div> : null}
