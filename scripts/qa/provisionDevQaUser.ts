...
import { devQaFullAccessProvisioningPlan, devQaM78iFixtureArtworkProvisioningPlan, devQaM78iFixturePricingProvisioningPlan, devQaM78iOperationalProvisioningPlan, devQaM78iPermissionFloorProvisioningPlan } from "../../server/lib/devQaFullAccessProvisioning";
...
async function provision() {
  ...
  if (permissionProfile !== "full" && permissionProfile !== "m78i" && permissionProfile !== "m78i_fixture_pricing" && permissionProfile !== "m78i_fixture_artwork") {
    throw new Error("PRINTERSHERO_DEV_QA_PERMISSION_PROFILE must be 'full', 'm78i', 'm78i_fixture_pricing', or 'm78i_fixture_artwork'.");
  }
  const plan = permissionProfile === "m78i" ? devQaM78iOperationalProvisioningPlan(config) : permissionProfile === "m78i_fixture_pricing" ? devQaM78iFixturePricingProvisioningPlan(config) : permissionProfile === "m78i_fixture_artwork" ? devQaM78iFixtureArtworkProvisioningPlan(config) : devQaFullAccessProvisioningPlan(config);
  const permissionFloorPlan = permissionProfile === "m78i" || permissionProfile === "m78i_fixture_pricing" || permissionProfile === "m78i_fixture_artwork" ? devQaM78iPermissionFloorProvisioningPlan(config) : undefined;
  ...
  return db.transaction(async (tx) => {
    ...
    if ((permissionProfile === "m78i" || permissionProfile === "m78i_fixture_pricing" || permissionProfile === "m78i_fixture_artwork") && organization.name !== "PrintersHero M7 QA") {
    ...
    if (existingUser) {
      const otherMembership = await tx.select({ organizationId: userOrganizations.organizationId }).from(userOrganizations).where(and(eq(userOrganizations.userId, existingUser.id), ne(userOrganizations.organizationId, config.organizationId))).limit(1);
      if (otherMembership.length > 0) throw new Error("Configured DEV QA email already has another organization membership; refusing to modify it.");
    }
    ...
    if (!user) throw new Error("DEV QA Browser could not be created.");
    ...
    // V2 assignments require this legacy membership safety column, which predates the shared Drizzle projection.
